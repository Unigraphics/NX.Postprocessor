...
#  Makino.tcl -
#
#  Created by chypchar  @  Fri Sep 07 17:04:19 2007 Eastern Daylight Time
...
########################################################################
  ...
  if { ![info exists mom_sys_post_initialized] } {
   ...
   ####  Listing File variables 
     ...
     set mom_sys_output_file_suffix                "MAKIN"
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_post_description                  "3-axis HS Makino with Carbon Cutting options"
...
#=============================================================
   PB_CMD_URC_Set_Makino_Coolant_Option_OFF
   PB_CMD_URC_Set_Makino_HighSpeed_Option_OFF
   PB_CMD_URC_Set_Home_Position_EOP
   MOM_do_template spindle_off
   PB_CMD_URC_Set_Makino_GRAPHITE_Option_OFF
   MOM_do_template end_of_program_1
   MOM_set_seq_off
   MOM_do_template rewind_stop_code
   PB_CMD_URC_Mach_Time
...
#=============================================================
   CIRCLE_SET
   PB_CMD_URC_Set_Cutcom_Register
...
#=============================================================
  ...
  global mom_motion_event
   ...
   RAPID_SET
   PB_CMD_URC_Set_Cutcom_Register
   set rapid_spindle_blk {G_adjust G_mode G_motion X Y Z F H}
   set rapid_spindle_x_blk {G_adjust G_mode G_motion X F H}
   set rapid_spindle_y_blk {G_adjust G_mode G_motion Y F H}
   set rapid_spindle_z_blk {G_adjust G_mode G_motion Z F H}
   set rapid_traverse_blk {G_mode G_motion G X Y Z F}
   set rapid_traverse_xy_blk {G_mode G_motion G X Y F}
   set rapid_traverse_yz_blk {G_mode G_motion G Y Z F}
   set rapid_traverse_xz_blk {G_mode G_motion G X Z F}
   ...
   PB_CMD_URC_Set_Makino_HighSpeed_Option_ON
}
...
#=============================================================
   PB_CMD_URC_Set_Makino_Coolant_Option_OFF
   PB_CMD_URC_Set_Makino_HighSpeed_Option_OFF
   PB_CMD_URC_Makino_Tool_Change_Header
   PB_CMD_tool_change_force_addresses
   PB_CMD_URC_Set_Home_Position_OSS
   MOM_force Once T
   MOM_do_template tool_change_1
   MOM_force Once M
   MOM_do_template auto_tool_change
   MOM_do_template tool_change_2
   PB_CMD_URC_Set_Makino_GRAPHITE_Option_ON
   MOM_force Once S M_spindle
   MOM_do_template spindle_rpm
   PB_CMD_URC_Set_Makino_Coolant_Option_ON
}
...
#=============================================================
   ...
   PB_CMD_URC_Get_Makino_Post_Options
   PB_CMD_URC_Init_Makino_Option_Flags
   PB_CMD_URC_Set_Makino_Header
   PB_CMD_URC_Init_Tool_Time
...
#=============================================================
proc PB_CMD_URC_Calc_Tool_Time { } {
#=============================================================
#
#
# Arguments:    none
#
#
# Insert Point: Locate this procedure at the very bottom of the End-of-Path
#               event of the Operation End sequence.
#
# Purpose:      Builds a list of machine times, per tool and operation.
#
#==============================================================================
#
global  mom_tool_name
global  mom_machine_time
global  prev_machine_time
global  mom_operation_name
global  machine_times
global  num_ops


#------------------------------------------------------------------------------
#   Track the number of operations - if this is the end of the 1st operation,
#   then this variable does not yet exist - set it to zero, as well as the
#   previous machine time.
#------------------------------------------------------------------------------
#
if { ![info exists num_ops] } {
   set num_ops 0
   set prev_machine_time 0
}


incr num_ops    ; # Increment the counter

#------------------------------------------------------------------------------
#   "mom_machine_time" tracks the total time of all operations.  By saving
#   the machine time at the end of each operation ("prev_machine_time") and
#   subtracting this value from the current running total ("mom_machine_time")
#   we can derive the machining time for the current tool/operation.
#------------------------------------------------------------------------------
#
set curr_machine_time [expr $mom_machine_time - $prev_machine_time]
set prev_machine_time $mom_machine_time


#------------------------------------------------------------------------------
#   Store the tool name, operation name and calculated machining time for
#   this tool/operation in a 1X3 array.
#------------------------------------------------------------------------------
#
set machine_times($num_ops,1) $mom_tool_name
set machine_times($num_ops,2) $mom_operation_name
set machine_times($num_ops,3) $curr_machine_time
}


#=============================================================
proc PB_CMD_URC_Get_Makino_Post_Options { } {
#=============================================================
#
# Arguments:    none
#
#
# Insert Point: Locate this procedure immediately at the top of the
#               Start-Of-Program section, just below (after) the
#               "PB_CMD_URC_AutoFileName" proc.
#
# Purpose:      Calls the external program: Makino_Post_Options.tcl which
#               is used to get the various options that the operator wants
#               to use during posting.
#
#
#           OPTION:             SETTINGS
#           ===================================================================
...
#
#           COOLANT:            OFF             (default)
#                               FLOOD           (select from pulldown)
#                               MIST            (select from pulldown)
#                               TAP             (select from pulldown)
#                               THROUGH         (select from pulldown)
#
#           INFO WINDOW OUTPUT  ON              (default)
#                               OFF             (select from pulldown)
#
#==============================================================================

    #--------------------------------------------------------------------------
    #   Define all of the global parameters
    #--------------------------------------------------------------------------
    #
    global  post_dir
    global  UC
    ...
    #--------------------------------------------------------------------------
    #   Get the UG base dir and create a variable pointing to ugwish.exe
    #--------------------------------------------------------------------------
    #
    set base_dir [MOM_ask_env_var UGII_BASE_DIR]

    set run_program "$base_dir\\MACH\\auxiliary\\ugwish.exe"


    #--------------------------------------------------------------------------
    #   Get the UG CAM base dir and create a variable pointing to the custom
    #   TCL program called: Makino_Post_Options.tcl and then run this program.
    #
    #   NOTE:
    #   The execution syntax of "set returned [exec $run_program $get_info ]"
    #   was used in order to pass the results of list "returned" back to
    #   this custom command inside of PostBuilder.
    #--------------------------------------------------------------------------
    #
    set post_dir [MOM_ask_env_var UGII_CAM_POST_DIR]

    set get_info "$post_dir\\Makino_Post_Options.tcl"

    set returned [exec $run_program $get_info ]


    #--------------------------------------------------------------------------
    #   The variable "returned" is a list containing the post options selected
    #   by the user.
    #
    #   The 1st item in the list (0) is the User Comment        (UC)
    #   The 2nd item in the list (1) is the High Speed Option   (HS)
    #   The 3rd item in the list (2) is the Circular Interp     (CI)
    #   The 4th item in the list (3) is the Graphite Option     (GR)
    #   The 5th item in the list (4) is the Coolant Option      (CL)
    #   The 6th item in the list (5) is the Info Window Option  (IW)
    #--------------------------------------------------------------------------
    #
    set UC [lindex $returned 0]
    set HS [lindex $returned 1]
    set CI [lindex $returned 2]
    set GR [lindex $returned 3]
    set CL [lindex $returned 4]
    set IW [lindex $returned 5]


    #--------------------------------------------------------------------------
    #   Now that we have these parameters we can set the circular interpolation
    #   mode.
    #--------------------------------------------------------------------------
    #
    if {$CI == "OFF"} {
    ...
    MOM_reload_kinematics   ;   # This loads these changes into the post
}


#=============================================================
proc PB_CMD_URC_Init_Makino_Option_Flags { } {
#=============================================================
#
# Arguments:    none
#
...
# Insert Point: Locate this procedure immediately at the top of the
#               Start-Of-Program section, just below (after) the
#               "PB_CMD_URC_Get_Makino_Post_Options" proc.
#
# Purpose:      Sets various flags to control the output of the Electrode,
#               and Graphite options such that they only occur at the 1st
#               tool change.  Also controls the High Speed option's "OFF"
#               command (G05 P0) such that it is not output at the 1st tool
#               change as well as the COOLANT option.
#
#==============================================================================
#
    ...
    #--------------------------------------------------------------------------
    #   Set the flags to TRUE (1) so that output will be generated only at the
    #   1st tool change.
    #--------------------------------------------------------------------------
    #
    set ST_Flag 1
...
#=============================================================
proc PB_CMD_URC_Init_Tool_Time { } {
#=============================================================
#
#
# Arguments:    none
#
#
# Insert Point: Locate this procedure at the very bottom of the Start-of-Program
#               event of the Program Start sequence.
#
# Purpose:      Sets the default toolchange time to 15 secs.  Edit this value
#               as required on a per-machine basis
#
#==============================================================================
#
global mom_kin_tool_change_time

set mom_kin_tool_change_time 15
MOM_reload_kinematics
}


#=============================================================
proc PB_CMD_URC_Mach_Time { } {
#=============================================================
#	
#
# Arguments:    none
#	
#
# Insert Point: Locate this procedure at the very bottom (last item) in the
#               End-Of-Program block of the Program End Sequence.
#
# Purpose:  Outputs the machining time for each operation as well as the total
#           time to the information window.
#           
#	
#==============================================================================
#
...
global  output_extn
global  machine_time_filename
global  tod


#------------------------------------------------------------------------------
#   Set the Time-Of-Day to read as <Full-day of the week> (%A), <Full Month> (%B)
#   <Day> (%d)/<Full Year> (%Y)   <hours> (%I):<minutes> (%M):<seconds> (%S)
#   <AM or PM> (%p)
#------------------------------------------------------------------------------
#
set tod [clock format [clock seconds] -format {%A, %B %d/%Y   %I:%M:%S%p}]



set machine_time_filename "${mom_output_file_directory}${mom_output_file_basename}_MachTime${output_extn}"


#   Call the machine tool-time calculation procedure.  NOTE: This proc sends
#   output to the listing device as well - separate its output with 2 blank
#   lines from that of the posted data.
#
PB_CMD_URC_Ouput_Tool_Time
}


#=============================================================
proc PB_CMD_URC_Makino_Tool_Change_Header { } {
#=============================================================
#
# Arguments:    none
#
#
# Insert Point: Locate this proc in the Auto Tool Change event, just after the
#               "PB_CMD_URC_Set_Makino_HighSpeed_Option_OFF" proc of the
#               Operation-Start-Sequence.
#
#
# Purpose:      Outputs a header of tooling information at each tool change.
#
#==============================================================================
#
    #--------------------------------------------------------------------------
    ...
    MOM_output_literal "( CUR. TOOL:  T$mom_tool_number)"
    MOM_output_literal "( TOOL NAME:  $mom_tool_name)"
    MOM_output_literal "( TOOL DIA.:  [format "%.4f" $mom_tool_diameter])"

    if { [ info exists mom_tool_corner1_radius ] } {

        if {$mom_tool_corner1_radius != 0} {
            MOM_output_literal "( TOOL RAD.:  [format "%.4f" $mom_tool_corner1_radius])"

        } else {
            MOM_output_literal "( TOOL RAD.:  0.0)"
        }
...
#=============================================================
proc PB_CMD_URC_Ouput_Tool_Time { } {
#=============================================================
#
#
# Arguments:    none
#
#
# Insert Point: This program is called by the PB_CMD_URC_Mach_Time command so
#               there is no need to install it anywhere.
#
# Purpose:      Outputs the machine tool time of each tool/operation to both
#               the listing device (NX information window) as well as to an
#               external file.  The file name uses the numeric portion of the
#               program group with a suffix of "_MachTime".  The name is
#               derived by the PB_CMD_URC_AutoFileName procedure.
#
#==============================================================================
#
global  machine_times
...
#------------------------------------------------------------------------------
#   Create the machine-time file - overwrite it if it already exists.
#------------------------------------------------------------------------------
#
set time_file [open $machine_time_filename w]


#------------------------------------------------------------------------------
#   Set up a header for the machine time file and write the header to the file.
#------------------------------------------------------------------------------
#
set separator   "==============================================================================="
...
set title       "                            MACHINE-TIME OUTPUT"

set fname         "File Name  :  $machine_time_filename"

set time_written  "Created on :  $tod"

set header [format "%-32s%-32s%-14s" "TOOL NAME:" "OPERATION NAME:" "MACHINE TIME:"]

set divider "-------------------------------------------------------------------------------"

puts $time_file $separator
puts $time_file $space
puts $time_file $title
puts $time_file $space
puts $time_file $time_written
puts $time_file $fname
puts $time_file $space
puts $time_file $separator
puts $time_file $space
puts $time_file $header
puts $time_file $divider


#------------------------------------------------------------------------------
#   Now cycle through the "machine_times" 1x3 array, and write out the time
#   for each operation.
#------------------------------------------------------------------------------
#
for {set i 1} {$i <= $num_ops} {incr i} {

    set output [format "%-32s%-32s%.2f %4s" $machine_times($i,1) $machine_times($i,2) $machine_times($i,3) "MIN."]

    puts $time_file $output
}


#------------------------------------------------------------------------------
#   Add the total machine time to the bottom of the machine time file
#------------------------------------------------------------------------------
#
set total_divider [format "%+77s" "============="]
set totals [format "%+64s%.2f %4s" "TOTAL TIME:  " $mom_machine_time "MIN."]

puts $time_file $total_divider
puts $time_file $totals

close $time_file    ; # Close the file now that we have finished writing to it.


#------------------------------------------------------------------------------
#   Open (as read-only) the newly created machine time file and output its
#   contents to the NX information window.
#------------------------------------------------------------------------------
#
set time_out [open $machine_time_filename r]

while {[gets $time_out curline] >= 0} {
    MOM_output_to_listing_device $curline
}
...
#=============================================================
proc PB_CMD_URC_Set_Cutcom_Register { } {
#=============================================================
#
# Arguments:    none
#
#
# Insert Point: Place this proc in the Motion section of the post, infront
#               (at the very top) of each motion event.
#
#
# Purpose:      Okay - here's the dealeo...
#
#               As long as you use the default, system defined tool radius
#               compensation (i.e. you define the cutcomp blocks in the
#               Machine Control section only), everything seems to work fine -
#               except that the G4x Dxx and G40 ALWAYS appear on a line on
#               their own.  I want the G41/42 Dxx & G40 to be on a linear motion
#               line.  So, I took these entries out of the Machine Control section
#               and added the G4x Dxx blocks directly into the linear motion blocks
#               and this is what I get as output:
#
#
#                   N1 G90
#                   N2 T75 M06
#                   N3 G00 G54 X1.1803 Y-.1892
#                   N4 G43 Z2. H75
#                   N5 Z.1
#                   N6 G01 Z0.0 F30. D75
#                   N7 G41 X1.1351 Y-.1
#                   N8 G03 X.9351 Y0.0 I-.2 J-.15
#
#
#               Note the location of the "D75" - it appears at the end of the
#               "G01 Z0.0..." line while the G41 is at the start of the next
#               line.
#
#               So by outputting the cutcom status I was able to ascertain that
#               at the very beginning of the program, it is undefined, up to
#               just before the 1st time a G41/42 occurs - so as long as the
#               cutcom status is UNDEFINED, I suppress the D register.  Now the
#               1st time there is a change, I force the D-register once and as a
#               result the D-value appears in the same line as the G41/42.  The
#               reason for forcing the D-register when there is a change is to
#               handle those cases where there are multiple operations, all using
#               the same tool and with cutcomp on.  Without being forced, the
#               output was correct for the 1st operation, but not for each
#               subsequent one.
#
#               Now, a problem arises when posting multiple operations with
#               tool changes in that the cutcom status is no longer UNDEFINED,
#               but actually turned "OFF" at the end of one operation, before
#               the next tool change.  As before, this state remains constant
#               until the next G41/42 - so, I also suppress the D-register if
#               the cutcom status == OFF.
#
#               By doing this, I now get the following output:
#
#                    N1 G90
#                    N2 T75 M06
#                    N3 G00 G54 X1.1803 Y-.1892
...
#                    N5 Z.1
#                    N6 G01 Z0.0 F30.
#                    N7 G41 X1.1351 Y-.1 D75
#                    N8 G03 X.9351 Y0.0 I-.2 J-.15
#
#               Note that the D75 and the G41 appear on the same line.
#
#               And, most importantly, the G40 now appears on a motion block
#               instead of on a line by itself...
#
#==============================================================================
    #
    ...
    #   Outputs the actual status for debugging purposes
    #MOM_output_literal "CUTCOM STATUS = $mom_cutcom_status"

    #------------------------------------------------------------------------------
    #   As explained above, if the status is undefined or turned off, suppress
    #   the D-register once (i.e. for the next line of output) and set the
    #   cutcomp_flag to 1.  If the status changes to either LEFT or RIGHT
    #   and the cutcomp_flag == 1, force the output of the D register once
    #   and then set the cutcomp_flag to 0.  This prevents the "D" being
    #   output again until there the cutcomp status has been turned off.
    #
    #   This is necessary in order to get the "D" value to appear when
    #   multiple operations with cutcomp use the same tool.
    #------------------------------------------------------------------------------
    #
    if {$mom_cutcom_status == "UNDEFINED" || $mom_cutcom_status == "OFF"} {
        MOM_suppress ONCE D
        set cutcomp_flag 1
    } elseif {($mom_cutcom_status == "LEFT" && $cutcomp_flag == 1) ||  ($mom_cutcom_status == "RIGHT" && $cutcomp_flag == 1)} {
        set cutcomp_flag 0
        MOM_force ONCE D
    }
}
...
#=============================================================
proc PB_CMD_URC_Set_Home_Position_EOP { } {
#=============================================================
#
# Arguments:    none
#
#
# Insert Point: Locate this procedure in End Of Program section of the
#               Program-End-Sequence, immediately before (above) the M05 
#               block.
#
# Purpose:      Outputs     G28 G91 Z0.
#                           G28 G91 X0. Y0.
#
#
#==============================================================================
    ...
    #----------------------------------------------------------------------
    #   Rather than just output the actual code using MOM_output_literal,
    #   in order to keep the G_mode and G_return's model I have chosen to
    #   execute two user-created block templates called "home_position_1"
    #   and "home_position_2" which are defined as follows:
    #
    #   home_position_1 =   G28 G91 Z0.0
    #   home_position_2 =   G28 G91 X0.0 Y0.0
    #
    #   Where   G28 = G_return - UDE=$mom_sys_return_code - digit_2 - modal
    #           G91 = G_mode - UDE=91 - digit_2 - modal
    #           X   = X - UDE=0 - coord - modal
    #           Y   = Y - UDE=0 - coord - modal
    #           Z   = Z - UDE=0 - coord - modal
    #
    #   "UDE" stands for "User Defined Expression"
    #
    #----------------------------------------------------------------------
    #
    MOM_force once G_mode G_return  ; # Have to do this to repeat the G28 & G91
    MOM_do_template home_position_1
    MOM_force once G_mode G_return  ; # Have to do this to repeat the G28 & G91
    MOM_do_template home_position_2
}


#=============================================================
proc PB_CMD_URC_Set_Home_Position_OSS { } {
#=============================================================
#
# Arguments:    none
#
#
# Insert Point: Locate this procedure in Auto Tool Change section of the
#               Operation-Start-Sequence, immediately before (above) the
#               tool select block (T).
#
# Purpose:      Outputs     G28 G91 Z0.
#                           G28 G91 X0. Y0.
...
#==============================================================================
    ...
    #--------------------------------------------------------------------------
    #   Only generate output for the 1st tool change when the HM_Flag == 1; 
    #   after which set the flag to FALSE (0).
    #--------------------------------------------------------------------------
    #
    if {$HM_Flag == 1} {
        ...
        #
        #
        #                     WORD    User Defined Expression  FORMAT    MODAL
        #           -----------------------------------------------------------
        #   Where   G28 =   G_return | $mom_sys_return_code  | digit_2  | YES
        #           G91 =   G_mode   | 91                    | digit_2  | YES
        #           X   =   X        | 0                     | coord    | YES
        #           Y   =   Y        | 0                     | coord    | YES
        #           Z   =   Z        | 0                     | coord    | YES
        #
        ...
        MOM_force once X Y Z    ; # Have to do this to force the next G01 line to
                                  # output an Y & Y value even if these == 0 
    }
}


#=============================================================
proc PB_CMD_URC_Set_Makino_Coolant_Option_OFF { } {
#=============================================================
#
# Arguments:    none
#
#
# Insert Point: NOTE:   THIS PROCEDURE MUST BE LOCATED IN TWO (2) SEPARATE
#                       LOCATIONS IN ORDER FOR THE HIGH SPEED MODE SWITCH
#                       TO WORK CORRECTLY!
#
#  LOCATION #1: Place a copy of this proc at the very top (i.e. 1st item)
#               of the Auto Tool Change section of the Operation-Start-
#               Sequence.
#
#  LOCATION #2: Place another copy of this proc at the very top of the 
#               End Of Program section, just before the M05, of the
#               Program-End-Sequence.
#
# Purpose:      If the COOLANT (CL) option has been set, turns it off
#               at the end of the operation, but only if there is going to
#               be a tool change.
#
#
#           COOLANT:            OFF             (default)
...
#==============================================================================
    ...
    #--------------------------------------------------------------------------
    #   If $CL == "FLOOD" or "MIST" or "TAP" or "THROUGH" and this is not the 
    #   1st tool change (i.e. CL_Flag == 0), then output the COOLANT "OFF" code.
    #   Basically, this code is not output at the 1st tool change which is where
    #   the CL_Flag gets reset to zero (0) - after which it is output just before
    #   every tool change.
    #
    #   Regardless of tool changes, it is output once again at the very end
    #   of the program as well.
    #--------------------------------------------------------------------------
    #
    if { ($CL_Flag == 0 && $CL == "FLOOD") || ($CL_Flag == 0 && $CL == "MIST") ||  ($CL_Flag == 0 && $CL == "TAP") || ($CL_Flag == 0 && $CL == "THROUGH") } {
...
#=============================================================
proc PB_CMD_URC_Set_Makino_Coolant_Option_ON { } {
#=============================================================
#
# Arguments:    none
#
#
# Insert Point: Locate this procedure in Auto Tool Change section of the
#               Operation-Start-Sequence, immediately below (after) the
#               spindle block (S M03).
#
# Purpose:      Outputs the specified coolant code after each tool change.
#
#
#           COOLANT:            OFF             (default)
#                               FLOOD           (select from pulldown)
...
#==============================================================================
    ...
    #--------------------------------------------------------------------------
    #   Only generate output if $CL == FLOOD,MIST,TAP, or THROUGH.
    #--------------------------------------------------------------------------
    #
    if { $CL == "FLOOD" } {
...
#=============================================================
proc PB_CMD_URC_Set_Makino_GRAPHITE_Option_OFF { } {
#=============================================================
#
# Arguments:    none
#
#
# Insert Point: Locate this procedure in End Of Program section of the
#               Program-End-Sequence, immediately after the M05 block and
#               just before (ahead of) the "PB_CMD_URC_Set_Makino_Start
#               _Position_EOP" proc.
#
# Purpose:      Outputs an additional line of G-code if the GRAPHITE (GR)
#               option is set to "ON" at the 1st tool change.
#
#
#               GRAPHITE OPTION:    ON      (default - use M413/M412)
#                                   OFF     (select from pulldown - no extra code)
...
#=============================================================
proc PB_CMD_URC_Set_Makino_GRAPHITE_Option_ON { } {
#=============================================================
#
# Arguments:    none
#
...
# Insert Point: Locate this procedure in Auto Tool Change section of the
#               Operation-Start-Sequence, immediately below (after) the
#               High Speed option proc "PB_CMD_URC_Set_Makino_HighSpeed_Option_ON".
#
# Purpose:      Outputs an additional line of G-code if the GRAPHITE (GR)
#               option is set to "ON" at the 1st tool change.
#
...
#==============================================================================
    ...
    #--------------------------------------------------------------------------
    #   Only generate output for the 1st tool change when the GR_Flag == 1 and
    #   if $GR = "ON"; after which set the flag to FALSE (0).
    #--------------------------------------------------------------------------
    #
    if {$GR_Flag == 1 && $GR == "ON"} {
...
#=============================================================
proc PB_CMD_URC_Set_Makino_Header { } {
#=============================================================
#
# Arguments:    none
#
#
# Insert Point: Locate this procedure in Start-Of-Program section, right after
#               the "PB_CMD_URC_Init_Makino_Option_Flags" proc.
#
#               NOTE: Don't forget to delete the MOM_set_seq_off block, the
#               rewind-stop block and the MOM_set_seq_on block that appear
#               there by default...)
#
# Purpose:      Outputs the Stackteck Makino Post header blocks
#
#==============================================================================
    ...
    #--------------------------------------------------------------------------
    #   Set the Time-Of-Day to read as <Abbr.-day of the week> (%a), <Abbr. Month> (%b)
    #   <Day> (%d)/<Full Year> (%Y)   <hours> (%I):<minutes> (%M):<seconds> (%S)
    #   <AM or PM> (%p)
    #--------------------------------------------------------------------------
    #
    ...
    #--------------------------------------------------------------------------
    #   Start outputting the program header.
    #--------------------------------------------------------------------------
    #
    MOM_set_seq_off

    MOM_output_literal "%"
    MOM_output_literal "O$mom_oper_program"

    MOM_set_seq_on

    MOM_output_literal "(Makino)"
    MOM_output_literal "(DATE: $tod)"

    #
    #   Only output the user comment (UC) if it contains something
    #
    if {$UC != "No User Comment"} {
        MOM_output_literal "($UC)"
    }
}


#=============================================================
proc PB_CMD_URC_Set_Makino_HighSpeed_Option_OFF { } {
#=============================================================
#
# Arguments:    none
#
...
#                       TO WORK CORRECTLY!
#
#  LOCATION #1: Place a copy of this proc at the top of the Auto Tool Change
#               section, just after (below)the "PB_CMD_URC_Set_Makino_Coolant
#               _Option_OFF" proc of the Operation-Start-Sequence.
#
#  LOCATION #2: Place another copy of this proc in the End Of Program section,
#               just after the "PB_CMD_URC_Set_Makino_COOLANT_Option_OFF" and
#               before the "PB_CMD_URC_Set_Home_Position_EOP" of the
#               Program-End-Sequence.
#
# Purpose:      If the High Speed (HS) option is set to "ON", turns it off
#               at the end of the operation, but only if there is going to
#               be a tool change.
#
#
...
#==============================================================================
    ...
    #--------------------------------------------------------------------------
    #   If $HS == "ON" and this is not the 1st tool change (i.e. HS_Flag == 0)
    #   then output the High Speed "OFF" code and set the HS_Flag to 1 - this
    #   tells the "PB_CMD_URC_Set_Makino_HighSpeed_Option_ON" proc that a tool
    #   change has occurred and that (if HS = "ON") the high-speed callout
    #   needs to be output.
    #
    #   Regardless of tool changes, it is output once again at the very end
    #   of the program as well.
...
#=============================================================
proc PB_CMD_URC_Set_Makino_HighSpeed_Option_ON { } {
#=============================================================
#
# Arguments:    none
#
#
# Insert Point: Locate this procedure at the very bottom (i.e. last item) of
#               Rapid Move block of the Tool-Path, MOTION sequence.  This proc
#               works in conjunction with "PB_CMD_URC_Set_Makino_HighSpeed
#               _Option_OFF".
#
# Purpose:      Outputs an additional line of G-code if the High Speed (HS)
#               option is set to "ON".
#
#
#               HIGH SPEED OPTION:  ON  (default - use G05 P10000)
...
#==============================================================================
    ...
    #--------------------------------------------------------------------------
    #   Only generate output if $HS = "ON" and the HS_Flag has been set to 1.
    #--------------------------------------------------------------------------
    #
    if {$HS_Flag == 1 && $HS == "ON"} {
        MOM_output_literal "M250"
