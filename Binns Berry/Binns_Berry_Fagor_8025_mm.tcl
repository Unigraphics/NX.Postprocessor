...
#
#  Created by zhang  @  Tue Mar 31 18:24:13 2009 China Standard Time
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_spindle_param(1,max)              "0"  
  set mom_sys_spindle_param(1,min)              "0"  
...
#=============================================================
   ...
   PB_CMD_tool_change_code
   MOM_force Once T_change
   MOM_do_template auto_tool_change
   MOM_force Once G S
   MOM_do_template max_spindle_speed
   MOM_do_template spindle_speed_range
}
...
#=============================================================
  global spindle_is_out  
   ...
   MOM_force once X Z M_spindle M_coolant F G_feed F T_change
...
MOM_output_literal "-----------------------"
MOM_output_literal "$mom_feed_engage_value"
MOM_output_literal "-----------------------"
}
...
#=============================================================
#--------------------------------------------------
#Add to calc the retract X and Z values.
#--------------------------------------------------
global pop_X pop_Z mom_pos mom_sys_lathe_x_factor mom_sys_lathe_x_double
global mom_sys_lathe_z_factor

set pop_X [expr $mom_pos(0)*$mom_sys_lathe_x_factor*$mom_sys_lathe_x_double]
set pop_Z [expr $mom_pos(2)*$mom_sys_lathe_z_factor]

global pop_temp

if {$pop_X > 0 || $pop_X == 0} {

	set pop_temp [string range [expr int($pop_X)] [expr [string length [expr int($pop_X)]] - 2] end]
	if {$pop_temp < 50} {
		set pop_X [expr [expr int($pop_X)] - $pop_temp + 100]
	}
	if {$pop_temp > 50 || $pop_temp  == 50} {
		set pop_X [expr [expr int($pop_X)] - $pop_temp + 150]
	}
}

if {$pop_Z > 0 ||$pop_Z == 0} {
	set pop_temp [string range [expr int($pop_Z)] [expr [string length [expr int($pop_Z)]] - 2] end]
	if {$pop_temp <50} {
		set pop_Z [expr [expr int($pop_Z)] - $pop_temp + 50]
	}
	if {$pop_temp > 50 || $pop_temp  == 50} {
		set pop_Z [expr [expr int($pop_Z)] - $pop_temp + 100]
	}
}
...
set pop_temp [string range [expr int($pop_X)] [expr [string length [expr int($pop_X)]] - 2] end]
	if {$pop_temp < 50} {
		set pop_X [expr [expr int($pop_X)] + $pop_temp ]
	}
	if {$pop_temp > 50 || $pop_temp  == 50} {
		set pop_X [expr [expr int($pop_X)] + $pop_temp + 50]
	}
...
MOM_output_literal "G00 X$pop_X Z$pop_Z M00 M05 M9"


#Cancel the mode minus
global mom_tool_post_select

if {[info exists mom_tool_post_select]} {
unset mom_tool_post_select
}
}
...
#=============================================================
proc PB_CMD_offset_select { } {
#=============================================================
#--------------------------------------------------------
#Coordinate selection
#<3/20/2009-Qing>
#mom_fixture_offset_value [0,6]-> G53~G59
#if mom_fixture_offset_value < 0 or >6
#Reset the mom_fixture_offset_value
#--------------------------------------------------------
...
set mom_fixture_offset_value 0
} elseif {$mom_fixture_offset_value > 6} {
...
#=============================================================
  ...
  if { $mom_spindle_mode == "SFM" || $mom_spindle_mode == "SMM" } {
    MOM_force once G_spin M_spindle S M_coolant
...
#=============================================================
proc PB_CMD_tool_change_code { } {
#=============================================================
global mom_tool_number mom_tool_adjust_register
global pop_tool_change_code

set pop_tool_change_code "$mom_tool_number.$mom_tool_adjust_register"
#MOM_output_literal "$pop_tool_change_code"
}


#=============================================================
proc PB_CMD_tool_post_select { } {
#=============================================================
global mom_tool_post_select
...
if {$mom_tool_post_select == "ACTIVE"} {
