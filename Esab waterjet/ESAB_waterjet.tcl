########################## TCL Event Handlers ##########################
#
#  Created by paffett  @  Mon Mar 14 14:42:34 2005 Eastern Daylight Time
...
#     source ${cam_debug_dir}mom_review.tcl
   ...
   ####  Listing File variables 
     ...
     set mom_sys_commentary_list                   "x y feed speed"
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_spindle_direction_code(CLW)       "58" 
  set mom_sys_spindle_direction_code(CCLW)      "58" 
  set mom_sys_spindle_direction_code(OFF)       "59" 
...
# generates start-of-path announcement messsage
MOM_output_literal "(OPER: $mom_path_name)"

# force some modal registers to be output for each path
...
#=============================================================
global mom_pos  mom_prev_pos  mom_motion_type
global ESAB_knife_state

# set switches based on axes moving
set deltax [expr $mom_pos(0)-$mom_prev_pos(0)]
set deltay [expr $mom_pos(1)-$mom_prev_pos(1)]

# if X or Y don't move, don't output a standalone G1 or G0
if { $deltax==0.0 && $deltay==0.0 } {
  MOM_suppress always G_motion F
  } else {
  MOM_suppress off G_motion F
  }

# if this is the engage move, turn on the water
switch $mom_motion_type {
  ENGAGE {
    if { $ESAB_knife_state == 0 } {
      MOM_do_template knife_on
      MOM_force_block once dwell
      MOM_do_template dwell
      set ESAB_knife_state -1
      }
    }
  RETURN {
    if { $ESAB_knife_state == -1 } {
      MOM_do_template knife_off
      MOM_do_template cancel_cutcom
      set ESAB_knife_state 0
      }
    }
  }
}
...
# initialize known knife state
set ESAB_knife_state 0

# no sequence numbers this post - ever
MOM_set_seq_off
MOM_suppress always N

# in case the user forgets to propgram a dwell - use 3 seconds
set mom_delay_value 3.0

# output program header blocks
