########################## TCL Event Handlers ##########################
#
#  Created by tommymartinez  @  Mon Jun 18 09:35:33 2007 Central Daylight Time
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_coolant_code(THRU)                "50" 
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_4th_axis_max_limit                "359.999"
  set mom_kin_4th_axis_min_incr                 "0.001"
  set mom_kin_4th_axis_min_limit                "-359.999"
  ...
  set mom_kin_max_arc_radius                    "120.0"
...
#=============================================================
  ...
  global mom_rotary_direction_5th
    ...
    if { $mom_rotary_direction_4th < 0 } {
      set mom_out_angle_pos(0) [expr abs($mom_out_angle_pos(0))]
      set mom_sys_leader(fourth_axis) [string trimright $mom_sys_leader(fourth_axis) "-"]
      if [EQ_is_zero $mom_out_angle_pos(0)] {
        append mom_sys_leader(fourth_axis) "-"
      } else {
...
#=============================================================
proc PB_CMD_UDE_w_retract_clear { } {
#=============================================================

uplevel #0 {

  proc MOM_w_retract_change {} {

  # Default is 1651 MM from Start of Program

    global mom_w_axis_retract
    global w_axis_clear

    set w_axis_clear $mom_w_axis_retract
  } 
} ; # uplevel
}
...
#=============================================================
proc PB_CMD_Z_W_ude_output_delay_detect { } {
#=============================================================

  # global mom_axis_position_now

  global zw_axis_set_used
  global saved_zw_axis_move


  if {[info exists zw_axis_set_used]} {
      if { $zw_axis_set_used == "FALSE" } {
          MOM_output_literal "$saved_zw_axis_move"
      }
  }

  set zw_axis_set_used "TRUE"

# The UDE must be used again for the above delayed CREATE template to output
}
...
#=============================================================
proc PB_CMD_date_and_time_stamp { } {
#=============================================================
...
#=============================================================
proc PB_CMD_end_path_retract_system { } {
#=============================================================
...
if { $mom_next_oper_has_tool_change == "YES" } {
    ...
    MOM_do_template coolant_off
    MOM_do_template spindle_off
    MOM_do_template auto_tool_change_z0 ;	# G98 Z0 D0
    MOM_do_template w_axis_retract_move_2 ;	# W1750 w-axis-tlc-position
    MOM_do_template opstop
}
}
...
#=============================================================
proc PB_CMD_first_move_outputs { } {
#=============================================================
# <rws 10-11-YR2006>  original
# <rws 11-16-YR2006>  Carry over set_csys fourth B-angle to mom_out_angle_pos

global mom_out_angle_pos
global saved_csys_fourth_axis_answer

# MOM_output_literal "saved_csys_fourth_axis_answer= [format %3.2f $saved_csys_fourth_axis_answer]"

#MOM_force once G_mode G_motion W_custom
#MOM_do_template w_axis_clear_move_1

MOM_force once X Y
#MOM_do_template initial_move_X_Y
...
#PB_CMD_Z_W_ude_output_delay_detect

#MOM_force once R P_cycle_clear
}
...
#=============================================================
proc PB_CMD_gohome_fixed_no_params { } {
#=============================================================
# Use a pre-set W-table value for a user commanded non-graphic GOHOME
...
if { $mom_motion_type == "GOHOME_DEFAULT" } {
  MOM_output_literal "GOHOME DEFAULT RECORDS HERE"

  MOM_do_template auto_tool_change_z0
  MOM_do_template w_axis_retract_move_2
}
}


#=============================================================
proc PB_CMD_gohome_no_params_fixed { } {
#=============================================================
#
#  This custom command allows you to modify the feed rate number
#  after it has been calculated by the system
...
#  ZW Parameters
#
#<09-11-06 sws> switched Z/W

#<10-11-06 rws> Z is quill  W is Table
...
set mom_sys_zw_type		"1"
#  =1		Z and W independent
#  =2		Z dependent on W  (Z extending quill)
#  =3 	W dependent on Z	(W extending quill)
#  =4		Z dependent on W  (Z extending quill controller handles offset)
#  =5		W dependent on Z  (W extending quill controller handles offset)
...
#=============================================================
proc MOM_set_axis_new { } {
#=============================================================

# <rws 10-11-2006>  Spitz
## RWS -- Experimental for GL MC60 8000B
## Need to Delay Output of Z or W output
## A CUSTOM_DIR UDE is used to use this event handler
...
# <rws 10-11-2006>
   ...
   global mom_axis_position_now ; # TRUE FALSE
   global zw_axis_set_used	; # TRUE FALSE
   global saved_zw_axis_move    ; # do template CREATE string for later output
...
#      MOM_output_literal "mom_motion_event= $mom_motion_event  UDE MOM_set_axis"
#  } else {
#      MOM_output_literal "NO MOTION EVENT"
#  }


 #  MOM_output_literal "UDE ENTRY mom_sys_table_leader= $mom_sys_table_leader"
 #  MOM_output_literal "UDE ENTRY mom_sys_quill_leader= $mom_sys_quill_leader"
 #  MOM_output_literal "UDE ENTRY mom_sys_zw_mode=      $mom_sys_zw_mode"

   switch $mom_axis_position {
      ZAXIS { 
         if {$mom_sys_leader(Z) == "W"} {
            set need_w_reset 0
            set mom_sys_leader(Z) Z
         }

         # MOM_output_literal "UDE mom_axis_position= $mom_axis_position"
         # MOM_output_literal "ZAXIS SWITCH set_axis UDE= mom_sys_leader= $mom_sys_leader(Z)"

         set mom_sys_zaxis_pos $mom_axis_position_value
         MOM_force once Z

         if { $mom_axis_position_now == "TRUE"} {
             MOM_do_template ude_zaxis_move
             set zw_axis_set_used "TRUE"
         } else {
             set saved_zw_axis_move [MOM_do_template ude_zaxis_move CREATE]
             set zw_axis_set_used "FALSE"
         }
      ...
      WAXIS  { 
         if {$mom_sys_leader(Z) == "Z"} {
            set need_w_reset 0
            set mom_sys_leader(Z) W
         }

  # MOM_output_literal "UDE mom_axis_position= $mom_axis_position"
  # MOM_output_literal "WAXIS SWITCH set_axis UDE= mom_sys_leader= $mom_sys_leader(Z)"
...
#=============================================================
proc MOM_set_axis_ORIGINAL_09_05 { } {
#=============================================================
...
#=============================================================
proc PB_CMD_init_variables_GL_MC60 { } {
#=============================================================
##  NOTE:  Following Installed Numerical Values are in METRIC MM Units

global zw_axis_set_used
set    zw_axis_set_used "FALSE"

global saved_zw_axis_move
set    saved_zw_axis_move "NULL"

global w_axis_clear
set    w_axis_clear 1651.0

global w_axis_tlc_limit
set    w_axis_tlc_limit 1750.0
}


#=============================================================
proc PB_CMD_initial_move_outputs { } {
#=============================================================
# <rws 10-11-YR2006>  original
# <rws 11-16-YR2006>  Carry over set_csys fourth B-angle to mom_out_angle_pos
...
#=============================================================
proc PB_CMD_mom_pos_report_only { } {
#=============================================================
global mom_pos

set zwpos [format %4.3f $mom_pos(2)]
MOM_output_literal "(ORIGINAL NON-COLINEAR mom_pos Z-W Position= $zwpos)"
}
...
set txt1 ".TXT , "
MOM_output_literal "(PGM,$mom_attr_PROGRAMVIEW_PROGRAM_NUMBER$txt1 $mom_group_name)"
}
...
# <rws 11-16-YR2006> De-bug
  if { [info exists mom_pos]} {
      # MOM_output_literal "mom_pos(3) FOURTH AXIS= $mom_pos(3)"
  }

# <rws 11-20-YR2006> Use mom_init_pos instead of mom_pos  
  if { [info exists mom_init_pos]} {
      # MOM_output_literal "mom_init_pos(3) INIT FOURTH AXIS= $mom_init_pos(3)"
      set mom_pos(3) $mom_init_pos(3)
  }

# <rws 11-16-YR2006>  Comment out-- no 5th Axis
...
# <rws 11-16-YR2006> PB V3.5.1  NX 4.03.3
  global saved_csys_fourth_axis_answer
  set    saved_csys_fourth_axis_answer $mom_out_angle_pos(0)
 ...
 # <rws 11-16-YR2006> Do not output answer here - too early
 # PB_CMD_fourth_axis_rotate_move
 # PB_CMD_fifth_axis_rotate_move

 # <rws 11-20-YR2006>  NO G68 Needed-  Get Out
 return
...
#=============================================================
#
 MOM_force once G G_mode G_motion G_feed S M_spindle X Y Z fourth_axis F H
}


#=============================================================
proc PB_CMD_tlc_Z_W_position_A { } {
#=============================================================

MOM_do_template absolute_metric ;	# G90 G71

MOM_do_template auto_tool_change_z0 ;	# G98 Z0 D0

MOM_do_template w_axis_retract_tlc_limit ;	# W1750 w-axis-tlc-position
}


#=============================================================
proc PB_CMD_tlc_Z_W_position_B { } {
#=============================================================
...
#=============================================================
proc PB_CMD_tlc_Z_W_position_end_program { } {
#=============================================================
...
# The following should be a UDE option

MOM_output_text "  " ; # Skip a Line
MOM_output_literal "M15 ! PALLET CHANGE"
MOM_output_literal "G90 G71"
MOM_output_literal "G00 G98 Z0. D0"
MOM_output_literal "W1750."
MOM_output_literal "B0."
}
...
#=============================================================
  MOM_force once G_adjust H X Y Z S fourth_axis
}
...
#=============================================================
   ...
   if { $mom_motion_event == "initial_move" || $mom_pos(3) != $mom_prev_pos(3)} {
       PB_CMD_mom_pos_report_only
   }
...
#<09-11-06 sws>
   ...
   if {$mom_sys_zw_mode == "$mom_sys_quill_leader"} {
      set mom_pos(2) [expr +1 * ($mom_sys_z_pos - $mom_sys_table_pos)]
      if [info exists mom_cycle_feed_to_pos] {
         set mom_cycle_feed_to_pos(2) [expr +1 * ($mom_sys_cycle_z_pos + $mom_cycle_feed_to - $mom_sys_table_pos)]
      }
      if [info exists mom_cycle_rapid_to_pos] {
         set mom_cycle_rapid_to_pos(2) [expr +1 * ($mom_sys_cycle_z_pos + $mom_cycle_rapid_to - $mom_sys_table_pos)]
      }
   }
}
...
global mom_kin_4th_axis_ang_offset
  ...
  MOM_output_to_listing_device "* MSYS *"
  for { set j 0 } { $j < 9 } { incr j } {
    MOM_output_to_listing_device "mom_msys_matrix($j)                \t= $mom_msys_matrix($j)"
  }
  for { set j 0 } { $j < 3 } { incr j } {
    MOM_output_to_listing_device "mom_msys_origin($j)                \t= $mom_msys_origin($j)"
  }

  global mom_machine_csys_matrix mom_csys_matrix
  MOM_output_to_listing_device "* Machine CSYS *"
  for { set j 0 } { $j < 12} { incr j } {
    MOM_output_to_listing_device "mom_machine_csys_matrix($j)        \t= $mom_machine_csys_matrix($j)"
  }
  MOM_output_to_listing_device "* CSYS *"
  for { set j 0 } { $j < 12} { incr j } {
    MOM_output_to_listing_device "mom_csys_matrix($j)                \t= $mom_csys_matrix($j)"
  }

  MOM_output_to_listing_device "mom_kin_4th_axis_ang_offset        \t= $mom_kin_4th_axis_ang_offset"
  MOM_output_to_listing_device "mom_kin_4th_axis_center_offset(0)  \t= $mom_kin_4th_axis_center_offset(0)"
  MOM_output_to_listing_device "mom_kin_4th_axis_center_offset(1)  \t= $mom_kin_4th_axis_center_offset(1)"
  MOM_output_to_listing_device "mom_kin_4th_axis_center_offset(2)  \t= $mom_kin_4th_axis_center_offset(2)"
  MOM_output_to_listing_device "mom_kin_4th_axis_direction         \t= $mom_kin_4th_axis_direction"
  MOM_output_to_listing_device "mom_kin_4th_axis_plane             \t= $mom_kin_4th_axis_plane" 
  MOM_output_to_listing_device "mom_kin_4th_axis_point(0)          \t= $mom_kin_4th_axis_point(0)"
  MOM_output_to_listing_device "mom_kin_4th_axis_point(1)          \t= $mom_kin_4th_axis_point(1)"
  MOM_output_to_listing_device "mom_kin_4th_axis_point(2)          \t= $mom_kin_4th_axis_point(2)"
  MOM_output_to_listing_device "mom_kin_4th_axis_rotation          \t= $mom_kin_4th_axis_rotation"
  MOM_output_to_listing_device "mom_kin_4th_axis_type              \t= $mom_kin_4th_axis_type"
  MOM_output_to_listing_device "mom_kin_4th_axis_vector(0)         \t= $mom_kin_4th_axis_vector(0)"  
  MOM_output_to_listing_device "mom_kin_4th_axis_vector(1)         \t= $mom_kin_4th_axis_vector(1)"  
  MOM_output_to_listing_device "mom_kin_4th_axis_vector(2)         \t= $mom_kin_4th_axis_vector(2)"  
  MOM_output_to_listing_device "mom_kin_4th_axis_zero              \t= $mom_kin_4th_axis_zero"
  MOM_output_to_listing_device "mom_kin_5th_axis_center_offset(0)  \t= $mom_kin_5th_axis_center_offset(0)"
  MOM_output_to_listing_device "mom_kin_5th_axis_center_offset(1)  \t= $mom_kin_5th_axis_center_offset(1)"
  MOM_output_to_listing_device "mom_kin_5th_axis_center_offset(2)  \t= $mom_kin_5th_axis_center_offset(2)"
  MOM_output_to_listing_device "mom_kin_5th_axis_point(0)          \t= $mom_kin_5th_axis_point(0)"
  MOM_output_to_listing_device "mom_kin_5th_axis_point(1)          \t= $mom_kin_5th_axis_point(1)"
  MOM_output_to_listing_device "mom_kin_5th_axis_point(2)          \t= $mom_kin_5th_axis_point(2)"
  MOM_output_to_listing_device "mom_kin_5th_axis_vector(0)         \t= $mom_kin_5th_axis_vector(0)"
  MOM_output_to_listing_device "mom_kin_5th_axis_vector(1)         \t= $mom_kin_5th_axis_vector(1)"
  MOM_output_to_listing_device "mom_kin_5th_axis_vector(2)         \t= $mom_kin_5th_axis_vector(2)"
  MOM_output_to_listing_device "mom_kin_machine_zero_offset(0)     \t= $mom_kin_machine_zero_offset(0)"
  MOM_output_to_listing_device "mom_kin_machine_zero_offset(1)     \t= $mom_kin_machine_zero_offset(1)"
  MOM_output_to_listing_device "mom_kin_machine_zero_offset(2)     \t= $mom_kin_machine_zero_offset(2)"
}
