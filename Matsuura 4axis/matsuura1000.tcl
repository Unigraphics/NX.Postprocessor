...
#  matsuura1000.tcl -
#
#  Created by wrightj  @  Thu Dec 18 13:19:14 2008 Central Daylight Time
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_rapid_feed_rate                   "472"
...
#=============================================================
   ...
   MOM_do_template end_of_program_8
   MOM_do_template end_of_program_9
   MOM_do_template end_of_program_10
   MOM_do_template end_of_program_11
   MOM_do_template end_of_program_12
   MOM_do_template end_of_program_13
   MOM_do_template end_of_program_14
   MOM_do_template end_of_program_15
   MOM_do_template end_of_program_16
   MOM_do_template end_of_program_17
   MOM_force Once M
...
#=============================================================
  ...
  global first_linear_move ; set first_linear_move 0
   ...
   PB_CMD_sequence_number_calls
   PB_CMD_force_A_address
   MOM_set_seq_off
   PB_CMD_g45_offset_calls
}
...
#=============================================================
proc PB_CMD_ask_dialog { } {
#=============================================================
  set my_wish "[MOM_ask_env_var UGII_CAM_AUXILIARY_DIR]ugwish"
  set cam_post_dir [MOM_ask_env_var UGII_CAM_POST_DIR]
  
  set var [exec $my_wish $cam_post_dir\\ask_dialog.tcl]

  set prog_name [lindex $var 0]
  set oper_nbr [lindex $var 1]
  set part_name [lindex $var 2]
  set part_nbr [lindex $var 3]

  MOM_set_seq_off
  MOM_output_literal " Prog Name = $prog_name"
  MOM_output_literal " Operation Number = $oper_nbr"
...
# The "clamp" and "unclamp" templates must be defined in the Post Builder.
 ...
 if { [info exists mom_rotary_clamp] } { 
     if { $mom_rotary_clamp == "ACTIVE" } {
          ...
          MOM_do_template clamp } else { MOM_do_template unclamp }
     }
 }
}
...
#
global mom_part_name mom_sys_control_in mom_sys_control_out mom_logname
...
MOM_set_seq_off
set file [open $infile "APPEND RDWR" ]
puts $file "   $mom_logname     [clock format [clock seconds] -format "%H:%M %m/%d/%Y"] "
close $file
...
#=============================================================
proc PB_CMD_cutcom_cancel { } {
#=============================================================
#
# This custom command will cancel cutter compensation in  format.
#  - 
# 11/18/2008
#
uplevel #0 {
...
#=============================================================
#
# This custom command will turn on cutter compensation in  format.
#  - 
# 11/18/2008
#
global mom_cutcom_adjust_register mom_command_status

	if {[info exists mom_cutcom_adjust_register] && $mom_command_status == "ACTIVE" } {
            MOM_do_template cutcom_off
...
#=============================================================
proc PB_CMD_cutter_comp_on { } {
#=============================================================
#
# This custom command will output cutter comp in the  format
#  - 
# 11/17/2008
#
uplevel #0 {
	proc MOM_cutcom {} {
		global mom_x_offset mom_x_offset_defined
		global mom_y_offset mom_y_offset_defined
		global mom_z_offset mom_z_offset_defined
                global mom_pos

                if {[expr $mom_x_offset_defined + $mom_y_offset_defined + $mom_z_offset_defined == 0]} {
                MOM_output_literal ""

                } else {

                MOM_output_literal "G90"
                MOM_output_literal "G0X[format %0.4f $mom_pos(0)]Y[format %0.4f $mom_pos(1)]Z[format %0.4f $mom_pos(2)]"
                MOM_output_literal "G91"
                       }
    		if {$mom_x_offset_defined == 1} {
    		MOM_output_literal "G45X0.D$mom_x_offset"
						}

		if {$mom_y_offset_defined == 1} {
     		MOM_output_literal "G45Y0.D$mom_y_offset"
						}

		if {$mom_z_offset_defined == 1} {
    		MOM_output_literal "G45Z0.D$mom_z_offset"
						}
                ...
                MOM_output_literal "G92X[format %0.4f $mom_pos(0)]Y[format %0.4f $mom_pos(1)]Z[format %0.4f $mom_pos(2)]"
                MOM_output_literal "G90"
                       } 
...
#=============================================================
proc PB_CMD_cutter_compensation { } {
#=============================================================
#
...
#
  ...
  #MOM_force once 
  MOM_do_template MOM_cutcomp_on
}
...
#=============================================================
proc PB_CMD_g45_offset_calls { } {
#=============================================================
#
# This custom command will output G45 work offset shifts.
#  - 
# 11/18/2008
#
uplevel #0 {
	proc MOM_g45_shift_offsets {} {
		global mom_x_offset mom_x_offset_defined
		global mom_y_offset mom_y_offset_defined
		global mom_z_offset mom_z_offset_defined
                ...
                MOM_do_template absolute_mode_2
                MOM_do_template g45_shift_call
                MOM_do_template incremental_mode
...
#=============================================================
proc PB_CMD_g45_old_do_not_use { } {
#=============================================================
#
# This custom command will output G45 work offset shifts.
#  - 
...
#=============================================================
proc PB_CMD_g46_offset_cancel { } {
#=============================================================
#
# This custom command will output G46 work offset cancels.
#  - 
# 04/28/2005
#
uplevel #0 {
	proc MOM_g46_offset_cancel {} {
		global mom_cancel_offsets
                global mom_x_offset_defined mom_y_offset_defined mom_z_offset_defined
                global mom_x_offset mom_y_offset mom_z_offset
                global mom_command_status

		 if {$mom_cancel_offsets == "ON"} {
                    MOM_do_template absolute_mode_2
                    MOM_do_template g45_shift_call
                    MOM_do_template incremental_mode    
		...
		if {$mom_cancel_offsets == "ON" && $mom_x_offset_defined == 1} {
    		    MOM_output_literal "G46X0.D$mom_x_offset"

                 } else { 

                        MOM_output_literal ""
                        } 
		if {$mom_cancel_offsets == "ON" && $mom_y_offset_defined == 1} {
    		    MOM_output_literal "G46Y0.D$mom_y_offset"

                 } else { 

                        MOM_output_literal ""
                        } 
		if {$mom_cancel_offsets == "ON" && $mom_z_offset_defined == 1} {
    		    MOM_output_literal "G46Z0.D$mom_z_offset"
...
#=============================================================
#
#  This custom command allows you to type information into program header
#
uplevel #0 {
proc MOM_header_info {} {
global mom_output_file_basename
global mom_revision
global mom_programmed_date
global mom_change_level
global mom_programmer
global mom_date

MOM_output_literal [format "(PROGRAM       :  %s.%s)" [string toupper $mom_output_file_basename] $mom_revision]
MOM_output_literal "(CHANGE DATE   :  $mom_programmed_date)"
MOM_output_literal "(CHANGE LEVEL  :  $mom_change_level)"
MOM_output_literal "(PROGRAMMER    :  $mom_programmer)"
MOM_output_literal "(POSTING DATE  :  [string range [string toupper $mom_date] 0 3], [string range [string toupper $mom_date] 4 10][string range $mom_date 20 23] [string range $mom_date 11 18])"
MOM_output_literal "(POSTPROCESSOR :  [string toupper matuura1000_fanuc6m_4ax])"
MOM_output_literal "G17G80G40G20"
MOM_output_literal "G0G98"
MOM_output_literal "G49H00"
MOM_output_literal "G20G91"
MOM_output_literal "/G28Z0."
MOM_output_literal "/G45X0.D98"
MOM_output_literal "/G45Y0.D99"
MOM_output_literal "/G92X0.Y0.Z0."
MOM_output_literal "/G28Y0."
...
#=============================================================
proc PB_CMD_sequence_number_calls { } {
#=============================================================
#
#  This procedure is used when selecting sequence numbers manually 
#
#
global mom_sequence_number mom_sequence_mode mom_sequence_text mom_command_status
...
uplevel #0 {
	...
	if {$mom_command_status == "ACTIVE"} {
    	    MOM_output_literal "$mom_sequence_mode$mom_sequence_number"
  				             }
	   		            }
	  }
}
...
#=============================================================
proc PB_CMD_start_feedrate { } {
#=============================================================
#
# This custom command will force feedrate @ start of sequences
#  - 
# 11/18/2008
#
uplevel #0 {
                MOM_do_template feedrate_force
...
#=============================================================
  MOM_force once M_spindle X Y Z R fourth_axis fifth_axis
}


#=============================================================
proc PB_CMD_start_of_operation_force_addresses_1 { } {
#=============================================================
  MOM_force once M_spindle X Y Z R fourth_axis fifth_axis
}
...
#=============================================================
proc PB_CMD_start_of_operation_force_xyz { } {
#=============================================================
  MOM_force once M_spindle X Y Z
}
