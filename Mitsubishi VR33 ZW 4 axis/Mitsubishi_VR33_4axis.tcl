########################## TCL Event Handlers ##########################
#
#  Created by lili  @  Mon Sep 18 16:00:22 2006 China Standard Time
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_post_description                  "This is a 4-Axis Milling Machine (XYZW) With Right angle head.\n\
                                                  "
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   COOLANT_SET ; CUTCOM_SET ; SPINDLE_SET ; RAPID_SET
   PB_CMD_init_move_force_address
   PB_CMD_head_rotation
...
#=============================================================
  ...
  global first_linear_move ; set first_linear_move 0
   ...
   MOM_output_literal "(OPERATION NAME: $mom_path_name)"
   global mom_tool_number
   MOM_output_literal "(TOOL NUMBER: $mom_tool_number)"
   PB_CMD_init_head_mode
}
...
#=============================================================
   ...
   MOM_set_seq_on
   MOM_force Once G_cutcom G_plane G_adjust G_motion
...
#=============================================================
   ...
   if {[info exists mom_next_oper_has_tool_change]} {
       if { $mom_next_oper_has_tool_change == "YES" } {
           set has_tool_change 1 

           MOM_do_template end_of_path 

       } else {
           set has_tool_change 0
       }
   }
}
...
#=============================================================
   global head_number pre_head_number
   global mom_sys_zw_mode mom_sys_quill_leader

   PB_CMD_set_quill_home
   PB_CMD_set_table_home

   if { $mom_sys_zw_mode == $mom_sys_quill_leader } {
       MOM_do_template clamp_zaxis_off
   }
   MOM_force once G
   MOM_do_template return_home
   #MOM_output_literal "G91 G28 Z0. W0."

   if {$head_number == 1100} {
      set head_number 1000
      set pre_head_number 1100
      MOM_do_template head_change
   }
}
...
#=============================================================
#
#  This custom command used to output head change code for Mitsubishi VR33 with right angle head.
#  <2006-09-13> Li Li

   global pre_head_number
   global head_number
   global mom_next_tool_status
   global is_first_tool
...
#=============================================================
proc PB_CMD_head_rotation { } {
#=============================================================
#
# This command handles head rotation
# <2006-09-13> Li Li
   ...
   if { $head_number == 1100 && $mom_pos(3) != $mom_prev_pos(3)} {

       if { $has_tool_change == 0 } {
		   
	   if { $pre_head_number == 1100 } {
               MOM_do_template cancel_five_face_machining
           }
           ...
           if { $mom_sys_zw_mode == $mom_sys_quill_leader } {
               ...
               MOM_do_template return_home
               #MOM_output_literal "G91 G28 Z0. W0."
               MOM_do_template zaxis_clamping
	   } else {
	       PB_CMD_set_quill_home
               PB_CMD_set_table_home
               MOM_force once G
       ...
       MOM_do_template head_rotation
   }
}


#=============================================================
proc PB_CMD_init_head_mode { } {
#=============================================================
   ...
   global pre_tool_axis
   global is_first_tool
   global mom_sys_zw_mode mom_sys_quill_leader

   set pre_head_number $head_number

   if { $mom_head_spindle_axis(2) == 1 } {
       set head_number 1000
   } else {
       set head_number 1100
   }
      
   if { $head_number != $pre_head_number } {
...
#=============================================================
proc PB_CMD_init_move_force_address { } {
#=============================================================
...
# modify for mitsubishi vr33
    set mom_sys_quill_pos $mom_sys_zaxis_home
#   MOM_force once Z
#   MOM_do_template zaxis_move
...
# modify for mitsubishi vr33
    set mom_sys_table_pos $mom_sys_waxis_home
#   MOM_force once Z
#   MOM_do_template zaxis_move
