...
#  Mikron_WF21D_mm.tcl -
#
#  Created by suyo  @  Mon Aug 27 14:51:59 2007 China Standard Time
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_cycle_drill_code                  "0"  
  set mom_sys_cycle_drill_dwell_code            "0"  
  set mom_sys_cycle_drill_deep_code             "0"  
  set mom_sys_cycle_drill_break_chip_code       "0"  
  set mom_sys_cycle_tap_code                    "0"  
  set mom_sys_cycle_bore_code                   "0"  
  set mom_sys_cycle_bore_drag_code              "0"  
  set mom_sys_cycle_bore_no_drag_code           "0"  
  set mom_sys_cycle_bore_dwell_code             "0"  
  set mom_sys_cycle_bore_manual_code            "0"  
  set mom_sys_cycle_bore_back_code              "0"  
  set mom_sys_cycle_bore_manual_dwell_code      "0"  
  ...
  set mom_sys_coolant_code(ON)                  "13" 
...
#=============================================================
  ...
  global mom_motion_event
   ...
   set rapid_spindle_blk {G_cutcom G_motion X Y Z}
   set rapid_spindle_x_blk {G_cutcom G_motion X}
   set rapid_spindle_y_blk {G_cutcom G_motion Y}
   set rapid_spindle_z_blk {G_cutcom G_motion Z}
   set rapid_traverse_blk {G_cutcom G_motion X Y Z}
   set rapid_traverse_xy_blk {G_cutcom G_motion X Y}
   set rapid_traverse_yz_blk {G_cutcom G_motion Y Z}
   set rapid_traverse_xz_blk {G_cutcom G_motion X Z}
...
MOM_do_template G_mode_ABS_INC
...
#=============================================================
 ...
 # Heidenhain cycle types
  ...
  if { $mom_motion_event == "bore_drag_move" || $mom_motion_event == "bore_dwell_move" || $mom_motion_event == "bore_dwell_move" } {
    set cycle_type_number 202	;# bore
    set cycle_type_name "BORING"    
  }

   if { $mom_motion_event == "bore_manual_move" || $mom_motion_event == "bore_manual_dwell_move" || $mom_motion_event == "bore_no_drag_move" } {
    set cycle_type_number 202	;# bore
    set cycle_type_name "BORING"    
  }

   if { $mom_motion_event == "bore_back_move" } {
    set cycle_type_number 204	;# universal pecking
    set cycle_type_name "BACK BORING"
   }

   if { $mom_motion_event == "drill_deep_move" } {
    set cycle_type_number 205	;# universal pecking
    set cycle_type_name "UNIVERSAL PECKING"
  }
  ...
  if { $mom_motion_event == "tap_move" } {
    ...
    set cycle_type_name "RIGID TAPPING NEW"
  }
...
#########################################################
  if { $cycle_type_number == 207 } {	;# only tapping
    ...
    MOM_force Once cycle_type_name 
    MOM_do_template cycle_block_def		;# cycle def 200/201 etc
    MOM_set_seq_off
    MOM_force Once cycle_rapid_to cycle_feed_to cycle_tap_lead cycle_surface cycle_pullback   

    MOM_do_template cycle_block_Q200	;# Q200 setup ( engage ) ht 
    MOM_do_template cycle_block_Q201	;# Q201 hole depth ( incremental )   
    MOM_do_template cycle_block_Q239	;# Q239 thread lead 
    MOM_do_template cycle_block_Q203    ;# Q203 Z of top of hole ( absolute )    
    MOM_do_template cycle_block_Q204	;# Q204 Zreturn after cycle ( incremental )
     MOM_set_seq_on
  }

###############################################################
   if { $cycle_type_number == 200 } {	;# only tapping
    
    MOM_force Once cycle_type_name

    MOM_do_template cycle_block_def	;# cycle def 
    MOM_set_seq_off

    MOM_force Once cycle_rapid_to cycle_feed_to plunging_feed_rate cycle_plunge cycle_dwell_top 
    MOM_force Once cycle_surface cycle_pullback cycle_dwell

    MOM_do_template cycle_block_Q200	;# Q200 setup ( engage ) ht 
    MOM_do_template cycle_block_Q201	;# Q201 hole depth ( incremental )
    MOM_do_template cycle_block_Q206	;# Q206 feedrate units/min   
    MOM_do_template cycle_block_Q202	;# Q202 peck size 
    MOM_do_template cycle_block_Q210	;# Q210 dwell above hole hardwired to zero    
    MOM_do_template cycle_block_Q203    ;# Q203 Z of top of hole ( absolute )    
    MOM_do_template cycle_block_Q204	;# Q204 Zreturn after cycle ( incremental )    
    MOM_do_template cycle_block_Q211	;# Q211 cycle dwell 
     MOM_set_seq_on
  }

####################################################
   if { $cycle_type_number == 201 } {	;# only tapping   
    ...
    MOM_set_seq_off
    MOM_force Once cycle_rapid_to cycle_feed_to plunging_feed_rate cycle_dwell  
    MOM_force Once cycle_feedrate cycle_surface cycle_pullback  
   
    MOM_do_template cycle_block_Q200	;# Q200 setup ( engage ) ht 
    MOM_do_template cycle_block_Q201	;# Q201 hole depth ( incremental )
    MOM_do_template cycle_block_Q206	;# Q206 feedrate units/min 
    MOM_do_template cycle_block_Q211	;# Q211 cycle dwell   
    MOM_do_template cycle_block_Q208	;# Q208 feedrate for retraction       
    MOM_do_template cycle_block_Q203    ;# Q203 Z of top of hole ( absolute )    
    MOM_do_template cycle_block_Q204	;# Q204 Zreturn after cycle ( incremental )      
    MOM_set_seq_on
  }
...
#################################################################
if { $cycle_type_number == 202 } {	;# only boring
    ...
    MOM_force Once cycle_rapid_to cycle_feed_to plunging_feed_rate cycle_feedrate  
    MOM_force Once cycle_surface cycle_pullback cycle_dwell disengaging_direction angle_of_spindle

    MOM_do_template cycle_block_Q200	;# Q200 setup ( engage ) ht 
    MOM_do_template cycle_block_Q201	;# Q201 hole depth ( incremental )
    MOM_do_template cycle_block_Q206	;# Q206 feedrate units/min 
    MOM_do_template cycle_block_Q211
    MOM_do_template cycle_block_Q208      
    MOM_do_template cycle_block_Q203    ;# Q203 Z of top of hole ( absolute )    
    MOM_do_template cycle_block_Q204	;# Q204 Zreturn after cycle ( incremental )    
    MOM_do_template cycle_block_Q214
    MOM_do_template cycle_block_Q336 
     MOM_set_seq_on
  }


##########################################################
if { $cycle_type_number == 204 } {	;# only boring
    ...
    MOM_force Once cycle_rapid_to depth_of_counterbore material_thickness off_center_distance
    MOM_force Once tool_edge_height preposition_feedrate counterboring_feedrate counterbore_floor_dwell_time
    MOM_force Once cycle_surface cycle_pullback disengaging_direction angle_of_spindle

    MOM_do_template cycle_block_Q200	
    MOM_do_template cycle_block_Q249	
    MOM_do_template cycle_block_Q250	
    MOM_do_template cycle_block_Q251
    MOM_do_template cycle_block_Q252     
    MOM_do_template cycle_block_Q253       
    MOM_do_template cycle_block_Q254  
    MOM_do_template cycle_block_Q255
    MOM_do_template cycle_block_Q203
    MOM_do_template cycle_block_Q204
    MOM_do_template cycle_block_Q214
    MOM_do_template cycle_block_Q336 
    MOM_set_seq_on
  }
####################################################################
  if { $cycle_type_number == 205 } {	;# only tapping
    MOM_force Once cycle_type_name

    MOM_do_template cycle_block_def	;# cycle def 200/201 etc

    MOM_set_seq_off
    MOM_force Once cycle_rapid_to cycle_feed_to plunging_feed_rate cycle_plunge  
    MOM_force Once cycle_surface cycle_pullback decrement_pecking_depth
    MOM_force Once min_pecking_depth upper_adv_stop_distance lower_adv_stop_distance
    MOM_force Once chip_break_infeed_depth chip_break_distance cycle_dwell starting_position preposition_feedrate
       
    MOM_do_template cycle_block_Q200	;# Q200 setup ( engage ) ht    
    MOM_do_template cycle_block_Q201	;# Q201 hole depth ( incremental ) 
    MOM_do_template cycle_block_Q206    ;#Q206 Feed rate for plunging
    MOM_do_template cycle_block_Q202	;#Q202 Plunging depth
    MOM_do_template cycle_block_Q203    ;# Q203 Z of top of hole ( absolute )  
    MOM_do_template cycle_block_Q204	;# Q204 Zreturn after cycle ( incremental )    
    MOM_do_template cycle_block_Q212
    MOM_do_template cycle_block_Q205
    MOM_do_template cycle_block_Q258
    MOM_do_template cycle_block_Q259
    MOM_do_template cycle_block_Q257
    MOM_do_template cycle_block_Q256
    MOM_do_template cycle_block_Q211
    MOM_do_template cycle_block_Q379
    MOM_do_template cycle_block_Q253
    MOM_set_seq_on
  }
...
MOM_output_literal "% $mom_attr_PROGRAMVIEW_PROGRAM_NUMBER G$mom_sys_unit_code($mom_output_unit)"
}


#=============================================================
proc PB_CMD_program_tail { } {
#=============================================================
global mom_attr_PROGRAMVIEW_PROGRAM_NUMBER
global mom_group_name
global mom_sys_unit_code
...
if {![info exists mom_group_name]} {set mom_group_name ""}
if {![info exists mom_attr_PROGRAMVIEW_PROGRAM_NUMBER]} {set mom_attr_PROGRAMVIEW_PROGRAM_NUMBER "O0001"}
MOM_output_literal "N9999 % $mom_attr_PROGRAMVIEW_PROGRAM_NUMBER G$mom_sys_unit_code($mom_output_unit) *"
}
