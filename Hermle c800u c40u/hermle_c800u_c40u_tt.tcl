########################## TCL Event Handlers ##########################
#
#  Created by kleber.ronconi  @  Fri Feb 18 13:59:50 2005 E. South America Daylight Time
...
#=============================================================
  ...
  global mom_motion_event
   ...
   set rapid_traverse_blk {G_motion X Y Z fourth_axis fifth_axis Text G_cutcom}
   set rapid_traverse_xy_blk {G_motion X Y fourth_axis fifth_axis Text G_cutcom}
   set rapid_traverse_yz_blk {G_motion Y Z fourth_axis fifth_axis Text G_cutcom}
   set rapid_traverse_xz_blk {G_motion X Z fourth_axis fifth_axis Text G_cutcom}
...
#--------- Define a data ------------------------------
      ...
      set date(0) [lindex $date_split 0]
      ...
      set data1 "$date(0), $date(2)/$date(1)/$date(4)"
#------------------------------------------------------

 MOM_output_literal "$co ARQUIVO  : $mom_part_name $ci"
 MOM_output_literal "$co FEITO POR: $mom_logname $ci"
 MOM_output_literal "$co DATA     : WEG - $data1 $ci"
 MOM_output_literal "$co MAQUINA  : HERMLE C800U e C40U - COMANDO: ITNC530 $ci"
...
#=============================================================
proc PB_CMD_cycldef19 { } {
#=============================================================
global ang_A  ang_C

MOM_do_template tpcm
MOM_force once G_motion Z M 
MOM_do_template return_home
MOM_force once G_motion fourth_axis fifth_axis
MOM_do_template rapid_AC 
MOM_output_literal "CYCL DEF 19.0 PLANO DE TRABALHO"
MOM_output_literal "CYCL DEF 19.1 A$ang_A C$ang_C"
MOM_output_literal ";"
}
...
global flag_cb  ang_A  ang_C
global co ci proc_name

 ##--- inicio de programa: reseta mesa -------------
 #set ang_A	0
 #set ang_C	0
 #$proc_name
...
#==============================================================================
# 17/fev/05	---	Kleber		- Liberacao inicial;
#------------------------------------------------------------------------------
#
}
...
#=============================================================
proc PB_CMD_init_multi_axis { } {
#=============================================================
global tpcm_mode
global mom_operation_type
global mom_msys_matrix mom_msys_origin
global feed feed_m128

#-- se ja esta M128 retorna -------
if {$tpcm_mode == 128} return

switch [string range $mom_operation_type 0 4] {
  Seque - Varia {
		 set tpcm_mode	128
                 set feed_m128  "F[format %.0f $feed]."
                 MOM_force once G_motion
    		}
  default       {set feed_m128 " "}
}
...
#=============================================================
proc PB_CMD_rapid_AC { } {
#=============================================================
global ang_A  ang_C tpcm_mode feed_m128
...
#=============================================================
proc PB_CMD_reset_table { } {
#=============================================================
global ang_A  ang_C
global co ci proc_name  tpcm_mode

 #--- RESETA ANGULOS DA MESA -------------
 set ang_A	0
 set ang_C	0
 MOM_output_literal "$co ------- ZERAR ANGULOS --------- $ci"

 MOM_suppress once Text
 if {$tpcm_mode == 128} {set tpcm_mode	129}
 MOM_do_template tpcm
 $proc_name
}
...
if {$flag_cb == 1} {
        #--- inicio de programa: reseta angulos -------------
	set ang_A	[format %.3f $mom_out_angle_pos(0)]
	set ang_C	[format %.3f $mom_out_angle_pos(1)]
        set flag_cb	2
        $proc_name
} else {
        #---- verifica se existe a variavel mom_prev_out_angle_pos ----------------------
        ...
        set A_flag	0
        set C_flag	0
        if {$mom_out_angle_pos(0) != $mom_prev_out_angle_pos(0)} {set A_flag	1}
        if {$mom_out_angle_pos(1) != $mom_prev_out_angle_pos(1)} {set C_flag	1}

        #--- se houve mudanca no s angulos, chama rapid_AC ------
        if {[expr $A_flag + $C_flag] > 0} {
	    set ang_A	[format %.3f $mom_out_angle_pos(0)]
	    set ang_C	[format %.3f $mom_out_angle_pos(1)]
            set tpcm_mode	129
            $proc_name
...
#=============================================================
proc PB_CMD_verif_operation { } {
#=============================================================
global mom_out_angle_pos mom_prev_out_angle_pos
global mom_rotary_delta_4th mom_rotary_delta_5th
global flag_cb  ang_A  ang_C tpcm_mode
global feed_m128 proc_name mom_operation_type
global ang_type  flag_cb 
global mom_kin_arc_output_mode arc_mode_default

set feed_m128   " "

#---- define nome da proc que posiciona a mesa --------
switch [string range $mom_operation_type 0 4] {
  Seque - Varia {
                 if {$ang_type == "cdef19"} {PB_CMD_reset_table}
		 set proc_name PB_CMD_rapid_AC
                 set ang_type	"m128"
                 #---- transf. movimento linear em circular -------
                 set mom_kin_arc_output_mode "LINEAR"
                 MOM_reload_kinematics
                 MOM_force once tpcm
    		}
  default       {
                 if {$ang_type == "m128"} {PB_CMD_reset_table}
                 set proc_name PB_CMD_cycldef19
                 set ang_type	"cdef19"
                 #---- volta movimto circular ao tipo definido no pós -----
                 set mom_kin_arc_output_mode $arc_mode_default
                 MOM_reload_kinematics
                }
}
#------------------------------------------------------
...
MOM_output_literal ";==== TROCA DE FERRAMENTA ===="
}
