########################## TCL Event Handlers ##########################
#
#  Created by ronconi  @  Wed Mar 30 15:25:59 2005 E. South America Daylight Time
...
#=============================================================
  ...
  global mom_motion_event
   ...
   RAPID_SET
   set rapid_traverse_blk {G_adjust G_motion G_mode X Y Z fourth_axis fifth_axis S H M_spindle}
...
#=============================================================
#--- Chama a rotina que recalcula os angulos
PB_CMD_recalcula_ang_B

#--- Redefine variaveis do posicionamento angular --------
global mom_out_angle_pos  udc_out_angle_pos

set mom_out_angle_pos(0) $udc_out_angle_pos(0)
set mom_out_angle_pos(1) $udc_out_angle_pos(1)
}
...
set tempo_anterior   0
 ...
 MOM_output_literal "$co MAQUINA  : INGERSOL CAB 50 GRAUS - COMANDO: SINUMERIK 840D $ci"
...
#=============================================================
proc PB_CMD_recalcula_ang_B { } {
#=============================================================
global mom_pos
global mom_out_angle_pos
global udc_motion
global udc_pos
global udc_out_angle_pos
global flag

set udc_pos(0)		 $mom_pos(0)
set udc_pos(1)		 $mom_pos(1)
set udc_pos(2)		 $mom_pos(2)
set udc_out_angle_pos(0) $mom_out_angle_pos(0)
set udc_out_angle_pos(1) $mom_out_angle_pos(1)

set rad2deg		 [expr 90 / asin(1)]
set deg2rad		 [expr asin(1) / 90]

set b_rad		 [expr $mom_out_angle_pos(0)*$deg2rad]
set c_rad		 [expr $mom_out_angle_pos(1)*$deg2rad]
set nutating_rad	 [expr 50*$deg2rad]
set delta_angle		 [expr 90*$deg2rad]
set udc_b_rad		 [expr 2*(asin((sin($b_rad/2))/(sin($nutating_rad))))]
set udc_out_angle_pos(0) [expr $udc_b_rad*$rad2deg]

 if {$mom_out_angle_pos(0) != 0} {
     set udc_c_rad [expr $c_rad+($delta_angle-(atan(((cos($nutating_rad))*(1-(cos($udc_b_rad))))/(sin($udc_b_rad)))))]
     set udc_out_angle_pos(1) [expr $udc_c_rad*$rad2deg]

 } elseif {$mom_out_angle_pos(0) == 0} {
     set udc_out_angle_pos(1) $mom_out_angle_pos(1)
 }
