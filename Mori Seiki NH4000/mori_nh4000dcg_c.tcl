########################## TCL Event Handlers ##########################
#
#  Created by MattE  @  Sat Feb 25 12:06:14 2006 Pacific Daylight Time
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_x_axis_limit                      "560"
  set mom_kin_y_axis_limit                      "560"
  set mom_kin_z_axis_limit                      "630"
...
#=============================================================
   ...
   MOM_force Once G_mode M_spindle
   MOM_do_template return_home
   MOM_force Once G_return G_mode
   MOM_do_template return_home_1
   MOM_do_template fourth_axis_unclamp
   MOM_do_template fourth_axis_rotate_final
   MOM_do_template fourth_axis_clamp
   MOM_do_template pallet_change_A
   MOM_force Once M
   MOM_do_template end_of_program
   MOM_do_template rewind_stop_code
   PB_CMD_ErrorReportToScreen
...
#=============================================================
   ...
   PB_CMD_install_error_list_wish
   PB_CMD_InitProcs
   PB_CMD_init_subprogram
   MOM_set_seq_off
   MOM_do_template rewind_stop_code
   PB_CMD_program_name_set
   MOM_do_template program_name
   PB_CMD_post_name
   MOM_force Once G_mode
   MOM_do_template absolute_mode
   MOM_force Once G_cutcom
   MOM_do_template reset_all_cycles
   MOM_force Once G_mode M_spindle
   MOM_do_template return_home
   MOM_force Once G_return G_mode
   MOM_do_template return_home_1
   ...
   MOM_do_template fourth_axis_rotate_initial
   MOM_do_template fourth_axis_clamp
}
...
#=============================================================
  ...
  global mom_motion_event
   ...
   RAPID_SET
   set rapid_spindle_blk {G_adjust G_motion G_mode X Y Z H S T M_coolant eol}
   set rapid_spindle_x_blk {G_adjust G_motion G_mode X H S T M_coolant eol}
   set rapid_spindle_y_blk {G_adjust G_motion G_mode Y H S T M_coolant eol}
   set rapid_spindle_z_blk {G_adjust G_motion G_mode Z H S T M_coolant eol}
   set rapid_traverse_blk {G_motion G_mode G X Y Z eol}
   set rapid_traverse_xy_blk {G_motion G_mode G X Y eol}
   set rapid_traverse_yz_blk {G_motion G_mode G Y Z eol}
   set rapid_traverse_xz_blk {G_motion G_mode G X Z eol}
...
#=============================================================
proc PB_CMD_DwellSecToMS { } {
#=============================================================
#
#  This custom command changes the dwell seconds into miliseconds
#

global jr_cycle_delay
global mom_cycle_delay

set jr_cycle_delay [expr $mom_cycle_delay * 1000]
}


#=============================================================
proc PB_CMD_ErrorReportToScreen { } {
#=============================================================
#
#  This custom command reports errors that where stored in an error file to the screen in
#  a TK window
#  If no errors occurred the old existing file is deleted
#

global gkw_error_file_ch
global gkw_error_file_name
global mom_output_file_directory
global mom_output_file_basename

if {[info exists gkw_error_file_ch] == 1 }  {
  close $gkw_error_file_ch
  set my_wish "[MOM_ask_env_var UGII_CAM_AUXILIARY_DIR]ugwish"
  set wish_script "[MOM_ask_env_var UGII_CAM_POST_DIR]error_list.tcl"
  set ier [catch {exec $my_wish $wish_script $gkw_error_file_name}]
  if {$ier == 1}  {
    set my_wish "wish"
    exec $my_wish $wish_script $gkw_error_file_name
  }
}  else  {
  set gkw_error_file_name "$mom_output_file_directory${mom_output_file_basename}_error.out"
  file delete $gkw_error_file_name
}
}


#=============================================================
proc PB_CMD_ErrorSpindleRevolutions { } {
#=============================================================
# generate error message when spindle speed is out of range

global mom_operation_name
global mom_spindle_rpm
global mom_operation_type
global jr_spindle_speed
global jr_machined_material

set jr_spindle_rpm $mom_spindle_rpm
set range_low 1
set range_high 14000

if { $jr_spindle_rpm < $range_low || $jr_spindle_rpm > $range_high && [string range $mom_operation_type 0 9] != "Mill Machi" }  {
  set ErrorMessage(0) "in Operation: $mom_operation_name"
  set ErrorMessage(1) [format "Spindle RPM: %.0f is not in specified range (${range_low}-${range_high}) " $jr_spindle_rpm]
  set ErrorMessage(2) "i.e. data is not in database please enter data or choose different tool"
  if { $jr_spindle_rpm > $range_high } {
    set ErrorMessage(2) [format "Spindle RPM is reduced to %.0f in this operation" $range_high]
  }
  BNL_log_error_to_file WARNING ErrorMessage
}
}


#=============================================================
proc PB_CMD_ErrorToolNumber { } {
#=============================================================
# generate error message when toolnumber falls not in range
...
if { $mom_tool_number < $range_low || $mom_tool_number > $range_high }  {
  set ErrorMessage(0) "Error in Operation: $mom_operation_name"
  set ErrorMessage(1) "Toolnumber $mom_tool_number is not in specified range (${range_low}-${range_high})"

BNL_log_error_to_file ERROR ErrorMessage
}
}
...
#=============================================================
proc PB_CMD_InitProcs { } {
#=============================================================
uplevel #0 {

  # the following command will modify several event handlers on the fly
  # That are written standard by the Post Builder or that are
  # part of ugpost_base.tcl
  ...
  foreach { proc_name } { 
    ...
    if { $proc_name=="MOM_tap" } {
      set added_command "\nPB_CMD_check_init_rigid_tapping\n"
      set lookup_string "PB_CMD_check_init_rigid_tapping"
    }
    if { $proc_name=="MOM_tap_move" } {
      set added_command "\nPB_CMD_correct_feed_tapping\n"
      set lookup_string "PB_CMD_correct_feed_tapping"
    }
    if { $proc_name=="MOM_initial_move" } {
      set added_command "\nPB_CMD_test_for_subprogram\n"
      set lookup_string "PB_CMD_test_for_subprogram"
    }
    if { $proc_name=="MOM_end_of_program" } {
      set added_command "\nPB_CMD_rename_output_file\n"
      set lookup_string "PB_CMD_rename_output_file"
    }
    set proc_args ""
    set proc_body ""
    if { [info procs $proc_name] != ""  } {
        ...
        if { [string first $lookup_string $proc_body] < 0  } {
            proc $proc_name $proc_args $proc_body$added_command
        }
    }
  }
#==============================================================
...
#==============================================================
  ...
  global mom_previous_fixture_offset_value

# automatische tafel draai module
  global mom_rotation_angle
  global mom_rotation_angle_mr
  global mom_rotation_direction
  global turn_once
  global mom_msys_matrix
  global RAD2DEG
  global toggle_sub
  global previous_angle_msys
  variable t1 [expr $mom_msys_matrix(0) >= 0]
  variable t2 [expr $mom_msys_matrix(2) >= 0]
  variable test1 [expr $t1 == 1 && $t2 == 1]
  variable test2 [expr $t1 == 1 && $t2 == 0]
  variable test3 [expr $t1 == 0 && $t2 == 1]
  variable test4 [expr $t1 == 0 && $t2 == 0]
  variable rotation_angle_sin [expr $RAD2DEG * asin($mom_msys_matrix(2))]
  variable rotation_angle_cos [expr $RAD2DEG * acos($mom_msys_matrix(0))]
  variable rotation_angle_cosn [expr - $rotation_angle_cos]
  if {$test1 == 1} {set mom_rotation_angle [expr - $rotation_angle_sin]}
  if {$test2 == 1} {set mom_rotation_angle [expr - $rotation_angle_sin]}
  if {$test3 == 1} {set mom_rotation_angle [expr - $rotation_angle_cos]}
  if {$test4 == 1} {set mom_rotation_angle [expr - $rotation_angle_cosn]}
  if {$mom_rotation_angle_mr != 0} {set mom_rotation_angle [expr $mom_rotation_angle + $mom_rotation_angle_mr]}
  #MOM_output_literal "hoek $mom_rotation_angle"
  set turn_once 1
  PB_CMD_pallet_draai
  set turn_once 2
  MOM_do_template fourth_axis_unclamp
  MOM_do_template pallet_draai_b_as
  MOM_do_template fourth_axis_clamp
  
  #subprogram call if info exists
  if {$toggle_sub == 1} {MOM_subprogram_user}
  #aid to remember axisrotation from msys
  set previous_angle_msys $mom_rotation_angle

  #backup procedure if start of path does not do the right thing  
  #if {$mom_fixture_offset_value != $mom_previous_fixture_offset_value} {
    #MOM_do_template datum_shift_0
    #MOM_do_template datum_shift_1_offset_no
    #set mom_previous_fixture_offset_value $mom_fixture_offset_value
  #}
...
proc MOM__test {} {
  ...
  # Close Output Files and Join them together.

  global MainFP MainFile SubFP SubFile
  global mom_output_file_full_name

  puts $SubFP "%"

  if { [catch {close $MainFP}] } {
    MOM_output_to_listing_device "Could not close \"$MainFile\"!!!!"
    MOM_abort "Error Closing File, See Listing Window for more Information!"
  }    
  if { [catch {close $SubFP}] } {
    MOM_output_to_listing_device "Could not close \"$SubFile\"!!!!"
    MOM_abort "Error Closing File, See Listing Window for more Information!"
  }    

MOM_close_output_file $mom_output_file_full_name

exec {copy /Y $MainFile /A + $SubFile /A $mom_output_file_full_name /A}
...
#==============================================================
proc MOM_NRT_Post_init { } {
#==============================================================

  global mom_machined_material
  global mom_stock_offset
  global mom_type_of_movement
  global jr_machined_material
  global mom_operator_message
  global mom_product_phase
  global mom_sys_hpcc_code
  global jr_tool_number

  if { $mom_machined_material=="COPPER" } {
     set jr_machined_material "cu" 
     #MOM_output_literal ";material: $jr_machined_material"  
  }
  if { $mom_machined_material=="STEEL" } {
     set jr_machined_material "st"
     #MOM_output_literal ";material: $jr_machined_material"
  }
  if { $mom_machined_material=="HARDENED STEEL" } {
     set jr_machined_material "sth"
     #MOM_output_literal ";material: $jr_machined_material"
  }
  if { $mom_machined_material=="GRAPHITE" } {
     set jr_machined_material "gr"
     #MOM_output_literal ";material: $jr_machined_material"
  }
 
 PB_CMD_GetToolLibraryData
 ...
 foreach {message} {
	...
	if {$message=="part_name"} {
		global mom_part_name
		set mom_operator_message "Part name: $mom_part_name"
	}
	if {$message=="user_id"} {
		global mom_logname
		set mom_operator_message "User: $mom_logname"
	}
	if {$message=="material"} {
		global mom_machined_material
		set mom_operator_message "Material: $mom_machined_material"
	}
	if {$message=="tool"} {
		global mom_tool_name
		set mom_operator_message "Tool description: $mom_tool_name"
	}
	if {$message=="tool_number"} {
		global jr_tool_number
		set mom_operator_message "Tool number: $jr_tool_number"
	}
	if {$message=="machining_method"} {
		global mom_oper_method
		set mom_operator_message "Product phase: $mom_product_phase"
	}
	if {$message=="flute_length"} {
		global mom_tool_flute_length
		set mom_operator_message [format "Flute length: %.2f" $mom_tool_flute_length]
	} 
	if {$message=="tool_length"} {
		global mom_tool_length
		set mom_operator_message [format "Tool length: %.2f" $mom_tool_length]
	}
	if {$message=="type_of_movement"} {
		global mom_type_of_movement
		set mom_operator_message "Type of movement: $mom_type_of_movement"
	}	
...
#==============================================================
 ...
 foreach {message} {
	...
	if {$message=="stock_offset"} {
		global mom_stock_offset
		set mom_operator_message [format "size decrement: %.2f" $mom_stock_offset]
	}

 	MOM_operator_message
  }

} ;# MOM_NRT_stock_offset
...
#=============================================================
proc PB_CMD_SetFeedrateMode { } {
#=============================================================
# Set FeedRated Mode to MMPR for Tapping
...
MOM_set_address_format F Feed_MMPR
}
...
global mom_sys_coolant_code
   ...
   if {[info exists mom_coolant_mode]} {
	   if {$mom_coolant_mode == "SPINDLE"} {
		   set mom_sys_coolant_code(OFFSPINDLE) "89"
		   MOM_do_template return_motion_spindle
	   }
	   if {$mom_coolant_mode == "AIR"} {
		   set mom_sys_coolant_code(OFFAIR) "59"
		   MOM_do_template return_motion_air
	   }
	   if {$mom_coolant_mode == "ON"} {
		   MOM_do_template return_motion
	   }
	   if {$mom_coolant_mode == "FLOOD"} {
		   MOM_do_template return_motion
	   }
   } 


	if {[info exists jr_hpcc_flag]} {
	   if {$jr_hpcc_flag == 1} {
	      MOM_do_template hpcc_off
	      #MOM_do_template hpcc_off_1
	      set jr_hpcc_flag 0
	   }
	}
   ...
   MOM_force Once M
   MOM_do_template tool_change_4
   PB_CMD_end_of_alignment_character
   #MOM_output_literal "G05P10000;"
   MOM_force once X Y Z




        set jr_tool_change_flag 1
        PB_CMD_init_rigid_tapping
        PB_CMD_ErrorToolNumber
        PB_CMD_ErrorSpindleRevolutions
}
...
#=============================================================
proc PB_CMD_check_init_rigid_tapping { } {
#=============================================================
#
#  This procedure sorts initiation of rigid tapping
#
...
#force mmpr mode for rigid tapping
set mom_cycle_feed_rate_mode "MMPR"

#if {$irt_counter == 0} {
    #if { $mom_cycle_feed_rate_mode=="MMPR" } {
       MOM_do_template feedratemode_mmpr
       #MOM_output_literal "$mom_cycle_feed_rate"
       #MOM_output_literal "$mom_cycle_feed_rate_per_rev"
       set mom_cycle_feed_rate $mom_cycle_feed_rate_per_rev
       #} else {MOM_do_template feedratemode_mmpm}
    MOM_force once S M
    MOM_do_template init_rigid_tapping
    #}
...
#=============================================================
proc PB_CMD_correct_feed_tapping { } {
#=============================================================

#  This procedure sorts feeds for rigid tapping
#
global mom_cycle_feed_rate_mode
global mom_cycle_feed_rate_per_rev
global feed
...
#=============================================================
#
#  This procedure is prevents a tool change when the tool number is not altered
#
...
#=============================================================
proc PB_CMD_hpcc_off { } {
#=============================================================
#
#  This custom command checks and outputs G05P0 high precision contour control
#
...
#=============================================================
#
#  This custom command checks and outputs G05P10000 high precision contour control
#
...
set jr_planar_milling_boolean {$mom_operation_type == "Planar Milling"}
set jr_cavity_milling_boolean {$mom_operation_type == "Cavity Milling"}
set jr_face_milling_boolean {$mom_operation_type == "Face Milling"}
set jr_planar_or_cavity [expr $jr_planar_milling_boolean | $jr_cavity_milling_boolean]
set jr_planar_or_cavity_or_face [expr $jr_planar_or_cavity | $jr_face_milling_boolean]

if {$jr_planar_or_cavity_or_face} {
   if {$mom_motion_event == "initial_move"} {
      if {$jr_tool_change_flag == 1} {
         #MOM_do_template hpcc_on_1
         MOM_do_template hpcc_on
         set jr_hpcc_flag 1
         set jr_tool_change_flag 0
      }
   }
}
}
...
#=============================================================
proc PB_CMD_init_coolant { } {
#=============================================================
global mom_coolant_mode

if { $mom_coolant_mode == "SPINDLE" || $mom_coolant_mode == "FLOOD"}  {
	MOM_force once M_coolant_1
} elseif { $mom_coolant_mode == "AIR" } {
	MOM_force once M_coolant} else {
	MOM_suppress always M_coolant_1
}
MOM_force once M_spindle S F
}


#=============================================================
proc PB_CMD_init_rigid_tapping { } {
#=============================================================
#
#  This procedure sorts initiation of rigid tapping
#
...
#=============================================================
proc PB_CMD_init_spindle_rotation { } {
#=============================================================
global spindle_turning_flag
set spindle_turning_flag 1
}


#=============================================================
proc PB_CMD_init_subprogram { } {
#=============================================================
uplevel #0 {
#
...
#
# This procedure can be used to enable your post to output sub programs.

variable toggle_sub 0
variable jr_sub_count 1
...
proc MOM_subprogram { } {
#=============================================================
  global mom_sub_pr_number
  global mom_number_of_repetitions
  global temp_sub_file_name
  global mom_seqnum
  global main_program_seqnum
  global mom_sequence_number mom_sequence_increment mom_sequence_frequency
  global mom_output_file_directory mom_output_file_basename output_extn
  global main_ptp_file_name
  global sub_ptp_file_name
  global teller_sub
  global ptp_file_name
  global toggle_sub
  global jr_group_name
  global jr_sub_count

  #in order to get the axis rotations not inside subprograms
  #the initiation should be called from the msys procedure

  set toggle_sub 1
  set jr_sub_count [expr $jr_sub_count + 1]
} ;# MOM_subprogram
#=============================================================
proc MOM_subprogram_user { } {
#=============================================================
  global mom_sub_pr_number
  global mom_number_of_repetitions
  global temp_sub_file_name
  ...
  global jr_group_name
  global mom_use_subpr_number
  global jr_create_dir
  global jr_sub_count

  # write line to call subprogram
  #MOM_force once sub_prog_variable repetitions_sub
  if {$mom_use_subpr_number == "NO"} {
     #set jr_group_name [string range $mom_group_name 1 4]
     set jr_group_name "000$jr_sub_count"
     MOM_do_template subprogram_call
  } else {
     MOM_do_template subprogram_call_prno
  }
  

  # reset line number and store last one for the main program
  set main_program_seqnum $mom_seqnum
  MOM_reset_sequence $mom_sequence_number $mom_sequence_increment $mom_sequence_frequency
  MOM_set_seq_on

  # stop output to current outputfile main
  if {$teller_sub == "0"} {
     MOM_close_output_file $ptp_file_name
     set main_ptp_file_name $ptp_file_name
     } else {MOM_close_output_file $main_ptp_file_name}

  # rename output file to get the data inside the new sub file
  if {$mom_use_subpr_number == "NO"} {
     set mom_sub_pr_number $jr_group_name
     set temp_sub_file_name "O$jr_group_name"
  } else {
     set temp_sub_file_name "O$mom_sub_pr_number"
  }
  set jr_output_file_directory "N:\\UG\\NC_files\\mori_seiki\\"
  set jr_create_dir $jr_output_file_directory
  set sub_ptp_file_name "${jr_create_dir}${temp_sub_file_name}"
  if {[file exists $sub_ptp_file_name] == 1} {
      if {$teller_sub == "0"} {
          MOM_remove_file $sub_ptp_file_name
           #puts stdout  "  REMOVED PRE-EXISTING SUB $sub_ptp_file_name"
         }
     }
  MOM_open_output_file $sub_ptp_file_name
  set ptp_file_name $sub_ptp_file_name

  # put in the header for the subprogram
  MOM_set_seq_off
  MOM_do_template rewind_stop_code
  MOM_do_template subprogram_header
  #MOM_do_template start_of_sub_program
  #MOM_set_seq_on
  set teller_sub [expr $teller_sub + 1]

  #reset toggle_sub retarded axis rotation for use within subprogram
  set toggle_sub 0
} ;# MOM_subprogram_user
#=============================================================
proc MOM_endsubprogram { } {
#=============================================================
  global mom_end_sub_pr_number
  global mom_end_sub_status
  global main_program_seqnum
  global mom_sequence_number mom_sequence_increment mom_sequence_frequency
  global mom_output_file_directory mom_output_file_basename output_extn
  global main_ptp_file_name
  ...
  # continue with the line numbers of the main program
  MOM_force once M
  MOM_do_template subprogram_end
  MOM_do_template rewind_stop_code
  MOM_reset_sequence $main_program_seqnum $mom_sequence_increment $mom_sequence_frequency
  #MOM_set_seq_on

  #close the subprogram file
  MOM_close_output_file $sub_ptp_file_name

  #open mainprogram file
  set ptp_file_name $main_ptp_file_name
...
#=============================================================
  ...
  #test if a sub has been utilised
  if {[info exists sub_ptp_file_name] != 0} {

     #odd way to empty the buffer
     MOM_close_output_file $ptp_file_name
     MOM_open_output_file $ptp_file_name

     #add subprogram file to main program
     set invoer [open $sub_ptp_file_name r]
     set data [read $invoer]
     set uitvoer [open $main_ptp_file_name a]
     puts $uitvoer "$data"
     flush $uitvoer
     }
} ;# MOM_appendsub
...
#==============================================================
proc MOM_callsubprogram { } {
#==============================================================
  global mom_end_sub_pr_number
  global mom_number_of_repetitions
  global mom_fixture_offset_value
  global mom_zero_text
  global mom_work_coordinate_number
  global jr_group_name
  set mom_fixture_offset_value $mom_work_coordinate_number
  #MOM_force once G_motion
  #MOM_zero
  #MOM_force once sub_prog_variable repetitions_sub
  #MOM_do_template datum_shift_0
  #MOM_do_template datum_shift_1_offset_no
  #MOM_do_template datum_shift_1_ix
  #MOM_do_template datum_shift_2_iy
  #MOM_do_template datum_shift_3_iz
  MOM_force once G
  #MOM_do_template local_coordinate_system
  MOM_force once M
  MOM_do_template subprogram_call
  #MOM_force once G X Y Z
  #MOM_do_template local_coordinate_system_cancel
} ;# MOM_callsubprogram
#==============================================================
proc MOM_callsubprrt { } {
#==============================================================
  global mom_end_sub_pr_number
  global mom_number_of_repetitions
  global mom_rotation_angle_mr
  ...
  global turn_once
  global previous_angle_msys
  set mom_rotation_angle [expr $previous_angle_msys + $mom_rotation_angle]
  MOM_rotate
  MOM_force once sub_prog_variable repetitions_sub
  MOM_do_template start_sub
} ;# MOM_callsubprrt
#==============================================================
proc MOM_calllabel { } {
#==============================================================
  global mom_label_number
  global mom_number_of_repetitions
  global mom_fixture_offset_value
  global mom_zero_text
  global mom_work_coordinate_number
...
#=============================================================
proc PB_CMD_install_error_list_wish { } {
#=============================================================
# Install error_list.tcl when not available

set FileContent  {# error_list.tcl --
#
# This outputs the content of the file (1st argument) into a window for the user
#
# Date		Who	Description
# 27-sep-2002	GKW	Initial version
#

for {set idx 0} {$idx < $argc} {incr idx} {if {[string last "wish.tcl" [lindex $argv $idx]] == -1} {break}}
set input_file [lindex $argv $idx]

set w .
#catch {destroy $w}
#toplevel $w
wm title . "Postprocessor Warning and/or Error messages"
#wm iconname $w "text"
#positionWindow $w

frame .buttons
pack .buttons -side bottom -fill x -pady 2m
button .buttons.dismiss -text close -command "destroy $w"
pack .buttons.dismiss -side left -expand 1

text .text -relief sunken -bd 2 -yscrollcommand ".scrolly set" -setgrid 1  -height 30 -bg white -width 90 -font courier
scrollbar .scrolly -command ".text yview"
pack .scrolly -side right -fill y
pack .text -expand yes -fill both

set input_ch [open $input_file r]
set text_output ""
for {set line 1} {![eof $input_ch]} {incr line}  {
  append text_output "[gets $input_ch]\n"
}
.text insert end $text_output
}

set FileName [MOM_ask_env_var UGII_CAM_POST_DIR]error_list.tcl

if { ![file exists $FileName] } {
  if { [catch {open $FileName w} FileChannel] } {
    MOM_abort "Not possible to install \"$FileName\""
  } else {
    puts $FileChannel $FileContent
    close $FileChannel
  }
}
}
...
#=============================================================
proc PB_CMD_output_ct { } {
#=============================================================
  global mom_machine_time

  MOM_output_literal [format "(Machine time: %.2f min.)" $mom_machine_time]
}


#=============================================================
proc PB_CMD_pallet_draai { } {
#=============================================================
global mom_b_as_preset 
global mom_out_angle_pos
global turn_once
global mom_rotation_angle
#set mom_pos(3) $mom_b_as_preset
#set mom_out_angle_pos(0) $mom_b_as_preset
set mom_out_angle_pos(0) [expr $mom_out_angle_pos(0) + $mom_rotation_angle]
if {$turn_once == 1} {set mom_out_angle_pos(0) $mom_rotation_angle}
if {$mom_out_angle_pos(0) >= 360} {set mom_out_angle_pos(0) [expr $mom_out_angle_pos(0) - 360]}
if {$mom_out_angle_pos(0) < 0} {set mom_out_angle_pos(0) [expr $mom_out_angle_pos(0) + 360]}
if {$turn_once == 2} {MOM_suppress once fourth_axis}
set turn_once 0
}
...
# options for full path name, file name only , uppercase
  ...
  MOM_output_literal "(POSTPROCESSOR FILE AND PATH:);"
  MOM_output_literal "([ string toupper $mom_event_handler_file_name ]);"
}


#=============================================================
proc PB_CMD_program_name_set { } {
#=============================================================
  # Outputs the first line with program number limited to 4 characters

global mom_group_name mom_operation_name
global gkw_program_name

if { ![info exists mom_group_name] }  {
  set mom_group_name $mom_operation_name
}

# MOM_output_literal "%[string range $mom_group_name 1 17] G71"

set gkw_program_name [string range $mom_group_name 1 4]
}
...
#=============================================================
  ...
  global mom_machine_time
     ...
     set invoer [open $ptp_file_name r]
     set data [read $invoer]
     set jr_main_file_name "O$gkw_program_name"
     set jr_dbsl {\\}
     set jr_acc {'}
     set jr_output_file_directory "N:\\UG\\NC_files\\mori_seiki\\"
     set main_ptp_file_name "${jr_output_file_directory}${jr_main_file_name}"
     set uitvoer [open $main_ptp_file_name w]
     puts $uitvoer "$data"
     flush $uitvoer
     close $uitvoer
     close $invoer
...
#=============================================================
proc PB_CMD_spindle_rotation_test { } {
#=============================================================
global spindle_turning_flag


if {$spindle_turning_flag == 1} {MOM_do_template spindle}
set spindle_turning_flag 0
}
...
#=============================================================
proc PB_CMD_test_for_subprogram { } {
#=============================================================
  global mom_fixture_offset_value
  global toggle_sub
  global jr_group_name
...
#=============================================================
proc BNL_log_error_to_file { gkw_error_level gkw_error_array } {
#=============================================================
#
#  This custom command reports errors to a file
#
upvar $gkw_error_array gkw_error_message
...
if {[info exists gkw_error_file_ch] == 0 }  {
  set gkw_error_file_name "$mom_output_file_directory${mom_output_file_basename}_error.out"
  set gkw_error_file_ch [open $gkw_error_file_name w]
}

puts $gkw_error_file_ch [format "%-8s%-80s" $gkw_error_level $gkw_error_message(0)]
for {set i 1} {$i < [array size gkw_error_message]} {incr i}  {
puts $gkw_error_file_ch [format "        %-80s" $gkw_error_message($i)]
}
unset gkw_error_message
}
...
#***************************
 set user_tcl_file ${cam_post_dir}mori_nh4000dcg_c_user.tcl
