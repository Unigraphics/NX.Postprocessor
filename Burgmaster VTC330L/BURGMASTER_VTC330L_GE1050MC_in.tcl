...
#
#  Created by suyo  @  Tue Apr 22 18:33:57 2008 China Standard Time
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_cir_vector                        "Unsigned Vector - Arc Start to Center"
  set mom_sys_spindle_ranges                    "0"  
  set mom_sys_rewind_stop_code                  "\#" 
  set mom_sys_home_pos(0)                       "0"  
  set mom_sys_home_pos(1)                       "15" 
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_x_axis_limit                      "60" 
  set mom_kin_y_axis_limit                      "36" 
  set mom_kin_z_axis_limit                      "12" 
...
#=============================================================
   MOM_do_template home_z
   MOM_do_template home_xy
...
#=============================================================
  ...
  global mom_motion_event
   ...
   RAPID_SET
   PB_CMD_check_spindle_speed
   set rapid_spindle_blk {G_motion G_mode X Y Z F H}
   set rapid_spindle_x_blk {G_motion G_mode X F H}
   set rapid_spindle_y_blk {G_motion G_mode Y F H}
   set rapid_spindle_z_blk {G_motion G_mode Z F H}
   set rapid_traverse_blk {G_motion G_mode X Y Z F S M_spindle}
   set rapid_traverse_xy_blk {G_motion G_mode X Y F S M_spindle}
   set rapid_traverse_yz_blk {G_motion G_mode Y Z F S M_spindle}
   set rapid_traverse_xz_blk {G_motion G_mode X Z F S M_spindle}
...
if { $mom_spindle_speed < 30 } {
    set mom_spindle_speed 30
     set mom_warning_info "The speed is lower than the min speed"
     MOM_catch_warning
   } elseif { $mom_spindle_speed > 2000 } {
    set mom_spindle_speed 2000
    set mom_warning_info "The speed is higer than the max speed"
    MOM_catch_warning
   }
}
...
global mom_nxt_pos
global mom_cutcom_adjust_register
global x_cutcom y_cutcom i_cutcom j_cutcom
global first_linear_flag  first_cutcom_flag

if { [info exists mom_cutcom_adjust_register] } {

    set x_cutcom [expr $mom_prev_pos(0)+0.0001]
    set y_cutcom [expr $mom_prev_pos(1)+0.0001]

    if { $mom_prev_pos(0) == $mom_pos(0) } {
      set j_cutcom 90
      MOM_suppress once X I
    } elseif { $mom_prev_pos(1) == $mom_pos(1) } {
      set i_cutcom 90
      MOM_suppress once Y J
    } else {
     set delt_x [expr $mom_pos(0)- $mom_prev_pos(0)]
     set delt_y [expr $mom_pos(1)- $mom_prev_pos(1)]
     set delt_l [expr sqrt($delt_x*$delt_x+$delt_y*$delt_y)]
     set i_cutcom [expr abs($delt_x/$delt_l)]
     set j_cutcom [expr abs($delt_y/$delt_l)]
    }

    if { $first_cutcom_flag } {
    MOM_force once G_cutcom D
    MOM_do_template linear_move_cutcom   
    set first_cutcom_flag 0
    }
}
}
...
#=============================================================
proc PB_CMD_first_linear_move_flag { } {
#=============================================================
global first_linear_flag

set first_linear_flag 1
}
...
#=============================================================
proc PB_CMD_last_motion_type { } {
#=============================================================
global mom_motion_type mom_motion_event
global last_motion_type last_motion_event

set last_motion_type $mom_motion_type
set last_motion_event $mom_motion_event
}
...
if { $first_linear_flag } {
   set first_linear_flag 0
   return
   }
...
#=============================================================
proc PB_CMD_rigid_tap_detect { } {
#=============================================================
global mom_cycle_option
global mom_spindle_speed mom_spindle_range
global pause_once_flag


if { ![info exists pause_once_flag] } { set pause_once_flag 0 }

if { $mom_spindle_range != 1 && $mom_spindle_range != 3 } {
   if { !$pause_once_flag } {
   PAUSE "The spindle speed for TAPPING should be set at range 1 or 3"
   set pause_once_flag 1
   }
}
...
#=============================================================
proc PB_CMD_save_first_z { } {
#=============================================================
global mom_pos
global saved_z 
global first_z_flag

set saved_z $mom_pos(2)
}


#=============================================================
proc PB_CMD_save_spindle_speed { } {
#=============================================================
global mom_spindle_speed
global saved_spindle_speed
...
#=============================================================
proc PB_CMD_set_circular_plane { } {
#=============================================================
...
if {$mom_spindle_range == 1 || $mom_spindle_range == "LHLG"} {
##Low HP LOW GEAR
   set rpm(0)         45    ; set scode(0)  0
   set rpm(1)         49    ; set scode(1)  1
   set rpm(2)         55    ; set scode(2)  2
   set rpm(3)         61    ; set scode(3)  3
   set rpm(4)         67    ; set scode(4)  4
   set rpm(5)         73    ; set scode(5)  5
   set rpm(6)         80    ; set scode(6)  6
   set rpm(7)         87    ; set scode(7)  7
   set rpm(8)         95    ; set scode(8)  8
   set rpm(9)        103    ; set scode(9)  9
   set rpm(10)       113    ; set scode(10)  10
   set rpm(11)       122    ; set scode(11)  11
   set rpm(12)       132    ; set scode(12)  12
   set rpm(13)       143    ; set scode(13)  13
   set rpm(14)       156    ; set scode(14)  14
   set rpm(15)       169    ; set scode(15)  15
   set rpm(16)       184    ; set scode(16)  16
   set rpm(17)       200    ; set scode(17)  17
   set rpm(18)       219    ; set scode(18)  18
   set rpm(19)       243    ; set scode(19)  19
   set range_table_min_index 0
   set range_table_max_index 19

} elseif {$mom_spindle_range == 2 || $mom_spindle_range == "HHLG"} {

## HIGH HP LOW GEAR
   set rpm(20)       138    ; set scode(20)  20
   set rpm(21)       149    ; set scode(21)  21
   set rpm(22)       164    ; set scode(22)  22
   set rpm(23)       181    ; set scode(23)  23
   set rpm(24)       199    ; set scode(24)  24
   set rpm(25)       218    ; set scode(25)  25
   set rpm(26)       237    ; set scode(26)  26
   set rpm(27)       261    ; set scode(27)  27
   set rpm(28)       283    ; set scode(28)  28
   set rpm(29)       308    ; set scode(29)  29
   set rpm(30)       336    ; set scode(30)  30
   set rpm(31)       360    ; set scode(31)  31
   set rpm(32)       395    ; set scode(32)  32
   set rpm(33)       426    ; set scode(33)  33
   set rpm(34)       465    ; set scode(34)  34
   set rpm(35)       505    ; set scode(35)  35
   set rpm(36)       547    ; set scode(36)  36
   set rpm(37)       600    ; set scode(37)  37
   set rpm(38)       652    ; set scode(38)  38
   set rpm(39)       725    ; set scode(39)  39
   set range_table_min_index 20
   set range_table_max_index 39

} elseif {$mom_spindle_range == 3 || $mom_spindle_range == "LHHG"} {
###LOW HP, HIGH GEAR
   set rpm(40)        223    ; set scode(40)  40
   set rpm(41)        243    ; set scode(41)  41
   set rpm(42)        268    ; set scode(42)  42
   set rpm(43)        294    ; set scode(43)  43
   set rpm(44)        324    ; set scode(44)  44
   set rpm(45)        354    ; set scode(45)  45
   set rpm(46)        387    ; set scode(46)  46
   set rpm(47)        424    ; set scode(47)  47
   set rpm(48)        465    ; set scode(48)  48
   set rpm(49)        502    ; set scode(49)  49
   set rpm(50)        547    ; set scode(50)  50
   set rpm(51)        590    ; set scode(51)  51
   set rpm(52)        643    ; set scode(52)  52
   set rpm(53)        698    ; set scode(53)  53
   set rpm(54)        762    ; set scode(54)  54
   set rpm(55)        815    ; set scode(55)  55
   set rpm(56)        900    ; set scode(56)  56
   set rpm(57)        966    ; set scode(57)  57
   set rpm(58)       1092    ; set scode(58)  58
   set rpm(59)       1170    ; set scode(59)  59
   set range_table_min_index 40
   set range_table_max_index 59

} elseif {$mom_spindle_range == 4 || $mom_spindle_range == "HHHG"} {
##HIGH HP, HIGH GEAR
   set rpm(60)        684    ; set scode(60)  60
   set rpm(61)        726    ; set scode(61)  61
   set rpm(62)        809    ; set scode(62)  62
   set rpm(63)        886    ; set scode(63)  63
   set rpm(64)        987    ; set scode(64)  64
   set rpm(65)       1061    ; set scode(65)  65
   set rpm(66)       1182    ; set scode(66)  66
   set rpm(67)       1265    ; set scode(67)  67
   set rpm(68)       1400    ; set scode(68)  68
   set rpm(69)       1507    ; set scode(69)  69
   set rpm(70)       1653    ; set scode(70)  70
   set rpm(71)       1767    ; set scode(71)  71
   set rpm(72)       1930    ; set scode(72)  72
   set rpm(73)       2048    ; set scode(73)  73
   set rpm(74)       2266    ; set scode(74)  74
   set rpm(75)       2461    ; set scode(75)  75
   set rpm(76)       2676    ; set scode(76)  76
   set rpm(77)       2909    ; set scode(77)  77
   set rpm(78)       3181    ; set scode(78)  78
   set rpm(79)       3500    ; set scode(79)  79
   set range_table_min_index 60
   set range_table_max_index 79
}
   set s_code $scode($range_table_min_index)

   if {$saved_spindle_speed  <  $rpm($range_table_min_index)} {
      set mom_warning_info "spindle speed under minimum, assumed minimum"
      MOM_catch_warning

   } elseif {$saved_spindle_speed  >  $rpm($range_table_max_index) } {
      set mom_warning_info "spindle speed over maximum, assumed maximum"
      MOM_catch_warning
      set s_code $scode($range_table_max_index)

   } else {
      for {set i $range_table_min_index} {$i  <=  $range_table_max_index} {incr i} {
         if { $saved_spindle_speed == $rpm($i) } {
            set s_code $scode($i)
            break
         } else {
          if { $saved_spindle_speed > $rpm($i) && $saved_spindle_speed < $rpm([expr $i+1]) } {
            set s_code $scode($i)
         #   PAUSE "The spindle speed is not exactly match in the spindle table in \n $mom_operation_name"
            break
         }          
        }
      }
...
set out $mom_sys_control_out
set in $mom_sys_control_in 


MOM_output_literal "$out ****** OPERATION NAME: $mom_operation_name ******* $in"
MOM_output_literal "$out ********************************************* $in"


global first_g86_flag
set first_g86_flag 1

global first_linear_flag
set first_linear_flag 1

global first_cutcom_flag
set first_cutcom_flag 1
}


#=============================================================
proc PB_CMD_suppress_g00 { } {
#=============================================================
global mom_pos mom_prev_pos


if {![info exists mom_prev_pos] } { 
   set mom_prev_pos(0) -999999999
   set mom_prev_pos(1) -999999999
}

if {$mom_pos(0) == $mom_prev_pos(0) && $mom_pos(1) == $mom_prev_pos(1)} {
...
#=============================================================
proc PB_CMD_suppress_g01 { } {
#=============================================================
global mom_pos mom_prev_pos

if {![info exists mom_prev_pos] } { 
   set mom_prev_pos(0) -9999999
   set mom_prev_pos(1) -9999999
}
...
#=============================================================
proc PB_CMD_suppress_g86 { } {
#=============================================================
global last_motion_type
global last_motion_event

if { $last_motion_event == "initial_move" || $last_motion_event == "rapid_move" } {  
    MOM_force once g86 RZ
   } else {
    MOM_suppress once g86
   }
}
