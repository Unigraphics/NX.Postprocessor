...
#
#  Created by suyo  @  Wed Apr 23 16:34:46 2008 China Standard Time
#  with Post Builder version  5.0.2.
#
########################################################################
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_home_pos(0)                       "10" 
  set mom_sys_home_pos(1)                       "10" 
  set mom_sys_home_pos(2)                       "10" 
  set mom_sys_zero                              "0"  
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_4th_axis_max_limit                "20" 
  ...
  set mom_kin_rapid_feed_rate                   "2362.2"
  ...
  set mom_kin_x_axis_limit                      "63" 
  set mom_kin_y_axis_limit                      "49.2"
  set mom_kin_z_axis_limit                      "39.4"
...
#=============================================================
   ...
   MOM_force Once G Z L
   MOM_do_template absolute_position_z
   MOM_force Once G X Y L
   MOM_do_template absolute_position_xy
   MOM_do_template m56
   MOM_force Once coord_rot A5 B5 C5 L1
   MOM_do_template coord_rot_cancel
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   ...
   MOM_do_template coord_shift_abs
...
#=============================================================
   ...
   PB_CMD_program_info
   MOM_do_template cutcom_off
   MOM_do_template inch_metric_mode
}
...
#=============================================================
# This proc is used to overwrite the automatical G141/G7 
...
#
  ...
  if { $mom_motion_event == "initial_move" || $mom_motion_event == "rapid_move" || $mom_motion_event == "first_move" || $mom_motion_type == "FROM" || $mom_motion_type == "APPROACH" } {
  
    if [llength [info commands PB_CMD_def_work_plane]] {
...
#=============================================================
proc PB_CMD_before_tool_change { } {
#=============================================================
MOM_force Once G Z
MOM_do_template absolute_position_z
...
MOM_force Once coord_rot A5 B5 C5 L1
MOM_do_template coord_rot_cancel
MOM_do_template tool_withdrawal_move
MOM_do_template opstop
}
...
if { $cycle_status == "OFF" } {
   set cycle_status ON
   MOM_force once cycle_y cycle_z cycle_retract_distance F
}
}
...
#=============================================================
  ...
  if { [string match "PART" $mom_sys_nc_output_mode] } {  
   ...
   # Disable work plane change for G141
    set mom_sys_work_plane_change 0

  } elseif { [string match "MACHINE" $mom_sys_nc_output_mode] } {

    if { ![EQ_is_equal $mom_out_angle_pos(0) $mom_prev_out_angle_pos(0)] || ![EQ_is_equal $mom_out_angle_pos(1) $mom_prev_out_angle_pos(1)] } {
    
    MOM_output_literal "(*********coord_rot_on*************)"
    MOM_force once G X Y Z L
    MOM_do_template absolute_position_z
    MOM_force once coord_rot A5 B5 C5 B7 C7 L1    
    MOM_do_template coord_rot_on
    MOM_do_template rapid_rotary_g7
    MOM_output_literal "(*********************************)" 
    }
...
#=============================================================
proc PB_CMD_disable_circular_move { } {
#=============================================================
global mom_sys_nc_output_mode
global mom_kin_arc_output_mode

if { $mom_sys_nc_output_mode == "MACHINE" } {
...
global mom_5axis_control_mode
catch {unset mom_5axis_control_mode}
}
...
## if the operation is not use local csys, then can output G141(M128)
 global mom_kin_coordinate_system_type
...
if {$mom_kin_coordinate_system_type != "CSYS"} {
   if { $mom_operation_type == "Variable-axis Surface Contouring" && $mom_tool_axis_type != 0 } {     
...
#=============================================================
  ...
  #<04-16-08 gsl> Initialize NC output mode to machine space
  #               - This should be default PB generated code
   global mom_sys_nc_output_mode
   if { ![info exists mom_sys_nc_output_mode] } {
      set mom_sys_nc_output_mode "AUTO"
   }
}
...
if { [info exists mom_5axis_control_mode] } {
    ...
    if { $mom_5axis_control_mode == "G141" } {
      set mom_sys_nc_output_mode "PART" 
     } elseif { $mom_5axis_control_mode == "G7" } {
      set mom_sys_nc_output_mode "MACHINE" 
     } 
...
#PB_CMD_disable_circular_move
}


#=============================================================
proc PB_CMD_path_info { } {
#=============================================================
...
set ci 	$mom_sys_control_in
 ...
 MOM_output_literal "$co Path : $mom_path_name  $ci" 
 MOM_output_literal "$co $mom_tool_name $mom_tool_number   $ci"
}
...
set ci 	$mom_sys_control_in
 ...
 MOM_output_literal "$co POSTPROCESSOR: [ string toupper $mom_event_handler_file_name ] $ci"
 MOM_output_literal "$co DATE     : $mom_date   $ci"
 MOM_output_literal "$co PROGRAMMER: $mom_logname   $ci"
}
...
#=============================================================
 ...
 if { $mom_kin_coordinate_system_type != "CSYS"} {
  ...
  set delt_x [format "%.${decimals}f" $mom_csys_matrix(9)]
  set delt_y [format "%.${decimals}f" $mom_csys_matrix(10)]
  set delt_z [format "%.${decimals}f" $mom_csys_matrix(11)]
  set rot_a [format "%.${rotary_decimals}f" $A]
  set rot_b [format "%.${rotary_decimals}f" $B]
  set rot_c [format "%.${rotary_decimals}f" $C]
}
...
#=============================================================
proc PB_CMD_tool_3d_correction { } {
#=============================================================
   global mom_sys_nc_output_mode   
   global mom_cutcom_status
   ...
   if { $mom_sys_nc_output_mode == "PART" } {
      if { $mom_cutcom_status == "OFF" } { 
        MOM_force once G_cutcom L2 F2
        set mom_cutcom_status ON
          }         
         MOM_do_template tool_3d_correction               
      }
}
