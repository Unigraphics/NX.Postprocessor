...
#
#  Created by suderman  @  Fri Aug 31 15:28:02 2007 Pacific Daylight Time
...
#=============================================================
   ...
   MOM_do_template xyplane_gohome
...
#=============================================================
proc PB_CMD_PB_CMD_ude_uplevel { } {
#=============================================================
uplevel #0 {	

set mom_stops_number 8
set mom_index_code 60

##override the proc MOM_text

proc  MOM_text {} {
#_______________________________________________________________________________
# This procedure is executed when the Text command is activated.
#_______________________________________________________________________________
  global mom_user_defined_text mom_record_fields mom_field_count
  global mom_sys_control_out mom_sys_control_in
  ...
  global mom_index_code
  global mom_stops_number
  global mom_switch_number
  ...
  "SET" {
   switch $mom_record_fields(1) {
	   "SWITCH" {
		  set mom_switch_number $mom_record_fields(2)		  
		   }
		"STOPS" {
	      set mom_stops_number $mom_record_fields(2)
			}
	    "INDEX" {
	      set mom_index_code $mom_record_fields(2)
			}          
      }
  }
...
#=============================================================
proc PB_CMD_output_index_table { } {
#=============================================================
...
# rotary index_table to preset stops.
   ...
   global pre_switch_number

  global mom_warning_info

   if {![info exists mom_switch_number]} {
     set mom_warning_info "The Switch Number doesn't define" 
     MOM_catch_warning 
     return
    }
   if {![info exists mom_stops_number]} {
     set mom_warning_info "The stop number doesn't define" 
     MOM_catch_warning 
     return
   }
  if {![info exists mom_index_code]} {
     set mom_warning_info "The Index Code doesn't define" 
     MOM_catch_warning 
     return
    }
   

   set index_code "M$mom_index_code"

   if {![info exists pre_switch_number]} {set pre_switch_number 0}
   if {[info exists mom_switch_number]} {	
      if {[EQ_is_equal $mom_switch_number $pre_switch_number]} {
         return
         }
     }
   

   set delta_index [expr $mom_switch_number - $pre_switch_number]

   if {$delta_index < 0} {set delta_index [expr $delta_index + $mom_stops_number]}
   
   for {set i 0} {$i < $delta_index} {incr i} {
      MOM_output_literal "$index_code"
   }

   set pre_switch_number $mom_switch_number
}
