...
#
#  Created by zhang  @  Tue Jun 17 14:21:24 2008 China Standard Time
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_x_axis_limit                      "14.2"
  set mom_kin_y_axis_limit                      "26.8"
  set mom_kin_z_axis_limit                      "30.7"
...
#=============================================================
  ...
  global mom_motion_event
   ...
   set rapid_spindle_blk {G_mode G_motion G_adjust H X Y Z}
   set rapid_spindle_x_blk {G_mode G_motion G_adjust H X}
   set rapid_spindle_y_blk {G_mode G_motion G_adjust H Y}
   set rapid_spindle_z_blk {G_mode G_motion G_adjust H Z}
   set rapid_traverse_blk {G_motion G X Y Z M_coolant}
   set rapid_traverse_xy_blk {G_motion G X Y M_coolant}
   set rapid_traverse_yz_blk {G_motion G Y Z M_coolant}
   set rapid_traverse_xz_blk {G_motion G X Z M_coolant}
...
#=============================================================
proc PB_CMD_dummy_tool { } {
#=============================================================
global prepare_tool_number mom_tool_number
set mom_tool_number $prepare_tool_number
 ...
 global mom_tool_number mom_next_tool_number
  ...
  if {[info exists mom_next_oper_has_tool_change]} {
    if {$mom_next_oper_has_tool_change == "YES" || $mom_next_tool_status == "FIRST"} {
        ...
        MOM_do_template spindle_off
        MOM_force Once G_mode G_motion G Z
        MOM_do_template home_z_axis
        MOM_do_template tool_len_adj_off
        MOM_do_template machine_coordinate_system
        MOM_force Once G_mode G_motion G X Y
        MOM_do_template home_xy_axis
...
#=============================================================
proc PB_CMD_index_b_axis { } {
#=============================================================
global mom_out_angle_pos
global last_b_angle

if { ![info exists last_b_angle ] } {
  set last_b_angle 0
}

if { $last_b_angle == $mom_out_angle_pos(0) } {
  return
}
...
set last_b_angle $mom_out_angle_pos(0)
...
#
  global mom_index_number
   global mom_mcode_start
   set index_code "M$mom_mcode_start"
...
#
   ...
   set angle_factor [expr 1.0/$mom_index_number]
    for {set i 0} {$i < $mom_index_number} {incr i} {
       set stops($i) [expr 360*$i*$angle_factor]
    }
 ...
 #  set total_stops [llength $stops]
   ...
   if {$delta_index < 0} {set delta_index [expr $delta_index + $mom_index_number]}
...
#=============================================================
global is_first_TC_flag 

set is_first_TC_flag  YES
}
...
#=============================================================
global mom_next_tool_status
global mom_next_tool_number mom_tool_number prepare_tool_number
global is_first_TC_flag

if { $is_first_TC_flag == "YES" } {
   MOM_force once T M
   MOM_do_template tool_change_1
   MOM_force once T M
   MOM_do_template tool_preselect
   set is_first_TC_flag NO
 } else {
   MOM_force once T M
   MOM_do_template tool_preselect
 }
...
if { $mom_next_tool_status == "FIRST" } {
 set prepare_tool_number [expr $mom_tool_number+1]
}
}
