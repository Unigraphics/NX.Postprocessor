...
#  Haas_VF-80.tcl -
#
#  Created by marks  @  Tue Sep 16 09:03:41 2008 Eastern Daylight Time
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_max_arc_radius                    "9.999"
  ...
  set mom_kin_x_axis_limit                      "84" 
  set mom_kin_y_axis_limit                      "32" 
...
#=============================================================
  ...
  global mom_motion_event
   ...
   RAPID_SET
   set rapid_spindle_blk {G_adjust G G_motion G_mode H X Y Z M_coolant}
   set rapid_spindle_x_blk {G_adjust G G_motion G_mode H X M_coolant}
   set rapid_spindle_y_blk {G_adjust G G_motion G_mode H Y M_coolant}
   set rapid_spindle_z_blk {G_adjust G G_motion G_mode H Z M_coolant}
...
if {![string match "*CONTROL" [string toupper $mom_operation_type]]} {
  ...
  for {set i 0} {$i<[string length $mom_path_name]} {incr i} {
    ...
    switch -- $c {
      ...
      "\]" -
      "*" { }
      default { append pname $c }
      }
    }

  MOM_output_text "${co}OPER: $pname${ci}"
...
global haas_tool_name

# issue operator display message for tool change
# -- uses TOOL CATALOG NUMBER field if present
...
# MOM_output_text "${co}TOOL # $mom_tool_number: $tnam${ci}"
set haas_tool_name $tnam
}
