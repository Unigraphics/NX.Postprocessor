##############################################################################
# Description
#     This is the TCL file for HELLER / UNIPRO II - 3 axis mill.
#
# Revisions
#
#   Date        Who   Reason
# 30-apr-2003   emk   Original.
# 23-may-2003   emk   PP corrected according to the PTR #134.
# 17-mar-2003   emk   PP corrected according to the PTR #2860.
#
# $HISTORY$
#     This post-processor was created using POSTBUILDER_VERSION=3.0.0.
#     This is a TCL implementation. UG/Post Builder edition is not
# available anymore.
#
###############################################################################
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_home_pos(0)                       "1250"
  set mom_sys_home_pos(1)                       "630"
  set mom_sys_home_pos(2)                       "560"
  set mom_sys_zero                              "0"  
...
#=============================================================
   PB_CMD_force_m9_m19
   MOM_force Once G_motion G_mode G X Y Z Text
   MOM_do_template start_of_program_1
   MOM_output_literal "M66"
   MOM_output_literal "M40"  
...
#=============================================================
  ...
  global mom_motion_event
   ...
   RAPID_SET
   set rapid_spindle_blk {G_adjust G_motion G_mode X Y Z}
   set rapid_spindle_x_blk {G_adjust G_motion G_mode X}
   set rapid_spindle_y_blk {G_adjust G_motion G_mode Y}
   set rapid_spindle_z_blk {G_adjust G_motion G_mode Z}
...
#=============================================================
proc PB_CMD_force_g54 { } {
#=============================================================
MOM_force once X Y Z G G_motion
}


#=============================================================
proc PB_CMD_force_m9_m19 { } {
#=============================================================
...
#=============================================================
global mom_tool_number  mom_tool_name mom_path_name
global mom_tool_diameter  mom_tool_catalog_number

set diam  [format %.1f $mom_tool_diameter]

MOM_output_literal "(C, $mom_path_name)"
MOM_output_literal "(C, $mom_tool_catalog_number D$diam)"
}
...
#=============================================================
global mom_tool_axis  cabec_flag 
global mom_date
global cycle_P0 cycle_P1  cycle_P2  cycle_P3  cycle_P4  cycle_P5
...
MOM_output_literal "(C, DATA $user_date)"


 #set mom_sys_spindle_axis(0) $mom_tool_axis(0)
 #set mom_sys_spindle_axis(1) $mom_tool_axis(1)
 #set mom_sys_spindle_axis(2) $mom_tool_axis(2)
}
...
#=============================================================
proc PB_CMD_posicao_cabecote { } {
#=============================================================
global mom_tool_axis cycle_P90  cabec_flag 
global mom_last_pos  mom_pos

if { ![info exists mom_last_pos] } {
	set mom_last_pos(0) $mom_pos(0)
        set mom_last_pos(1) $mom_pos(1)
        set mom_last_pos(2) $mom_pos(2)
}

 if {$cabec_flag == 0} {
     MOM_output_literal "(C, POSICAO VERTICAL)"
     set cabec_flag 1
 }
 MOM_output_literal "M66"
 set cycle_P90  "Y"
...
#=============================================================
proc PB_CMD_redefine_workplane_set { } {
#=============================================================
#=============================================================
proc WORKPLANE_SET {  } {
#=============================================================
   ...
   if { ![info exists mom_sys_spindle_axis] } {
      ...
      set xx $mom_sys_spindle_axis(0)
      set yy $mom_sys_spindle_axis(1)
      set zz $mom_sys_spindle_axis(2)

      if { [EQ_is_zero $yy] && [EQ_is_zero $zz] } {
         set mom_cycle_spindle_axis 0
      } elseif { [EQ_is_zero $xx] && [EQ_is_zero $zz] } {
...
#=============================================================
proc PB_CMD_set_cycle { } {
#=============================================================
global mom_cycle_rapid_to_pos  mom_cycle_feed_to_pos
global mom_cycle_retract_to_pos  mom_cycle_delay
global mom_cycle_feed_rate  mom_cycle_feed_rate_per_rev
global mom_cycle_feed_to  mom_cycle_rapid_to  mom_cycle_retract_to
global mom_cycle_step1  mom_cycle_step2
global cycle_P90  mom_current_motion
global cycle_P0 cycle_P1  cycle_P2  cycle_P3  cycle_P4  cycle_P5

set repete  0

if {$cycle_P0 != $mom_cycle_rapid_to} {
    set repete  1
    set cycle_P0   $mom_cycle_rapid_to
}
if {$cycle_P1 != $mom_cycle_feed_to_pos(1)} {
    set repete  1
    set cycle_P1   $mom_cycle_feed_to_pos(1)
}

if {[info exists mom_cycle_delay]} {
    if {$cycle_P2 != $mom_cycle_delay} {
        set repete  1
        set cycle_P2   $mom_cycle_delay
    }
}

if {$mom_current_motion == "drill_deep_move"} {
     if {$cycle_P3 != $mom_cycle_step1} {
         set repete  1
         set cycle_P3   $mom_cycle_step1
     }
     if {[info exists mom_cycle_step2]} {
         if {$cycle_P4 != $mom_cycle_step2} {
             set repete  1
             set cycle_P4   $mom_cycle_step2
    }
    } else {
            if {$cycle_P4 != $mom_cycle_step1} {
                set repete  1
                set cycle_P4   $mom_cycle_step1
            }
    }
}

if {$cycle_P5 != $mom_cycle_retract_to} {
     set repete  1
     set cycle_P5   $mom_cycle_retract_to
}

if {$repete == 1} {
    #MOM_force once cycle_p0 cycle_p1 cycle_p2 cycle_p3 cycle_p4 cycle_p5 cycle_p90
    MOM_force once cycle_p0 cycle_p1 cycle_p5 cycle_p90
}
}
