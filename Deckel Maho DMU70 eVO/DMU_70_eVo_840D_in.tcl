...
#
#  Created by zhang  @  Thu Aug 28 16:10:54 2008 China Standard Time
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_4th_axis_max_limit                "102"
...
#=============================================================
  global mom_tool_change_type mom_manual_tool_change
   MOM_do_template supa_z
   MOM_do_template supa_y
   MOM_do_template supa_x
   ...
   PB_CMD_next_tool_set
   MOM_force Once D
   MOM_do_template auto_tool_adj
...
#=============================================================
  ...
  global mom_motion_event
   ...
   PB_CMD_coordainte_set
   PB_CMD_suppress_B_C
   set rapid_traverse_blk {G_cutcom G_motion G_mode X Y Z fourth_axis fifth_axis S M_spindle}
...
#=============================================================
proc PB_CMD_MCS_CSYS_initialize { } {
#=============================================================
#MCS_CSYS
global wq_DMG_C
set wq_DMG_C 0
...
global wq_origin wq_vector_x wq_vector_y wq_vector_z
global wq_C_angle wq_B_angle wq_A_angle
global RAD2DEG DEG2RAD
global mom_kin_coordinate_system_type
 
if {$mom_kin_coordinate_system_type == "CSYS"} {
   for {set i 0} {$i<3} {incr i} {
       set wq_vector_x($i) $mom_csys_matrix($i)
       set wq_vector_y($i) $mom_csys_matrix([expr $i+3])
       set wq_vector_z($i) $mom_csys_matrix([expr $i+6])            

       set vsA($i) 0.0;set vsAA($) 0.0;set vsAAA($i) 0.0
       set veA($i) 0.0;set veAA($) 0.0;set veAAA($i) 0.0
       set vcA($i) 0.0;set vcAA($) 0.0;set vcAAA($i) 0.0
       set vbA($i) 0.0;set vbAA($) 0.0;set vbAAA($i) 0.0

       set vsB($i) 0.0;set vsBB($) 0.0;set vsBBB($i) 0.0
       set veB($i) 0.0;set veBB($) 0.0;set veBBB($i) 0.0
       set vcB($i) 0.0;set vcBB($) 0.0;set vcBBB($i) 0.0
       set vbB($i) 0.0;set vbBB($) 0.0;set vbBBB($i) 0.0

       set vsC($i) 0.0;set vsCC($) 0.0;set vsCCC($i) 0.0
       set veC($i) 0.0;set veCC($) 0.0;set veCCC($i) 0.0
       set vcC($i) 0.0;set vcCC($) 0.0;set vcCCC($i) 0.0
       set vbC($i) 0.0;set vbCC($) 0.0;set vbCCC($i) 0.0
         
       set vny($i) 0.0
     }

     set temp(0) 0.0;set temp(1) 0.0;set temp(2) 1.0	
     if {![VEC3_is_equal wq_vector_z temp]} {
         set vsA(0) 0.0 ; set vsA(1) 0.0 ; set vsA(2) 1.0
	 set veA(0) 0.0 ; set veA(1) $wq_vector_z(1) ;  set veA(2) $wq_vector_z(2)
         if {![VEC3_is_zero veA]} {
              VEC3_unitize veA veAA
	      VEC3_cross vsA veAA vcA
	      set angleA [expr [VEC3_dot vsA veAA]/([VEC3_mag vsA]*[VEC3_mag veAA])]
	      if [EQ_is_ge $angleA 1.0] {set angleA 1.0}
	      if [EQ_is_le $angleA -1.0] {set angleA -1.0}
	      set wq_A_angle [expr acos($angleA)*$RAD2DEG]
              set vbA(0) 1.0
              set vbA(1) 0.0
              set vbA(2) 0.0
              VEC3_unitize vcA vcAA
              set chk_pointA [VEC3_dot vcAA vbA]
              if [EQ_is_lt $chk_pointA 0.0] { set wq_A_angle [expr -$wq_A_angle] }
         } else {
              set wq_A_angle 0.0
         }

         set vsB(0) 0.0
	 set vsB(1) $wq_vector_z(1)
	 set vsB(2) $wq_vector_z(2)
	 set veB(0) $wq_vector_z(0)
	 set veB(1) $wq_vector_z(1)
	 set veB(2) $wq_vector_z(2)
         if {![VEC3_is_zero vsB]} {
            VEC3_unitize vsB vsBB
	    VEC3_cross vsBB veB vcB
            set angleB [expr [VEC3_dot vsBB veB]/([VEC3_mag vsBB]*[VEC3_mag veB])]
	    if [EQ_is_ge $angleB 1.0] {set angleB 1.0}
	    if [EQ_is_le $angleB -1.0] {set angleB -1.0}
	    set wq_B_angle [expr acos($angleB)*$RAD2DEG]
	    set vbB(0) 0.0
	    set vbB(1) 1.0
            set vbB(2) 0.0
	    set axis_x(0) 1.0
	    set axis_x(1) 0.0
	    set axis_x(2) 0.0
	    VEC3_rot_any vbB axis_x wq_A_angle vbBB
	    VEC3_unitize vcB vcBB
	    VEC3_unitize vbBB vbBBB
            set chk_pointB [VEC3_dot vcBB vbBBB]
	    set True_B $wq_B_angle
            if [EQ_is_lt $chk_pointB 0.0] {
               set wq_B_angle [expr -$wq_B_angle]
             }
         } else {
            if [EQ_is_le $wq_vector_z(0) 0.0] {
                set wq_B_angle -90.0
            } else {
                set wq_B_angle 90.0
            }
         ...
         for {set i 0} {$i<3} {incr i} {
            ...
            set roted_xx($i) 0.0
	    set roted_xxx($i) 0.0
	    set roted_xxxx($i) 0.0
          }
	  set axis_x(0) 1;set axis_x(1) 0;set axis_x(2) 0;
	  set axis_y(0) 0;set axis_y(1) 1;set axis_y(2) 0;
	  set axis_yy(0) 0;set axis_yy(1) 1;set axis_yy(2) 0;

	  VEC3_unitize vcB vcBB
	  VEC3_rot_any axis_y axis_x wq_A_angle axis_yy
	  VEC3_rot_any axis_x axis_yy wq_B_angle roted_x
	  VEC3_unitize roted_x roted_xx
          set angleC [expr [VEC3_dot roted_xx wq_vector_x]/([VEC3_mag roted_xx]*[VEC3_mag wq_vector_x])]
	  if [EQ_is_ge $angleC 1.0] {set angleC 1.0}
	  if [EQ_is_le $angleC -1.0] {set angleC -1.0}
	  set wq_C_angle [expr acos($angleC)*$RAD2DEG]
	  VEC3_cross roted_xx wq_vector_x roted_xxx
	  VEC3_unitize roted_xxx roted_xxxx
	  set chk_pointC [VEC3_dot roted_xxxx wq_vector_z]
	   if [EQ_is_lt $chk_pointC 0.0] {
	      set wq_C_angle [expr -$wq_C_angle]
          }
     } else { 
	   set wq_A_angle 0.0
	   set wq_B_angle 0.0
	   ...
	   set vsC(1) 0.0
	   set vsC(2) 0.0
	   set veC(0) $wq_vector_x(0)
	   set veC(1) $wq_vector_x(1)
	   set veC(2) $wq_vector_x(2)
	   set vbC(0) 0.0
	   set vbC(1) 0.0
	   set vbC(2) 1.0

	   set angleC [expr [VEC3_dot vsC veC]/([VEC3_mag vsC]*[VEC3_mag veC])]	    
           if [EQ_is_ge $angleC 1.0] {set angleC 1.0}
	   if [EQ_is_le $angleC -1.0] {set angleC -1.0}
	   set wq_C_angle [expr acos($angleC)*$RAD2DEG]

	   VEC3_cross vsC veC vcC
           set chk_pointC [VEC3_dot vcC vbC]
           if [EQ_is_lt $chk_pointC 0.0] {
               set wq_C_angle [expr -$wq_C_angle]
            } 
       }
...
#####################################
proc  VEC3_rot_any {u v a w} {
#####################################
global DEG2RAD RAD2DEG
      upvar $u u1 ; upvar $v v1 ; upvar $a a1 ; upvar $w w1
      set m(0) [expr $v1(0)*$v1(0)*(1-cos($a1*$DEG2RAD)) + cos($a1*$DEG2RAD)]
      set m(1) [expr $v1(1)*$v1(0)*(1-cos($a1*$DEG2RAD))+$v1(2)*sin($a1*$DEG2RAD)]
      set m(2) [expr $v1(2)*$v1(0)*(1-cos($a1*$DEG2RAD))-$v1(1)*sin($a1*$DEG2RAD)]
      set m(3) [expr $v1(0)*$v1(1)*(1-cos($a1*$DEG2RAD))-$v1(2)*sin($a1*$DEG2RAD)]
      set m(4) [expr $v1(1)*$v1(1)*(1-cos($a1*$DEG2RAD)) + cos($a1*$DEG2RAD)]
      set m(5) [expr $v1(2)*$v1(1)*(1-cos($a1*$DEG2RAD))+$v1(0)*sin($a1*$DEG2RAD)]
      set m(6) [expr $v1(0)*$v1(2)*(1-cos($a1*$DEG2RAD))+$v1(1)*sin($a1*$DEG2RAD)]
      set m(7) [expr $v1(1)*$v1(2)*(1-cos($a1*$DEG2RAD))-$v1(0)*sin($a1*$DEG2RAD)]
      set m(8) [expr $v1(2)*$v1(2)*(1-cos($a1*$DEG2RAD)) + cos($a1*$DEG2RAD)]
      for {set i 0} {$i<3} {incr i} {	
      set w1($i) [expr $u1(0)*$m($i)+$u1(1)*$m([expr $i+3])+$u1(2)*$m([expr $i+6])]
      }
}
...
proc wq_ZToolAxis { } {
##########################################
global mom_tool_axis wq_ZTool_X wq_ZTool_Y RAD2DEG

if {[EQ_is_zero $mom_tool_axis(2)]} {
    if {[EQ_is_zero $mom_tool_axis(1)]} {
        if {[EQ_is_gt $mom_tool_axis(0) 0.0]} {
              set wq_ZTool_X 0
	      set wq_ZTool_Y 90.0
        } elseif {[EQ_is_lt $mom_tool_axis(0) 0.0]} {
              set wq_ZTool_X 0
              set wq_ZTool_Y -90.0
        } else {
              MOM_output_literal "Tool Axis Vector is incorrect as 0,0,0"
	      set wq_ZTool_X 0
              set wq_ZTool_Y 0
        }
    } else {
        if {[EQ_is_zero $mom_tool_axis(0)]} {
             if {[EQ_is_gt $mom_tool_axis(1) 0.0]} {
	      set wq_ZTool_X -90.0
              set wq_ZTool_Y 0 
             } elseif {[EQ_is_lt $mom_tool_axis(1) 0.0]} {
	      set wq_ZTool_X 90.0
              set wq_ZTool_Y 0   
             } else {
              MOM_output_literal "Tool Axis Vector is incorrect as 0,0,0"
	      set wq_ZTool_X 0
             ...
             set wq_ZTool_X -90.0
             set wq_ZTool_Y [expr atan($mom_tool_axis(0)/$mom_tool_axis(1))*$RAD2DEG]
             if {[EQ_is_lt $mom_tool_axis(1) 0.0]} {
                  if {[EQ_is_gt $mom_tool_axis(0) 0.0]} {
                     set wq_ZTool_Y [expr 180.0 + $wq_ZTool_Y]
                  } else { 
                     set wq_ZTool_Y [expr -180.0 + $wq_ZTool_Y]
                  }
             }
    ...
    if {[EQ_is_zero $mom_tool_axis(1)]} {
        set wq_ZTool_X 0.0
        if {[EQ_is_zero $mom_tool_axis(0)]} {
            if {[EQ_is_ge $mom_tool_axis(2) 0.0]} {
               set wq_ZTool_Y 0.0
            } else {
               set wq_ZTool_Y 180.0
            }
        } else {
            set wq_ZTool_Y [expr atan($mom_tool_axis(0)/$mom_tool_axis(2))*$RAD2DEG]
            if {[EQ_is_lt $mom_tool_axis(2) 0.0]} {
                if {[EQ_is_ge $mom_tool_axis(0) 0.0]} {
                    set wq_ZTool_Y [expr 180.0 + $wq_ZTool_Y]
                } else {
                    set wq_ZTool_Y [expr -180.0 + $wq_ZTool_Y]
                }
        ...
        set wq_ZTool_X [expr atan($mom_tool_axis(1)/$mom_tool_axis(2))*$RAD2DEG]
            if {[EQ_is_lt $mom_tool_axis(2) 0.0]} {
                if {[EQ_is_ge $mom_tool_axis(1) 0.0]} {
                     set wq_ZTool_X [expr 180.0 + $wq_ZTool_X]
                } else {
                     set wq_ZTool_X [expr -180.0 + $wq_ZTool_X]
                }
            }
            set wq_ZTool_X [expr -$wq_ZTool_X]
        if {[EQ_is_zero $mom_tool_axis(0)]} {
            set wq_ZTool_Y 0.0
        } else {
            set wq_ZTool_Y [expr atan($mom_tool_axis(0)/sqrt($mom_tool_axis(1)*$mom_tool_axis(1)+$mom_tool_axis(2)*$mom_tool_axis(2)))*$RAD2DEG]
        }
    }
}
}    
...
##########################################                      
proc MCS_Angle {Vector Angle1 Angle2} {
##########################################
upvar $Vector V
upvar $Angle1 A1
upvar $Angle2 A2
...
if {![info exists V(2)]}  {
     MOM_output_literal ";(WARNING: Variable V(2) not exists)"
     return
} else {
     if [EQ_is_zero $V(2)] {
        set A1 -90.0
     } else {
        set A1 [expr $RAD2DEG*atan(sqrt($V(0)*$V(0)+$V(1)*$V(1))/$V(2))]
        if [EQ_is_lt $V(2) 0.0] {
            set A1 [expr 180.0 + $A1]
        }
        set A1 [expr -$A1]
     }
}

if {[EQ_is_zero $V(0)]&&[EQ_is_zero $V(1)]} {
    set A2 0.0
} else {
    if [EQ_is_zero $V(1)] {
       if [EQ_is_gt $V(0) 0.0] {
       ...
       set A2 [expr $RAD2DEG*atan($V(0)/$V(1))]
       if [EQ_is_lt $V(1) 0.0] {
           set A2 [expr 180 + $A2]
       }
    }
...
#=============================================================
proc PB_CMD_coordainte_set { } {
#=============================================================
global pop_operation_type mom_pos mom_mcs_goto 


if {$pop_operation_type=="5axis"} {
...
#MOM_output_literal "********$mom_sys_spindle_direction_code(0)"
MOM_output_literal "********$mom_spindle_direction"
}


#=============================================================
proc PB_CMD_cycle_def { } {
#=============================================================
...
global pop_cycle_retract_to
global pop_mom_pos
global pop_cycle_clearance_plane
global pop_mom_cycle_feed_to_pos
...
if { $mom_out_angle_pos(1) !=  $mom_prev_out_angle_pos(1) } {
...
if { $pop_flag == "True" } {
	...
	set pop_flag "False"
#	MOM_force once X Y
#	MOM_force once G_motion wq_RTP wq_RFP wq_SDIS wq_DPR wq_DP wq_FDEP wq_FDPR wq_DAM wq_DTB wq_DTS wq_SDAC wq_FFR wq_RFF wq_FRF wq_MPIT wq_VARI wq_PIT wq_POSS wq_SST wq_SST1 wq_SDIR wq_RPA wq_RPO wq_RPAP wq_POSS_G86
#	MOM_force once wq_comma_1 wq_comma_2 wq_comma_3 wq_comma_4 wq_comma_5 wq_comma_6 wq_comma_7 wq_comma_8 wq_comma_9 wq_comma_10 wq_comma_11 wq_comma_12 wq_cycle_end
}

#Cycle redefine
if { [info exists pop_cycle_retract_to] } {

	if { $pop_cycle_retract_to != $mom_cycle_retract_to ||  $pop_mom_pos != $mom_pos(2) || $pop_cycle_clearance_plane != $mom_cycle_clearance_plane || $pop_mom_cycle_feed_to_pos != $mom_cycle_feed_to_pos(2)} {
		MOM_force once X Y
		MOM_force once G_motion wq_RTP wq_RFP wq_SDIS wq_DPR wq_DP wq_FDEP wq_FDPR wq_DAM wq_DTB wq_DTS wq_SDAC wq_FFR wq_RFF wq_FRF wq_MPIT wq_VARI wq_PIT wq_POSS wq_SST wq_SST1 wq_SDIR wq_RPA wq_RPO wq_RPAP wq_POSS_G86
		MOM_force once wq_comma_1 wq_comma_2 wq_comma_3 wq_comma_4 wq_comma_5 wq_comma_6 wq_comma_7 wq_comma_8 wq_comma_9 wq_comma_10 wq_comma_11 wq_comma_12 wq_cycle_end
...
#Store the value of the lastest para.
set pop_cycle_retract_to $mom_cycle_retract_to
set pop_mom_pos $mom_pos(2)
set pop_cycle_clearance_plane $mom_cycle_clearance_plane
set pop_mom_cycle_feed_to_pos $mom_cycle_feed_to_pos(2)


set mom_sys_leader(G_motion) "MCALL CYCLE"
}


#=============================================================
proc PB_CMD_cycle_end { } {
#=============================================================
global mom_sys_leader pop_operation_type

set mom_sys_leader(G_motion) "G"
MOM_force once G_motion wq_RTP wq_DP wq_RFP wq_SDIS wq_DPR wq_FDEP wq_FDPR wq_DAM wq_DTB wq_DTS wq_SDAC wq_FFR wq_RFF wq_FRF wq_MPIT wq_VARI wq_PIT wq_POSS wq_SST wq_SST1 wq_SDIR wq_RPA wq_RPO wq_RPAP wq_POSS_G86
MOM_force once wq_comma_1 wq_comma_2 wq_comma_3 wq_comma_4 wq_comma_5 wq_comma_6 wq_comma_7 wq_comma_8 wq_comma_9 wq_comma_10 wq_comma_11 wq_comma_12 wq_cycle_end
}
...
#=============================================================
global wq_prev_msys_matrix mom_msys_matrix
global mom_S840D_RTP 
global mom_S840D_RFP 
global mom_S840D_SDIS 
global mom_S840D_DPR 
global mom_S840D_FDEP 
global mom_S840D_FDPR 
global mom_S840D_SDR 
global mom_S840D_DAM 
global mom_S840D_DTB 
global mom_S840D_DTS 
global mom_S840D_SDAC 
global mom_S840D_FFR 
global mom_S840D_RFF 
global mom_S840D_FRF 
global mom_S840D_MPIT 
global mom_S840D_VARI 
global mom_S840D_ENC 
global mom_S840D_PIT 
global mom_S840D_POSS 
global mom_S840D_SST 
global mom_S840D_SST1 
global mom_S840D_SDIR 
global mom_S840D_RPA 
global mom_S840D_RPO 
global mom_S840D_RPAP 
global mom_S840D_POSS_G86
global mom_S840D_PTAB
global pop_save_angle_B 
global pop_save_angle_C
global pop_save_rot_B
global pop_save_rot_C

for {set i 0} {$i<9} {incr i} {
    set wq_prev_msys_matrix([expr $i+3]) $mom_msys_matrix($i)
}

if [info exists mom_S840D_RTP] {unset mom_S840D_RTP}
if [info exists mom_S840D_RFP] {unset mom_S840D_RFP}
if [info exists mom_S840D_SDIS] {unset mom_S840D_SDIS}
if [info exists mom_S840D_DPR] {unset mom_S840D_DPR}
if [info exists mom_S840D_FDEP] {unset mom_S840D_FDEP}
if [info exists mom_S840D_FDPR] {unset mom_S840D_FDPR}
if [info exists mom_S840D_SDR] {unset mom_S840D_SDR}
if [info exists mom_S840D_DAM] {unset mom_S840D_DAM}
if [info exists mom_S840D_DTB] {unset mom_S840D_DTB}
if [info exists mom_S840D_DTS] {unset mom_S840D_DTS}
if [info exists mom_S840D_SDAC] {unset mom_S840D_SDAC}
if [info exists mom_S840D_FFR] {unset mom_S840D_FFR}
if [info exists mom_S840D_RRF] {unset mom_S840D_RRF}
if [info exists mom_S840D_FRF] {unset mom_S840D_FRF}
if [info exists mom_S840D_MPIT] {unset mom_S840D_MPIT}
if [info exists mom_S840D_VARI] {unset mom_S840D_VARI}
if [info exists mom_S840D_ENC] {unset mom_S840D_ENC}
if [info exists mom_S840D_PIT] {unset mom_S840D_PIT}
if [info exists mom_S840D_POSS] {unset mom_S840D_POSS}
if [info exists mom_S840D_SST] {unset mom_S840D_SST}
if [info exists mom_S840D_SST1] {unset mom_S840D_SST1}
if [info exists mom_S840D_SDIR] {unset mom_S840D_SDIR}
if [info exists mom_S840D_RPA] {unset mom_S840D_RPA}
if [info exists mom_S840D_RPO] {unset mom_S840D_RPO}
if [info exists mom_S840D_RPAP] {unset mom_S840D_RPAP}
if [info exists mom_S840D_POSS_G86] {unset mom_S840D_POSS_G86}
if [info exists mom_S840D_PTAB] {unset mom_S840D_PTAB}
...
global pop_cycle_clearance_plane
global pop_mom_cycle_feed_to_pos
if [info exists pop_cycle_retract_to] {unset pop_cycle_retract_to}
if [info exists pop_mom_pos] {unset pop_mom_pos}
if [info exists pop_cycle_clearance_plane] {unset pop_cycle_clearance_plane}
if [info exists pop_mom_cycle_feed_to_pos] {unset pop_mom_cycle_feed_to_pos}
}
...
MOM_output_literal "SUPA G00 D0 Z-.1"
MOM_output_literal "SUPA Y-.1"
MOM_output_literal "SUPA X.1"
MOM_output_literal "G00 G90 B0.0 C0.0"
...
#=============================================================
#Init the sys Varibles
global pop_operation_type
global mom_sys_leader

set pop_operation_type "3axis"
set mom_sys_leader(G_motion) "G"
}
...
#=============================================================
proc PB_CMD_next_tool_set { } {
#=============================================================
#The end operation unset next_tool_num, not output next Tool.
global mom_next_tool_status mom_next_tool_number

if {[info exists mom_next_tool_status]&&$mom_next_tool_status=="FIRST"} {
    if {[info exists mom_next_tool_number]} {
         unset mom_next_tool_number
    }
}
}
...
#=============================================================
proc PB_CMD_operation_set_initial { } {
#=============================================================
#MCS Rotation & Fixture offset  

global mom_fixture_offset_value mom_out_angle_pos
...
#If 5aixs
#MOM_output_literal "$mom_kin_coordinate_system_type"

if {$mom_kin_coordinate_system_type == "CSYS" } {
        set mom_warning_info "Error.Please use Local & Use Main Coordinate"
	MOM_catch_warning
}
if {[string match "*VAR*" [string toupper $mom_operation_type]] && $mom_kin_coordinate_system_type != "CSYS"} {
...
#=============================================================
proc PB_CMD_operation_type_cancel { } {
#=============================================================
...
#=============================================================
proc PB_CMD_pro_start { } {
#=============================================================
#Init the system
MOM_output_literal "G00 G90 G70 G40"
...
#MOM_output_literal "Get $mom_fixture_offset_value"
if {![info exist mom_fixture_offset_value]||$mom_fixture_offset_value=="0"} {
...
global wq_origin
  ...
  set wq_origin(0) [format "%.${decimals}f" $mom_csys_matrix(9)]
  set wq_origin(1) [format "%.${decimals}f" $mom_csys_matrix(10)]
  set wq_origin(2) [format "%.${decimals}f" $mom_csys_matrix(11)]
if 0 {
  set wq_A_angle [format "%.${rotary_decimals}f" $A]
  set wq_B_angle [format "%.${rotary_decimals}f" $B]
  set wq_C_angle [format "%.${rotary_decimals}f" $C]
}
}
}
...
#=============================================================
proc PB_CMD_suppress_B_C { } {
#=============================================================
global pop_operation_type

if {$pop_operation_type == "3Plus2"} {
