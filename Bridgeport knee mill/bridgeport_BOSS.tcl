...
#=============================================================
  ...
  global mom_motion_event
   ...
   RAPID_SET
   set rapid_spindle_blk {G_motion G_mode X Y Z H}
   set rapid_spindle_x_blk {G_motion G_mode X H}
   set rapid_spindle_y_blk {G_motion G_mode Y H}
   set rapid_spindle_z_blk {G_motion G_mode Z H}
...
global mom_sys_control_in mom_sys_control_out mom_tool_name mom_tool_number
...
set cat_length [string length $mom_tool_catalog_number]

if {$cat_length > 0} {set cat_info $mom_tool_catalog_number}

MOM_output_literal "$co PATH NAME  : [string toupper $mom_path_name] WITH TOOL NO: [string toupper $mom_tool_number] $ci"
MOM_output_literal "$co TOOL NAME  : [string toupper $mom_tool_name] TOOL CATALOG NO. [string toupper $cat_info] $ci"
...
global mom_sys_control_in mom_sys_control_out mom_part_unit

# get the tape name and use it for PARTNO

set tape_name "NONE ENTERED  "
if {[info exists mom_group_name]} {
    set tape_name [string range $mom_group_name 0 end]
    }
set units "INCHES"
if {$mom_part_unit != "IN"} { set units "METRIC"}
...
# get some variables
MOM_set_seq_off
MOM_output_literal "$co      [ string toupper [string range $mom_date 0 2]] [string range $mom_date 4 9] [string range $mom_date 22 23]    [ string range $mom_date 11 15] $ci"
MOM_output_literal "$co    PROGRAM NUMBER: [string toupper $tape_name] $ci"
MOM_output_literal "$co    PART DESCRIPTION: [ string toupper [file tail $mom_part_name]] $ci"
MOM_output_literal "$co    *****   $units  PROGRAM   ***** $ci"
}
...
#=============================================================
  MOM_force once S M_spindle X Y Z
}
...
#=============================================================
proc PB_CMD_rapid_check { } {
#=============================================================
global g_flag

if {![info exists g_flag]} {
	set g_flag 1
	MOM_do_template rapid_spindle_1
   }
}


#=============================================================
proc PB_CMD_unset_g_flag { } {
#=============================================================
global g_flag

if {[info exists g_flag]}  { unset g_flag}
