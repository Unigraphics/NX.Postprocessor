########################## TCL Event Handlers ##########################
#
#  Created by sciuccarelli  @  Fri Nov 26 14:18:10 2004 SA Eastern Standard Time
#  with Post Builder version  3.2.1.
#
########################################################################
...
########## SYSTEM VARIABLE DECLARATIONS ##############
  ...
  set mom_sys_spindle_range_code(1)             ""   
  set mom_sys_spindle_param(1,max)              ""   
  set mom_sys_spindle_param(1,min)              ""   
  set mom_sys_spindle_range_code(2)             ""   
  set mom_sys_spindle_param(2,max)              ""   
  set mom_sys_spindle_param(2,min)              ""   
  set mom_sys_spindle_range_code(3)             ""   
  set mom_sys_spindle_param(3,max)              ""   
  set mom_sys_spindle_param(3,min)              ""   
  set mom_sys_spindle_range_code(4)             ""   
  set mom_sys_spindle_param(4,max)              ""   
  set mom_sys_spindle_param(4,min)              ""   
  set mom_sys_spindle_range_code(5)             ""   
  set mom_sys_spindle_param(5,max)              ""   
  set mom_sys_spindle_param(5,min)              ""   
  set mom_sys_spindle_range_code(6)             ""   
  set mom_sys_spindle_param(6,max)              ""   
  set mom_sys_spindle_param(6,min)              ""   
  set mom_sys_spindle_range_code(7)             ""   
  set mom_sys_spindle_param(7,max)              ""   
  set mom_sys_spindle_param(7,min)              ""   
  set mom_sys_spindle_range_code(8)             ""   
  set mom_sys_spindle_param(8,max)              ""   
  set mom_sys_spindle_param(8,min)              ""   
  set mom_sys_spindle_range_code(9)             ""   
  set mom_sys_spindle_param(9,max)              ""   
  set mom_sys_spindle_param(9,min)              ""   
...
#=============================================================
   PB_CMD_tiempo_total
   MOM_force Once Text G_cutcom G_motion X Z T
   MOM_do_template tool_change_axis_preset_2
...
#
#  This command will be called automatically during post processing.
#  After importing this command to your post, you do not need to
#  place it anywhere.
#
...
# change the 0 the third line below to drop leading characters in the tape name

set tape_name " NONE ENTERED  "
if {[info exists mom_group_name] && $mom_group_name != "PROGRAM" && $mom_group_name != "NC_PROGRAM"} {
    set tape_name [string range $mom_group_name 0 [string length $mom_group_name]]
    }
...
MOM_output_literal "(NOMBRE PIEZA: [ string toupper [file tail $mom_part_name]])"
MOM_output_literal "(CREADO POR: [ string toupper $mom_logname])"
MOM_output_literal "(FECHA CREACION: [ string range $mom_date 0 2], [string range $mom_date 4 9], [string range $mom_date 20 23])"
MOM_output_literal "(HORA CREACION: [ string range $mom_date 11 15])"
}
...
#=============================================================
proc PB_CMD_start_of_alignment_character_1 { } {
#=============================================================
 # This command can be used to output a special sequence number character.  
 # Replace the ":" with any character that you require.
 # You must use the command "PB_CMD_end_of_alignment_character" to reset
...
#=============================================================
  MOM_force once S M_spindle X Y Z F M_range
}
...
#=============================================================
proc PB_CMD_tiempo_operacion { } {
#=============================================================
global mom_machine_time tiempo_anterior 

set tiempo [expr $mom_machine_time - $tiempo_anterior]

MOM_output_literal "(TIEMPO DE OPERACION: [format "%4.1f" $tiempo] min.)"
...
MOM_output_literal "(TIEMPO TOTAL: [format "%4.1f" $mom_machine_time] min.)"
}
