########################## TCL Event Handlers ##########################
#
#  Created by suyo  @  Mon Jan 29 13:56:24 2007 China Standard Time
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_rapid_feed_rate                   "1653.5"
  ...
  set mom_kin_x_axis_limit                      "51.2"
  set mom_kin_y_axis_limit                      "47.2"
  set mom_kin_z_axis_limit                      "43.3"
...
#=============================================================
  ...
  global mom_motion_event
   ...
   set rapid_traverse_blk {G_motion G_mode X Y Z fourth_axis S M_spindle}
   set rapid_traverse_xy_blk {G_motion G_mode X Y fourth_axis S M_spindle}
   set rapid_traverse_yz_blk {G_motion G_mode Y Z fourth_axis S M_spindle}
   set rapid_traverse_xz_blk {G_motion G_mode X Z fourth_axis S M_spindle}
...
#=============================================================
  ...
  global first_linear_move ; set first_linear_move 0
   ...
   MOM_force Once Tool_number_indicator G_plane
...
#=============================================================
   ...
   MOM_do_template cycle_off_check
}
...
if {[info exists is_HPCC_mode]} {
  if { $is_HPCC_mode == "ON" } {
    MOM_output_literal "(HPCC MODE OFF)"
    MOM_force ONCE Macro_call HPCC_on
...
if {[info exists mom_head_spindle_axis]} {

    if {$mom_head_spindle_axis(1) == 1} {
       MOM_output_literal "(Right Angle Head)"       

       #right angle used for G18
       set mom_cutcom_plane "XZ"
	   set mom_cycle_spindle_axis 1
	   set mom_sys_spindle_axis(0) 0
	   set mom_sys_spindle_axis(1) 1
	   set mom_sys_spindle_axis(2) 0

    } elseif {$mom_head_spindle_axis(2) == 1} {             

       #right angle used for G17
       set mom_cutcom_plane "XY"
	   set mom_cycle_spindle_axis 2
	   set mom_sys_spindle_axis(0) 0
	   set mom_sys_spindle_axis(1) 0
	   set mom_sys_spindle_axis(2) 1
      } elseif {$mom_head_spindle_axis(0) == 1} {

       MOM_output_to_listing_device "Head spindle axis is not correct."
       MOM_abort "Head spindle axis is not correct."       
     }
} else {
       MOM_output_literal "(spindle axis :Z)"
       set mom_cutcom_plane "XY"
       set mom_cycle_spindle_axis 2
       set mom_sys_spindle_axis(0) 0
       set mom_sys_spindle_axis(1) 0
...
global is_HPCC_mode
 ...
 if { [info exists is_HPCC_mode] && $is_HPCC_mode == "ON"} {       
             MOM_output_literal "(HPCC MODE ON)"
             MOM_force ONCE Macro_call HPCC_on                        
...
MOM_output_literal "(current tool number: $mom_tool_number)"
#prepare for the next tool
 
set is_first_tool 0

if {$mom_next_tool_status == "NEXT"} {
    MOM_output_literal "(tool preselect)"
