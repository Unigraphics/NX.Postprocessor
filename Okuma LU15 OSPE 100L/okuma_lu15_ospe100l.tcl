########################## TCL Event Handlers ##########################
#
#  Created by edson.kawakita  @  Tue Oct 19 11:32:49 2004 E. South America Daylight Time
...
#=============================================================
  ...
  global turret_current mom_warning_info
    ...
    if {$mom_tool_head != "$ind_head" && \
       ...
       set mom_warning_info "mom_tool_head = $mom_tool_head IS INVALID, USING REAR"
       MOM_catch_warning
    }
}
...
if {$flag(debug) == "1"} {
    source [MOM_ask_env_var UGII_CAM_DEBUG_DIR]volker_mom_debug.tcl
...
set last_x [expr $last_x * $mom_sys_lathe_x_factor * $mom_sys_lathe_x_double]

 if {$mom_spindle_speed == $mom_spindle_rpm} {
    set last_spindle $mom_spindle_rpm
 } else {
    set last_spindle [expr ($mom_spindle_speed * 1000) / ($pi * $last_x)]	
 }
 
 if {$last_spindle != 0} {
    if {$last_spindle > 1400} {
       set last_spindle 1400
    }
    MOM_output_literal "G97S[expr int($last_spindle)]"
 }
}


#=============================================================
proc PB_CMD_last_spindle_output { } {
#=============================================================
global last_spindle_flag

if {$last_spindle_flag == "TRUE"} {
    PB_CMD_last_spindle
    set last_spindle_flag "FALSE"
}
}
...
#       MOM_output_literal "G110"
       set g_status "G110"
   } elseif {$mom_ude_turret_index == "B"} {
#       MOM_output_literal "G111"
       set g_status "G111"
   } else {
#       MOM_output_literal "(***   TORRE NAO DEFINIDO   ***)"
        set g_status "(***   TORRE NAO DEFINIDO   ***)"
   }
...
set last_spindle_flag "FALSE"
   ...
   MOM_output_literal "G13 (TORRE A)"
...
proc MOM_call_sub { } {
#=============================================================
  global mom_ude_usin_int
  global mom_ude_usin_int_defined
  global mom_ude_v30
  global mom_ude_v31
  global mom_ude_v40

   if {$mom_ude_usin_int_defined == 1} {
      MOM_output_literal "V30=[format %.3f $mom_ude_v30]"
      MOM_output_literal "V31=[format %.3f $mom_ude_v31]"
      MOM_output_literal "V40=[format %.3f $mom_ude_v40]"
      MOM_output_literal "CALL O[string toupper $mom_ude_usin_int]"
   }
}
}
...
global mom_ude_turret_index
 ...
 if {$mom_ude_turret_type != "NONE"} {
    ...
    MOM_set_seq_on
    MOM_output_literal "RECM=[format %.3f $mom_ude_recm] (ALTURA DO REBAIXO DA CASTANHA)"
    MOM_output_literal "PECA=[format %.3f $mom_ude_peca] (COMPRIMENTO DA PECA)"
    MOM_output_literal "SBMT=[format %.3f $mom_ude_sbmt] (SOBREMETAL P/ PROXIMA OPERACAO)"
    MOM_output_literal "DIS$mom_ude_turret_type=[format %.3f $mom_ude_disa_disb] (DIST P/ TROCA DE FERR)"
    MOM_output_literal "CALL OZOM$mom_ude_turret_type"
    MOM_set_seq_off
    MOM_output_literal "(*********************************************)"
    MOM_set_seq_on
    MOM_do_template spindle_max_rpm
