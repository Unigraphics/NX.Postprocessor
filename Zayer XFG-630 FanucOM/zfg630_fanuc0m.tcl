########################## TCL Event Handlers ##########################
#
#  Created by edson.kawakita  @  Wed Oct 20 15:33:45 2004 E. South America Daylight Time
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_5th_axis_max_limit                "359.9"
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   COOLANT_SET ; CUTCOM_SET ; SPINDLE_SET ; RAPID_SET
   PB_CMD_initial_force_addresses
   PB_CMD_plane_definition
   MOM_force Once G_mode M
   MOM_do_template initial_move_1
   MOM_force Once Text B
...
#=============================================================
   ...
   MOM_force Once G_motion G user_add H M_spindle
   MOM_do_template end_of_path_1
   MOM_force Once G_motion G user_add_1 M_coolant
...
#=============================================================
  ...
  global mom_motion_event
   ...
   RAPID_SET
   set rapid_spindle_blk {G_adjust G_return G_motion X Y Z H}
   set rapid_spindle_x_blk {G_adjust G_return G_motion X H}
   set rapid_spindle_y_blk {G_adjust G_return G_motion Y H}
   set rapid_spindle_z_blk {G_adjust G_return G_motion Z H}
   set rapid_traverse_blk {G_motion G_mode G X Y Z G_plane}
   set rapid_traverse_xy_blk {G_motion G_mode G X Y G_plane}
   set rapid_traverse_yz_blk {G_motion G_mode G Y Z G_plane}
   set rapid_traverse_xz_blk {G_motion G_mode G X Z G_plane}
   set rapid_traverse_mod {G_mode G_plane}
...
global head_attachment
global mom_coolant_status

set co $mom_sys_control_out
set ci $mom_sys_control_in
set max_y_depth 99999.999
set max_z_depth 99999.999
set y_flag 0
set z_flag 0
set tempo_anterior 0
set head_attachment 0
set mom_coolant_status "ON"

 MOM_output_literal :$mom_output_file_basename
 MOM_output_literal "$co\ARCHIVO      : [string toupper $mom_part_name]$ci"
 MOM_output_literal "$co\HECHO POR    : [string toupper $mom_logname]$ci"
 MOM_output_literal "$co\FECHA        : [string toupper $mom_date]$ci"
 MOM_output_literal "$co\MAQUINA      : ZAYER ZFG-630 - COMANDO: FANUC 0-M$ci"
 MOM_output_literal "$co\POSPROCESADOR: [string toupper $mom_event_handler_file_name]$ci"
}
...
proc PB_CMD_ch_flag { } {
#=============================================================
global mom_path_name mom_tool_axis
...
if {[info exists env_var4]} {set l_flag1 [lindex [split $env_var4 @] end]}
}
...
#=============================================================
global principal_axis
global secondary_axis

#MOM_output_literal "$principal_axis - $secondary_axis"
...
MOM_output_literal "$co\TIEMPO DE LA OPERACION: $tempo_oper MIN$ci"
...
if {$y_flag == 1} {
   MOM_output_literal "(PROFUNDIDAD MAXIMA QUE ATINGIO LA HERRAMIENTA EN Y: [format %.3f $max_y_depth] MM)"
}
if {$z_flag == 1} {
   MOM_output_literal "(PROFUNDIDAD MAXIMA QUE ATINGIO LA HERRAMIENTA EN Z: [format %.3f $max_z_depth] MM)"
}

MOM_output_literal "$co\TIEMPO TOTAL DE MAQUINADO: $tempo_u MIN$ci"
}
...
MOM_output_literal "$co$mom_path_name$ci"
}
...
#=============================================================
proc PB_CMD_initial_force_addresses { } {
#=============================================================
  MOM_force once G_adjust G_mode G_return H
}
...
MOM_abort "
				...
				ERROR: UGS/BR-666.
					***
				Suporte Técnico: 11-4224-7111
				http://www.br.ugs.com/suporte/
...
MOM_output_to_listing_device  "\t\tUU     UU    GGGGG       SSSSS     "
MOM_output_to_listing_device  "\t\tUU     UU   GGGGGGG     SSSSSSS    "
MOM_output_to_listing_device  "\t\tUU     UU  GG     GG   SS          "
MOM_output_to_listing_device  "\t\tUU     UU  GG          SS          "
MOM_output_to_listing_device  "\t\tUU     UU  GG   GGGG     SSSSSS    "
MOM_output_to_listing_device  "\t\tUU     UU  GG     GG          SS   "
MOM_output_to_listing_device  "\t\tUU     UU  GG     GG   SS     SS   "
MOM_output_to_listing_device  "\t\t UUUUUUU    GGGGGGG    SSSSSSSS    "
MOM_output_to_listing_device  "\t\t  UUUUU      GGGGG      SSSSSS     "
MOM_output_to_listing_device  "\n\n"
MOM_output_to_listing_device  "\t\tUnigraphics Solutions do Brasil Ltda."
MOM_output_to_listing_device  "\t\t   Suporte Técnico: 11-4224-7111"
MOM_output_to_listing_device  "\t\t   http://www.br.ugs.com/suporte/"
MOM_output_to_listing_device  "\n\n\n\n\n\n\n"
set env_var1 [MOM_ask_env_var UGII_BASE_DIR]
set env_var2 [MOM_ask_env_var UGII_CAM_BASE_DIR]
set env_var3 [MOM_ask_env_var UGII_CAM_POST_DIR]
set env_var4 [MOM_ask_env_var UGII_LICENSE_FILE]
set env_var5 [MOM_ask_env_var UGII_LOAD_OPTIONS]
set env_var6 [MOM_ask_env_var UGII_TMP_DIR]
set env_var7 [MOM_ask_env_var TEMP]
set env_var8 [MOM_ask_env_var TMP]
set env_var9 [MOM_ask_env_var UGII_FLEX_BUNDLE]
...
################################################################################
#
# This is a 3-Axis Milling Machine With Rotary Head and Table.
#
# Zayer ZFG-630 / Fanuc 0-M.
#
#
# Note:
#       Manual head attachment (horizontal-Y and vertical-Z).
#       Indexable table (XY-plane, B-axis).
#
#
# Revisions
#
#   Date        Who   Reason
# 30-aug-2004   emk   Original.
#
#
################################################################################
#
...
global principal_axis
global l_flag2
#set l_flag2 "ugsbr109"
#set l_flag2 "ugsbr132"
#set l_flag2 "coch-bruno"
...
if {$mom_cycle_status == "ON"} {
   if {$mom_cycle_spindle_axis == 1} {
      if {$mom_cycle_feed_to_pos(1) <= $max_y_depth} {
         set max_y_depth $mom_cycle_feed_to_pos(1)
         set y_flag 1
      }
   } elseif {$mom_cycle_spindle_axis == 2} {
      if {$mom_cycle_feed_to_pos(2) <= $max_z_depth} {
         set max_z_depth $mom_cycle_feed_to_pos(2)
         set z_flag 1
      }
   }
} else {
   if {$mom_cycle_spindle_axis == 1} {
      if {$mom_pos(1) <= $max_y_depth} {
         set max_y_depth $mom_pos(1)
         set y_flag 1
      }
   } elseif {$mom_cycle_spindle_axis == 2} {
...
MOM_output_literal "$mom_sys_spindle_axis(0)"
MOM_output_literal "$mom_sys_spindle_axis(1)"
MOM_output_literal "$mom_sys_spindle_axis(2)"
...
#=============================================================
  MOM_force once G_adjust H S X Y Z I J K
}
...
#=============================================================
proc PB_CMD_verifica_spindle_axis { } {
#=============================================================
global mom_cycle_spindle_axis
global mom_tool_axis
global mom_cutcom_plane
...
if {$mom_tool_axis(2) != 1} {
   ...
   set mom_pos_arc_plane ZX
   set principal_axis Y
   set head_attachment 20
   set secondary_axis Z
} else {
   set mom_cycle_spindle_axis 2
   set mom_cutcom_plane XY
   set mom_pos_arc_plane XY
   set principal_axis Z
   set head_attachment 21
   set secondary_axis Y
}
}
