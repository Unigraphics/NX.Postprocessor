...
#
#  Created by xiongx  @  Tue Jun 10 10:55:38 2008 China Standard Time
...
####### KINEMATIC VARIABLE DECLARATIONS ##############
  ...
  set mom_kin_tool_change_time                  "12" 
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   ...
   PB_CMD_output_m128_first
   PB_CMD_def_work_plane
   MOM_force Once G_motion fmax
...
#=============================================================
  ...
  global mom_kin_max_fpm mom_motion_event
   ...
   PB_CMD_go_home_pos
   MOM_force Once Text Text S
   MOM_do_template tool_change
   PB_CMD_call_LBL_500
   PB_CMD_output_m128_init
   PB_CMD_def_work_plane
   MOM_force Once G_motion fmax
   MOM_do_template initial_move_xy
   MOM_force Once G_motion fmax
...
#=============================================================
   ...
   PB_CMD_UPLEVEL_multiaxis_control_mode
   PB_CMD_uplevel_PB_PAUSE
   PB_CMD_uplevel_before_output
...
#=============================================================
proc PB_CMD_LBL_500_output { } {
#=============================================================
MOM_output_literal "LBL 500"
#MOM_output_literal "M140 MB MAX F99999"
MOM_output_literal "M129"
MOM_output_literal "M22"
MOM_output_literal "PLANE RESET STAY"
MOM_output_literal "L  Z-0.1 R0 FMAX M91"
MOM_output_literal "L  X+3  Y-0.1 R0 FMAX M91"
MOM_output_literal "L  B+0  C+0 FMAX M9"
...
MOM_output_literal "L  C+0  B+0 FMAX"
...
#=============================================================
 ...
 if {$li_5axis_mode} {  
    ...
    MOM_suppress once m128
 }
}


#=============================================================
proc PB_CMD_UPLEVEL_multiaxis_control_mode { } {
#=============================================================
uplevel #0 {
...
#=============================================================
proc MOM_multiaxis_control_mode {} {
#=============================================================
# This proc is used to overwrite the automatical M128/CYCL 19 
# output by users. This UDE function is non-model.

     global mom_5axis_control_pos   
     global mom_5axis_control_mode

     global mom_multiaxis_control_mode
     set mom_5axis_control_mode $mom_multiaxis_control_mode

     if {$mom_multiaxis_control_mode == "M128"} {
        set mom_5axis_control_pos 0
     } else {
...
#=============================================================
proc PB_CMD_call_LBL_500 { } {
#=============================================================
MOM_output_literal "CALL LBL 500"
}
...
#=============================================================
   ...
   if { [string match "CLW" $mom_arc_direction] } {
      set circle_direction "DR-"
   } elseif { [string match "CCLW" $mom_arc_direction] } {
      set circle_direction "DR+"
   }
}
...
#=============================================================
 #<lili 04-01-2008> reversion
 # 2nd clearance plane Q203 - retract to 
 # Add bore_dwell_move into ream cycle 201
 # Add retraction feed rate
 # Add drilling type 240
 
  global mom_cycle_retract_to mom_cycle_rapid_to_pos mom_feed_retract_value
  global cycle_init_flag  mom_cycle_delay feed mom_cycle_option
  ...
  global cycle_peck_size  cycle_type_number cycle_type_name;# js
  ...
  global peck_select_dia peck_dia_value

 #----- Heidenhain cycle types and names-------------------------------------------------
  if {$mom_motion_event == "bore_move" || $mom_motion_event == "bore_dwell_move"} {
      set cycle_type_number 201    ;# ream
      set cycle_type_name "REAMING ~"
  } elseif {$mom_motion_event == "bore_drag_move"} {
      set cycle_type_number 202    ;# bore
      set cycle_type_name "BORING ~"
  } elseif {$mom_motion_event == "tap_move"} {
      set cycle_type_number 207    ;# rigid tapping
      set cycle_type_name "TAPPING ~"
  } elseif {$mom_motion_event == "drill_move" || $mom_motion_event == "drill_dwell_move"} {
      set cycle_type_number 240  ;# centering
      set cycle_type_name "CENTERING ~"
  } else {
      set cycle_type_number 200    ;# drill & peck
      set cycle_type_name "DRILLING ~"
  }
 #------ Delay -------------------------------------------------------------------------
 ...
 #------Feedrate----------------------------------------------------------------
  ...
  global cycle_retract_feed
  if {[info exists mom_feed_retract_value] && ![EQ_is_zero $mom_feed_retract_value]} {
	  set cycle_retract_feed $mom_feed_retract_value
  } else {
	  set cycle_retract_feed $feed
  }
 ...
 #----- centering base------------------------------------------------------------------------------ 
  global mom_cycle_counter_sink_dia mom_cycle_hole_dia
  if {[info exists mom_cycle_counter_sink_dia] && ![EQ_is_zero $mom_cycle_counter_sink_dia]} {
      set peck_select_dia 1
      set peck_dia_value [expr ($mom_cycle_counter_sink_dia*(-1.0))]
      set mom_cycle_feed_to 0.0
  } else {
      set peck_select_dia 0
      set peck_dia_value 0.0
  }
 ...
 # to ensure the hole Z height Q203 is set correctly.
  ...
  if { ![EQ_is_equal $mom_pos(2) $mom_prev_pos(2)] } {set cycle_init_flag  "TRUE"}

  if {$cycle_init_flag == "TRUE"} {
    MOM_force once cycle_rapid_to cycle_feed_to cycle_surface cycle_pullback cycle_feedrate cycle_return_feed cycle_dwell_top cycle_dwell cycle_tap_lead cycle_orient cycle_dia cycle_q343
      switch $cycle_type_number {
        200 {
              MOM_do_template cycle_block
              MOM_set_seq_off
              MOM_do_template cycle_block_200     ;# Q200 setup ( engage ) ht
              MOM_do_template cycle_block_201     ;# Q201 hole depth ( incremental )
              MOM_do_template cycle_block_206     ;# Q206 feedrate units/min
              MOM_do_template cycle_block_202     ;# Q202 peck size
              MOM_do_template cycle_block_210     ;# Q210 dwell above hole hardwired to zero
              MOM_do_template cycle_block_203     ;# Q203 Z of top of hole ( absolute )
              MOM_do_template cycle_block_204     ;# Q204 Zreturn after cycle ( incremental )
              MOM_do_template cycle_block_211     ;# Q211 cycle dwell]
              MOM_set_seq_on
            }
        201 {
              MOM_do_template cycle_block
              ...
              MOM_do_template cycle_block_206     ;# Q206 feedrate units/min
              MOM_do_template cycle_block_211     ;# Q211 cycle dwell
              MOM_do_template cycle_block_208     ;# Q208 retraction feed rate
              MOM_do_template cycle_block_203     ;# Q203 Z of top of hole ( absolute )
              MOM_do_template cycle_block_204     ;# Q204 Zreturn after cycle ( incremental )
              MOM_set_seq_on
            }
        202 {
              ...
              MOM_do_template cycle_block_203     ;# Q203 Z of top of hole ( absolute )
              MOM_do_template cycle_block_204     ;# Q204 Zreturn after cycle ( incremental )
              MOM_do_template cycle_block_214     ;# Q214 Disengaging direction
              MOM_do_template cycle_block_336     ;# Q336 Angle of spindle
              MOM_set_seq_on
            }
        207 {
              MOM_do_template cycle_block
              ...
              MOM_do_template cycle_block_200     ;# Q200 setup ( engage ) ht
              MOM_do_template cycle_block_201     ;# Q201 hole depth ( incremental )
              MOM_do_template cycle_block_239     ;# Q239 pitch
              MOM_do_template cycle_block_203     ;# Q203 Z of top of hole ( absolute )
              MOM_do_template cycle_block_204     ;# Q204 Zreturn after cycle ( incremental )
              MOM_set_seq_on
            }
	240 {
              ...
              MOM_set_seq_off
              MOM_do_template cycle_block_200     ;# Q200 setup ( engage ) ht
	      MOM_do_template cycle_block_343     ;# Q343 select depth/diameter
              MOM_do_template cycle_block_201     ;# Q201 hole depth ( incremental )
	      MOM_do_template cycle_block_344     ;# Q344 select depth/diameter
              MOM_do_template cycle_block_206     ;# Q206 feedrate units/min
              MOM_do_template cycle_block_211     ;# Q211 cycle dwell]
              MOM_do_template cycle_block_203     ;# Q203 Z of top of hole ( absolute )
              MOM_do_template cycle_block_204     ;# Q204 Zreturn after cycle ( incremental )
...
# case where cycles stay down
  ...
  global mom_cycle_retract_mode mom_cycle_rapid_to_pos mom_cycle_rapid_to
  global mom_mcs_goto mom_pos ang_A ang_C cycle_rapid_mcs_goto
  global DEG2RAD PI
  
 
#MOM_output_text "___$mom_mcs_goto(2) ___$mom_cycle_rapid_to  $cycle_rapid_mcs_goto(2)"

 
       MOM_force Once G_motion Z Text
...
#=============================================================
proc PB_CMD_datum_shift { } {
#=============================================================
 global start_program_flag

if {$start_program_flag} {
     MOM_output_literal "CYCL DEF 7.0 DATUM SHIFT"
     MOM_output_literal "CYCL DEF 7.1 X+0"
     MOM_output_literal "CYCL DEF 7.2 Y+0"
     MOM_output_literal "CYCL DEF 7.3 Z+0"
...
#=============================================================
   global li_5axis_mode ang_B ang_C li_cycl19_output
   global work_plane_defined feed li_last_m128_output
   global mom_pos mom_mcs_goto init_opr 
   ...
   if {!$li_5axis_mode} {
      ...
      set ang_B    [format %.3f "$mom_out_angle_pos(0)"]
      set ang_C    [format %.3f "$mom_out_angle_pos(1)"]

     #>>>
#	  MOM_output_text "____$mom_out_angle_pos(0) $mom_prev_out_angle_pos(0)"
#		 MOM_output_text "____$mom_out_angle_pos(1) $mom_prev_out_angle_pos(1)"
      if {![EQ_is_equal $mom_out_angle_pos(0) $mom_prev_out_angle_pos(0)] || ![EQ_is_equal $mom_out_angle_pos(1) $mom_prev_out_angle_pos(1)] } {
          ...
          MOM_output_literal "PLANE SPATIAL SPA+$ang_B SPB+0 SPC+$ang_C TURN FMAX COORD ROT"
	  MOM_force once X Y Z
          MOM_force always fmax	  
      } elseif {[info exists li_last_m128_output] && $li_last_m128_output == 1} {
          if {[EQ_is_zero $ang_B]} {
              set ang_C 0.0
	      set mom_out_angle_pos(1) 0.0
	      set mom_pos(3) 0.0
          ...
          set li_last_m128_output 0 	  		  
          set init_opr 0
          MOM_output_literal "PLANE SPATIAL SPA+$ang_B SPB+0 SPC+$ang_C TURN FMAX COORD ROT"
          MOM_force once X Y Z
          MOM_force always fmax
      }
	  set mom_prev_out_angle_pos(0) $ang_B
	  set mom_prev_out_angle_pos(1) $ang_C
...
#=============================================================
proc PB_CMD_def_work_plane_cycle { } {
#=============================================================
   global li_5axis_mode ang_B ang_C li_cycl19_output
   global mom_kin_4th_axis_min_limit  mom_kin_4th_axis_max_limit
     ...
     #>>>
      ...
      if { $mom_out_angle_pos(0) != $mom_prev_out_angle_pos(0) || $mom_out_angle_pos(1) != $mom_prev_out_angle_pos(1) } {
          if {[EQ_is_zero $ang_B]} {
           ...
           #   MOM_output_literal "PLANE SPATIAL SPA+0 SPB+0 SPC+0 MOVE F10000"
           #   MOM_output_literal "L A+Q120 C+Q122 F10000" 
	  }
          MOM_output_literal "PLANE SPATIAL SPA+$ang_B SPB+0 SPC+$ang_C TURN FMAX COORD ROT"
          set li_cycl19_output 1
      }
	  set mom_prev_out_angle_pos(0) $ang_B
          set mom_prev_out_angle_pos(1) $ang_C 
	  MOM_reload_variable -a mom_out_prev_angle_pos
...
# This flag used to output 5axis or 3plus2 mode. zero for 3plus2.
set li_5axis_mode 0

if {$mom_5axis_control_mode == "AUTO"} {
...
# Reset work plane rotation before M128
global save_5axis_mode
if {![info exists save_5axis_mode]} {
    set save_5axis_mode $li_5axis_mode
}

# 3plus2 mode , reload kinematics to dual-table
if {!$li_5axis_mode} {
    global mom_kin_machine_type mom_kin_4th_axis_plane mom_kin_5th_axis_plane
	global mom_kin_4th_axis_type mom_kin_5th_axis_type
	global mom_kin_5th_axis_max_limit mom_kin_5th_axis_min_limit mom_kin_4th_axis_max_limit mom_kin_4th_axis_min_limit
    ...
    if {$save_5axis_mode != $li_5axis_mode} {
...
set from_pos(0) $mom_pos(0)
set from_pos(1) $mom_pos(1)
set from_pos(2) $mom_pos(2)
set from_pos(3) $mom_out_angle_pos(0)
set from_pos(4) $mom_out_angle_pos(1)
}


#=============================================================
proc PB_CMD_from_move_output { } {
#=============================================================
global from_pos
global mom_pos mom_out_angle_pos

if {[array exists from_pos]} {
   set save_pos(0) $mom_pos(0)
   set save_pos(1) $mom_pos(1)
   set save_pos(2) $mom_pos(2)
   set save_pos(3) $mom_out_angle_pos(0)
   set save_pos(4) $mom_out_angle_pos(1)

   set mom_pos(0) $from_pos(0)
   set mom_pos(1) $from_pos(1)
   set mom_pos(2) $from_pos(2)
   set mom_out_angle_pos(0) $from_pos(3)
   set mom_out_angle_pos(1) $from_pos(4)

   MOM_rapid_move

   set mom_pos(0) $save_pos(0)
   set mom_pos(1) $save_pos(1)
   set mom_pos(2) $save_pos(2)
   set mom_out_angle_pos(0) $save_pos(3)
   set mom_out_angle_pos(1) $save_pos(4)
...
#=============================================================
proc PB_CMD_go_home_end { } {
#=============================================================
 #  MOM_output_literal "CYCL DEF 19.0 WORK PLAN"
...
if {0} {
 ...
 if {!$li_5axis_mode} {
	 MOM_output_literal "CYCL DEF 19.0 WORK PLAN"
	 MOM_output_literal "CYCL DEF 19.1 A0.0 C0.0"
  	 MOM_output_literal "CYCL DEF 19.0 WORK PLAN"
     MOM_output_literal "CYCL DEF 19.1 "
    # MOM_output_literal ";-------------------------------"
     MOM_output_literal "L M128"
     MOM_output_literal "L A+0 FMAX"
	 MOM_output_literal "M129"
 }
 }
  #  MOM_output_literal "L Z+800 FMAX M91"

  # MOM_output_literal ";"
  # MOM_output_literal "CYCL DEF 19.0 WORK PLAN"
  # MOM_output_literal "CYCL DEF 19.1 A0.0 C0.0"
  # MOM_output_literal "L A+Q120 C+Q122 R0 F MAX"
  # MOM_output_literal "CYCL DEF 19.0 WORK PLAN"
  # MOM_output_literal "CYCL DEF 19.1 "
  # MOM_output_literal ";-------------------------------"
}
...
#
   ...
   while {[EQ_is_ge [expr abs($helix_angle)] 5400]} {
       if {$helix_angle>0} {
          set factor 1
       } else {
	  set factor -1
       }
       set helix_angle_last [expr $helix_angle - $factor*5400] 
	   
       set helix_angle [expr $factor*5400]
       set helix_height_once [expr $helix_angle*$mom_helix_pitch/(360*$helix_direction)]
       set helix_height_last [expr $helix_height - $helix_height_once]
       set helix_height $helix_height_once
       if {![EQ_is_zero $helix_height]} {
	  ...
	  MOM_do_template circular_move_1
       ...
       set helix_height $helix_height_last
       set helix_angle $helix_angle_last
   }
...
#
#<lili 03-22-2008> hole making issue, output circular instead of helix
   if {![EQ_is_zero $helix_height]} {
      MOM_do_template coolant_on
      MOM_force Once Text I J
      MOM_suppress Once K
...
#=============================================================
proc PB_CMD_init_operation_output { } {
#=============================================================
   global mom_path_name mom_date mom_output_file_basename mom_output_file_suffix
   global mom_tool_diameter mom_tool_name mom_tool_number
   global mom_tool_diameter mom_tool_corner1_radius
   global mom_sys_control_out mom_sys_control_in
   global start_program_flag init_opr
   ...
   if {[info exists mom_tool_corner1_radius]} {
       set lower_radius "R=[format %.3f $mom_tool_corner1_radius]"
   }
   
   if {[info exists mom_tool_diameter]} {
       set tool_diameter "D=[format %.3f $mom_tool_diameter]"
   }

 if {$start_program_flag} {
   MOM_output_literal "$co $mom_output_file_basename.$mom_output_file_suffix $ci"
   MOM_output_literal "$co $mom_date $ci"
 }
   MOM_output_literal "$co TOOL=0 $tool_diameter $lower_radius $ci"
 if {$start_program_flag} {
   MOM_output_literal "$co 5 Axis Machine Program $ci"
   MOM_output_literal "$co $ci"
 }

 set start_program_flag 0
}
...
#=============================================================
   global rad2deg
   global ini_prog last_F last_RPM last_spindle_dir
   global PI
   ...
   global start_program_flag
   set start_program_flag 1
}


#=============================================================
proc PB_CMD_ken_output_m128 { } {
#=============================================================
global ken_output_m128

if {$ken_output_m128 == 1} {
MOM_output_literal "M128"
set ken_output_m128 0
}
}
...
set machine_time [format %.2f "$mom_machine_time"]
MOM_output_literal "$co Machine Time : $machine_time min $ci"
}
...
#=============================================================
#  Rotina alterada e testada para comando iTNC530: OK!
...
uplevel #0 {
 ...
 #-------------------------------------------------------------------------------
   ...
   for {set ii 0} {$ii < $mom_nurbs_point_count} {incr ii} {
      ...
      #------------- Rotina para converter coeficientes para formato "x.yyy Ez" (ex.: "10.7234" para "1.0723 E1")
      for {set kk 0}  {$kk < 9} {incr kk} {
            set ind_knot($kk) 0
            if { [expr abs($mom_nurbs_coefficients($ii,$kk)) >= 10] } {

    #           MOM_output_literal "==>------------------------------------------------"
    #           MOM_output_literal "==> $mom_nurbs_coefficients($ii,$kk)"
    #           MOM_output_literal "==>------------------------------------------------"

                if { [expr abs($mom_nurbs_coefficients($ii,$kk)) < 100] } {
                    set ind_knot($kk) 1
                    set mom_nurbs_coefficients($ii,$kk) [expr $mom_nurbs_coefficients($ii,$kk) / 10]
                } elseif { [expr abs($mom_nurbs_coefficients($ii,$kk)) < 1000] } {
                    set ind_knot($kk) 2
                    set mom_nurbs_coefficients($ii,$kk) [expr $mom_nurbs_coefficients($ii,$kk) / 100]
                }
            }
      }
      #-----------------------------------------------------------------------------------------------------------------
      ...
      for {set jj 0}  {$jj < 9} {incr jj} {
            if {$ind_knot($jj) != 0} {
                switch $jj {
                    0 {MOM_add_to_address_buffer K3X END "E$ind_knot($jj)"}
                    1 {MOM_add_to_address_buffer K2X END "E$ind_knot($jj)"}
                    2 {MOM_add_to_address_buffer K1X END "E$ind_knot($jj)"}
                    3 {MOM_add_to_address_buffer K3Y END "E$ind_knot($jj)"}
                    4 {MOM_add_to_address_buffer K2Y END "E$ind_knot($jj)"}
                    5 {MOM_add_to_address_buffer K1Y END "E$ind_knot($jj)"}
                    6 {MOM_add_to_address_buffer K3Z END "E$ind_knot($jj)"}
                    7 {MOM_add_to_address_buffer K2Z END "E$ind_knot($jj)"}
                    8 {MOM_add_to_address_buffer K1Z END "E$ind_knot($jj)"}
                }
            }
     }
...
#=============================================================
proc PB_CMD_output_begin { } {
#=============================================================
global mom_group_name mom_operation_name mom_output_unit

if {$mom_output_unit == "IN"} {set mom_output_unit INCH}

if [info exist mom_group_name] {
MOM_output_literal "0 BEGIN PGM $mom_group_name $mom_output_unit"
} else {
MOM_output_literal "0 BEGIN PGM $mom_operation_name $mom_output_unit"
}

if {$mom_output_unit == "INCH"} {set mom_output_unit IN}
}


#=============================================================
proc PB_CMD_output_cycl_19_ac_linear { } {
#=============================================================
   global li_cycl19_output ang_A ang_C
   if {[info exists li_cycl19_output] && $li_cycl19_output==1} {
       MOM_output_literal "PLANE SPATIAL SPA$ang_A SPB+0 SPC$ang_C MOVE F10000"
    #   MOM_output_literal "L A+Q120 C+Q122 F10000" 
       MOM_force once X Y Z
       set li_cycl19_output 0
   }
}


#=============================================================
proc PB_CMD_output_cycl_19_ac_rapid { } {
#=============================================================
   global li_cycl19_output ang_A ang_C
   if {[info exists li_cycl19_output] && $li_cycl19_output==1} {
       MOM_output_literal "PLANE SPATIAL SPA$ang_A SPB+0 SPC$ang_C MOVE F10000"
...
if [info exist mom_group_name] {
MOM_output_literal "END PGM $mom_group_name $mom_output_unit"
} else {
MOM_output_literal "END PGM $mom_operation_name $mom_output_unit"
}
...
#=============================================================
proc PB_CMD_output_m128_first { } {
#=============================================================
   global li_5axis_mode ang_A ang_C li_cycl19_output
   global li_last_m128_output
   global ken_output_m128
   set ken_output_m128 0
   ...
   if {$li_5axis_mode} {
      ...
      if { ![info exists li_last_m128_output] || $li_last_m128_output == 0 } {
         #MOM_output_literal "L M128"
		 set ken_output_m128 1
      } 
   #   set li_last_m128_output 1
	  MOM_force Once X Y Z fourth_axis fifth_axis F G_cutcom

   } else {
      global mom_out_angle_pos last_B last_C
      global mom_prev_out_angle_pos

      if { [info exists li_last_m128_output] && $li_last_m128_output == 1 } {
          MOM_output_literal "M129"
      }
     # set li_last_m128_output 0
   }
...
#=============================================================
proc PB_CMD_output_m128_init { } {
#=============================================================
   global li_5axis_mode ang_A ang_C li_cycl19_output
   global li_last_m128_output feed   
  # set li_last_m128_output 0
   global ken_output_m128
   set ken_output_m128 0
...
if {0} {
       global li_csys_flag
       global ang_A ang_B ang_C
       if {[info exists li_csys_flag] && $li_csys_flag == 1} {
           MOM_output_literal "PLANE SPATIAL SPA+$ang_B SPB+0 SPC+$ang_C TURN FMAX COORD ROT"
           set li_csys_flag 0
       }	
}	    
...
#=============================================================
proc PB_CMD_reload_rotary_angle { } {
#=============================================================
   global mom_prev_out_angle_pos
...
#=============================================================
 global init_opr mom_out_angle_pos
...
#=============================================================
   ...
   global last_spindle_dir mom_spindle_direction
  ...
  #<12-05-07 gsl> Corrected logic
   ...
   if { ![info exists mom_spindle_direction] } {
      ...
      set last_spindle_dir $mom_spindle_direction
   }
}
...
global mom_operation_name mom_main_mcs
global mom_csys_matrix

if { [EQ_is_equal $mom_csys_matrix(0) 1.0] && [EQ_is_equal $mom_csys_matrix(4) 1.0] && [EQ_is_equal $mom_csys_matrix(8) 1.0] } {return}

if {$mom_kin_coordinate_system_type == "LOCAL"} {		
	PB_PAUSE "Local CSYS Error" "CSYS Error, Please set to \"Use Main MCS\" or \"CSYS Rotation\" \n Operation: $mom_operation_name"
} elseif {$mom_kin_coordinate_system_type == "CSYS"} {	
	PB_PAUSE "CSYS Warning" "CSYS Rotation are using in \n $mom_operation_name \n";# Spatial Angle will be output"
return			  
	global li_csys_flag
	set li_csys_flag 1 
  ...
  if {[EQ_is_equal $cos_b_sq 0.0]} {
    ...
    VEC3_scale scale ut wt

    if {$m2 > 0} {
      set A [expr -$RAD2DEG * acos($m8)]
	  set B 270.0
      set C [expr $RAD2DEG * acos($wt(1))]
    } else {
      set A [expr $RAD2DEG * acos($m8)]
	  set B 90.0
      set C [expr $RAD2DEG * acos($wt(1)) - 180.0]
    }
  ...
  global ang_A ang_B ang_C
  set ang_A [format "%.${rotary_decimals}f" $A]
  set ang_B [format "%.${rotary_decimals}f" $B]
  set ang_C [format "%.${rotary_decimals}f" $C]	
...
return
  ...
  global li_disable_post_pause
    if { [info exists li_disable_post_pause] &&  $li_disable_post_pause == 1 } {
return
  }
  ...
  set post_dir "[file dirname $mom_event_handler_file_name]"
 
  if { [file exists ${cam_aux_dir}$ug_wish] && [file exists ${post_dir}//post_pause.tcl]} {

     set title "POST Warning"
     set msg ""
    ...
    set res [exec ${cam_aux_dir}$ug_wish ${post_dir}//post_pause.tcl $title $msg]
	switch $res {
       no {
          set li_disable_post_pause 1
       }
       cancel {        
...
#=============================================================
proc PB_CMD_uplevel_before_output { } {
#=============================================================
#************
uplevel #0 {
   if { [llength [info commands "MOM_before_output_sim"]] } {
      rename MOM_before_output_sim ""
   }

   if { [llength [info commands "MOM_before_output"]] } {
      rename MOM_before_output MOM_before_output_sim
   } else {
      proc MOM_before_output_sim {} {}
   }
...
#=============================================================
proc PB_CMD_verif_mov_angular { } {
#=============================================================
   global mom_out_angle_pos mom_path_name
   global mom_prev_out_angle_pos
...
#=============================================================
   global last_RPM mom_spindle_speed mom_spindle_direction last_spindle_dir
   if { $last_RPM != $mom_spindle_speed || $last_spindle_dir != $mom_spindle_direction} {
      #MOM_suppress once T
	  if {$last_spindle_dir != $mom_spindle_direction} {
		 MOM_output_literal "M5"
	  }
      MOM_force once spindle_Q1631
      MOM_do_template spindle_speed
	  MOM_output_literal "TOOL CALL SQ1631"
      set last_RPM $mom_spindle_speed
	  if {$last_spindle_dir == $mom_spindle_direction} {
