...
#
#  Created by suyo  @  Thu Aug 30 13:49:01 2007 China Standard Time
...
#=============================================================
proc PB_CMD_index_rotary_table_old { } {
#=============================================================
#
# This custom command will output M codes to index the
# rotary index_table to preset stops.
...
global mom_index_number
global mom_mcode_start
global first_index_number

global mom_prev_pos
global mom_pos

if {[info exists mom_prev_pos]} {
if {[EQ_is_equal $mom_prev_pos(3) $mom_pos(3)]} {
return
}
}

   set index_code $mom_mcode_start
...
# defines eight stops each 45 degrees apart.  
#
   if {[EQ_is_equal $mom_index_number 0]} {
      set mom_warning_info "the index number should be greater than 0"
      MOM_catch_warning
      return
   }
    set angle_factor [expr 1.0/$mom_index_number]
  ...
  # set total_stops [llength $stops]
   ...
   for {set i 0} {$i < $mom_index_number} {incr i} {
      if { $stops($i) == $mom_pos(3)} {
         set index_pos $i
		 set index_code [expr $mom_mcode_start+$i] 
         break
      }
   }
   ...
   if {$delta_index < 0} {set delta_index [expr $delta_index + $mom_index_number]}    
   for {set i 0} {$i <= $delta_index} {incr i} {
	   if { [EQ_is_equal $first_index_number 1]} {
		   set first_index_number 0
		   MOM_output_literal "M$index_code (A[format %.3f $mom_pos(3)])"
	    }
	  if {![EQ_is_equal $pre_index_pos $index_pos]} { 
      MOM_output_literal "M$index_code (A[format %.3f $mom_pos(3)])" 
	  }     
   }
