########################## TCL Event Handlers ##########################
#
#  Created by kleber.ronconi  @  Thu Apr 08 14:12:23 2004 E. South America Daylight Time
...
#=============================================================
  ...
  global mom_motion_event
   ...
   set rapid_traverse_blk {G_motion X Y Z S Text G_cutcom M_spindle}
   set rapid_traverse_xy_blk {G_motion X Y S Text G_cutcom M_spindle}
   set rapid_traverse_yz_blk {G_motion Y Z S Text G_cutcom M_spindle}
   set rapid_traverse_xz_blk {G_motion X Z S Text G_cutcom M_spindle}
...
set tempo_anterior   0
 ...
 MOM_output_literal "; MAQUINA  : ??? - COMANDO: ?TNC???"
 MOM_output_literal "; -------------------------------------------------------------- "
...
#_______________________________________________________________________________
  ...
  for {set ii 0} {$ii < $mom_nurbs_point_count} {incr ii} {
	  ...
	  for {set kk 0}  {$kk < 9} {incr kk} {
	        set ind_knot($kk) 0
	        if {abs($mom_nurbs_coefficients($ii,$kk)) >= "10"} {
	#        	MOM_output_literal "==>------------------------------------------------"
	#        	MOM_output_literal "==> $mom_nurbs_coefficients($ii,$kk)"
	#        	MOM_output_literal "==>------------------------------------------------"
	
	        	if {abs($mom_nurbs_coefficients($ii,$kk)) < "100"} {
	        	    set ind_knot($kk) 1
	        	    set mom_nurbs_coefficients($ii,$kk) [expr $mom_nurbs_coefficients($ii,$kk) / 10]
	        	} elseif {abs($mom_nurbs_coefficients($ii,$kk)) < "1000"} {
	        	    set ind_knot($kk) 2
